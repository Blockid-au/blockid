...
- **Degraded runs are diagnosable** — the audit dump is written even when a run throws, with the provider strike ledger, per-wave timings and the wave that hit the deadline; quality rows carry `providers_struck` / `deadline_hit_wave`; `/api/status.tbr_quality.last_degraded` and the `/admin/funnel` Trust row say "last run degraded (providers struck: …)" next to the last good share instead of a placeholder.
- **Persona-aware chrome** — an evaluator / program seat never sees the founder phase banner, founder tabs or "Scout keeps doing…" copy; one `personaFor(seat)` rule with tests and a live-qa pin. The cookie-prefs, feedback and report-chat pills share one bottom-right stack (safe-area aware, lifted above sticky CTAs; a duplicate feedback button removed). The Google sign-in button's report-only CSP line is allow-listed in the page sweep.
- **Startup Index movers** — a listing with no prior close is "new" (never −99), "Biggest drops" only ever holds negative changes, and the index and listings carry a "Sample data" label while fewer than 30 companies are tracked.
- **Docs** — the founder's planning session rewrote `docs/plans/SOURCE-OF-TRUTH.md` as the G30 proposal (PROPOSED — awaiting founder review, no code); the previous SOT is archived under `docs/archive/`.
...
- **P2** — guest analyses (no account) are erased by the delivery address (0442); the AU legal / context / SaaS knowledge rows print as external references, not "Data connector"; a sentence that states a fact ("… which currently serves 1,200–1,500 customers …") is never exempt from the claim gate even inside advice.
- **Honesty** — the v3.27.0 line "free reports are the v3 document" was not true on production until this release; the ≥ 0.85 grounding KPI remains unverified live (three showcase runs degraded on provider outages on 2026-09-21).

## 2026-09-21 — v3.27.0: Grounding rules, provider resilience, free reports on the v3 document, print + verdict-band demo, Startup Value Index light (G28)

- **Free reports are the v3 document (G28-C)** — the first two free business reports per e-mail (guest or account) and an entitled member's intake run now go through the same ReportV2 pipeline the paid Trusted Business Report uses (`orchestrateReport`, tier standard, the full 16-section document, no trim) instead of the S32 seven-voice PDF. The document is stored on the `analyses` row (envelope `version: "tbr-v2"` in `full_report_json`, no migration), `/analyze` renders it with the same `<TbrReportV2>` as the workspace and `/tbr/<token>`, the e-mail body is the 1-page investment view with the PDF twin attached and signed page + PDF links (the existing download-token path — `/analyze/<id>?t=…` opens the report from a mail client with no cookie), and the free-allowance ledger is stamped `sent` when the e-mail goes out. Every G25-C guard is unchanged (address required, disposable / honeypot / per-IP limits, daily cap → queued, ledger sequence, A$3 quote on the third run); one pipeline run per grant (a stored document is never re-orchestrated; only a run that produced nothing is retried by the cron). The S32 runner stays only for rows written before G28 (`dispatch.ts` routes by stored shape). Spec § 6 back to "full standard document". Live-qa 43 gains the real-run assertions (poll `kind: "v2"`, 8 chapters, PDF, ledger `sent`) behind `LIVE_QA_SPEND_OK` and a spend-free signed-link probe.
- **Grounding rules for the residual patterns (G28-A)** — a rate derived from two evidence rows, a statutory fee, a content cadence or a market sub-count must be cited from a row or written as a declared estimate ("We estimate … (unevidenced)") — never a bare number; new topic-gated knowledge rows (`au-legal`: ASIC annual review band + trade-mark fee range with sources; `sector-entities` only when the AU market anchor names the industry) with provenance the critic sees; Vietnamese estimate markers; the contract cap no longer trims the score-ledger rule off the prompt tails; prompt-eval fixture `TBR-grounding-v2.5.0`. **The ≥ 0.85 showcase KPI is pinned deterministically but not yet verified live** — both paid verification runs degraded fully during a DeepInfra outage.
- **Provider resilience (G28-B)** — per-stage model timeouts (criterion 60 s, chapter / synthesis 120 s; env overrides), run-scoped provider strikes (two worker timeouts or `engine_overloaded` / 429 answers skip that provider for the rest of the run so the chain falls through quickly), a 120 s wall-clock reserve for the synthesis wave, and a run with ≥ 7 degraded chapters is never persisted as a report (one digest line instead). The first-analysis cron starts v2 runs without awaiting them (a run outlives the 290 s cron clamp).
- **Print + verdict bands (G28-D)** — A4 `@page` + print rules (chrome hidden, no orphan headings, table rows intact) with a print probe; `/tbr/demo?band=A|B|C|D` renders the four investment-view bands from fixtures with a band switcher.
- **Startup Value Index on the light template (G28-D)** — startupvalueindex.com: navy / cyan tokens, orange kept graphic-only (210 orange text uses at 2.0:1 fixed), no OS auto-dark, 12 px minimum, 44 px targets, sticky ticker column at 375 (deployed separately after this release).
- **QA** — the page sweep compares pathnames for query-string visits; lane 21 pins the explicit "Pay A$3 & get my report" label; the analyze phase line no longer names the criteria count.

## 2026-09-21 — v3.26.1: Review + UI/UX fixes for v3.26.0

- **Review (no P0; 1 P1, 4 P2, 7 P3 — all fixed)** — a claim the pipeline admitted as `[unevidenced]` now keeps an "(unverified)" suffix wherever its markers are stripped (key points, risk rows, 90-day plan, the e-mail) instead of reading as a plain fact; the report e-mail a visitor just asked for is transactional, so a promotions opt-out no longer withholds it and burns the free grant; `FREE_REPORTS_DAILY_CAP=0` refuses to the pay path instead of queueing forever; the intake ceilings key on the trusted client-IP hop; the ledger IP hash rotates daily; erasure also deletes free-report grants by the normalised e-mail hash (0441); the checkout-caller guard covers all seven session-minting routes with the four quote panels allow-listed; the Quick Report option opens a review block before its Pay button; explicit "Pay A$3 …" labels; docs drift corrected.
- **UI/UX check (ui-ux-pro-max on production, 375 / 1280 / dark emulation)** — every Trusted Business Report chapter was clipped at 375 px (grid columns without `min-w-0`) — fixed; dashboard bars and the route / revenue / valuation visuals are legible on phones; 11 px labels raised to 12 px; the "What weighs against" heading no longer red; raw `**` in showcase strengths renders bold; footer `.au` contrast 3.46 → 5.4:1; consent banner, footer links, pricing toggle, `/analyze` back link, checkout back link, tour popover and chapter chips reach 44 px; dossier title and `/admin/validation` phone layouts; plan chips use the sold plan names; Lucide sector icons on `/startup-index`.
- **QA** — live-qa lanes 21 / 37 / 38 / 41 / 43 / mobile re-pinned on the v3 report and the light template; the page sweep visits `/checkout/review` with an order in the query.

## 2026-09-21 — v3.26.0: Light template complete + Trusted Business Report v3 (G26 · G27)

- **Light template on every page** — the evaluator, accelerator, investor and admin surfaces join the light template (300+ `dark:` variants stripped, 50 dense tables with sticky sunken headers and zebra rows, white dialogs and drawers, banners with a navy / bear / warn rule, `/admin/sales` rebuilt), as do the remaining marketing, tools, guides, invite and startup-package pages; the unused `ProShell` is gone. The light-template guard now **enforces** (0 hits, four allow-list entries with reasons) and the page sweep reports a `light` defect when a page's body or first section is not light or its text not dark. Demo-cohort scores now show on the program desk.
- **Trusted Business Report v3 — an investor-grade report** (founder priority). Modelled on IC memos, due-diligence reports, equity research and independent valuation reports: **Dashboard** (SVI, Evidence Confidence, verdict band, valuation range, 8-dimension bars with the p25–p75 band and n) → **Investment view** (a deterministic A / B / C / D rubric — investable now / with conditions / not yet / insufficient evidence — with conviction, three reasons, three risks and the conditions to clear; always "BlockID structures the evidence; evaluators and founders make the decision") → **Key points** → **Valuation** (methods, applicable, consensus row, what moves it) → the **8 chapters with one identical anatomy** (score · band · benchmark with n · verdict · evidence used · strengths · risks · what to improve + lift · investor takeaway · ledger) → **Risk matrix** → **90-day plan** ranked by lift ÷ effort → Money on the table → Appendix + Evidence cited. The same order on web, PDF (A4; 27 pages standard, 10 free) and DOCX; the report-pipeline e-mail carries the investment-view page (the free-grant path still e-mails the first-analysis PDF — v3 routing for it follows); EN + VI; never-say and "benchmark without n" guards run over the rendered text. Runbook `docs/ops/reports.md`.
- **Fixes** — post-deploy smoke pins follow G25 (evaluator rungs land on the review step, the home CTA is "Start a cohort", the retired `/pilot` routes answer 301); the report disclaimer links use the action token (the dark opt-in read 2.94:1).

## 2026-09-21 — v3.25.0: Commerce simplification + light template foundation (G25 · G26-T/M/W1 · G24 fixes)

- **Paid pilot + coupon removed (founder decision)** — the Cohort Validation Pilot SKUs, `/pilot`, `/vi/pilot`, `/pilot/investor`, the pilot delivery kit purchase path and the pilot → annual coupon conversion are gone (301s to the solutions pages / `/workspace/accelerator/onboarding`). Programs go straight to the sold ladder: "Start a cohort" opens the Cohort 25 annual trial. Validation tracker levels 3–5 now read written proposals, the first paying program and renewals from revenue events; the proposal generator writes a Cohort proposal. `pilot_orders` stays a read-only ledger (0437).
- **First two business reports free** — a visitor's first two full reports are free per e-mail address: the e-mail is required before the run (consent line with the data principle, unsubscribe any time), the PDF is e-mailed and the report shown on-page; the third goes to the A$3 quote. Abuse guard (per-IP daily limit, honeypot, disposable domains) and a platform daily cap with a polite queued state. Counts of people who submitted / received on `/api/status` and `/admin/funnel` (0439, erasure map 0440).
- **Review before pay** — no click ever opens Stripe directly. Every plan, trial, credit pack, package and API hand-off lands on `/checkout/review` (EN + VI): name, inclusions, price inc. GST with the GST share, billing interval, trial / renewal / cancellation terms, seller entity and data principle; only the explicit **Pay** / **Add card & start trial** button starts checkout. Card-required sign-up shows the same review above the card field. A guard test allow-lists the checkout callers.
- **Anthropic via the Claude CLI** — `ANTHROPIC_API_KEY` is optional; absent → `not_configured`, the Claude CLI subscription is the fallback tier (free chain order unchanged). Three code paths that required the key (nightly prompt eval, C-level review, deploy gate 1) fixed. Demo-startup names checked against ABN Lookup / ASIC / IP Australia (two replaced); a 33-row sourced funding-announcements CSV feeds `funding_round` proposals; `docs/ops/founder-items.md` lists what still needs a human.
- **Light template (foundation + marketing + founder workspace)** — light is the only default (no OS auto-dark), one navy `#1B2A5E` / cyan `#0e7490` palette on white and soft-grey surfaces with dark ink (body ≥ 4.5:1, headings ≥ 7:1); PageHeader / Card / Table / Field / Button primitives; light nav and footer; marketing pages, `/vi`, showcase, docs, guides, legal, sign-up, search and the whole founder workspace (onboarding, shell, charts, 50 tables) converted; a guard test lists the remaining dark surfaces (evaluator/admin and report surfaces follow in v3.26.0).
- **Fixes** — `/workspace/accelerator` no longer hits the error boundary (React #441: a Server Component passed a function to the client demo button); the cohort page no longer scrolls horizontally; invisible dark-mode text on 15 evaluator files; 44 px targets on sort buttons / footnotes; `text-ink-500` contrast; citation markers stripped on the Assessment Card, dossier and e-mail; claim-gate and critic-filter precision; demo cohort double-POST race; `ai_runs` FK retry scoped to the prompt-version constraint; console guard tolerates two Cloudflare GTM lines per document.

## 2026-09-21 — v3.24.0: Report readability, demo cohort, AI-run integrity (G24)

- **Citations you can read** — `[ev:…]` markers now render as numbered superscript footnotes (stable numbering, one number per evidence row, identical on web, PDF and DOCX) with an "Evidence cited" appendix (row, evidence level, source, date; EN + VI); an unsupported claim shows a small "unverified" chip; unknown ids render nothing. The grounding audit still reads the stored markers.
- **Demo cohort for workflow demos** — an evaluator seat loads five fictional startups (deterministic scores from the demo register, no AI call, zero cost; evidence levels L1–L5, one conflicting claim, one stale connector) so filters, compare, overrides, snapshots, the Cohort Report, demo-day pack and feedback letters all have data. Every card carries "Demo data — fictional"; demo batches are excluded from benchmarks, the Startup Index, calibration, the institutional API, organisation exports and the validation tracker's auto rows (a non-admin seat's demo counts as a Level-2 "workflow demo run"). "Load a demo cohort" sits beside "Import CSV" on every empty cohort state; the pilot kit gains a "Demo run" pre-step. Owner removes it in one click. Migration 0436.
- **AI-run integrity** — the `ai_runs` ledger had never received a row: no prompt version was ever registered, so every insert violated the FK. Prompt versions are now registered on first use and `prompt_version_id` is nullable (0435). A 401 marks that provider unconfigured for the process lifetime with one log line naming the env var (no key material logged). `funding_round` outcome proposals read a curated public-announcement source. The error digest raises one line when report quality is not "ok" for more than 24 h. `docs/ops/ai-runs.md`.
- **Showcase grounding 0.50 → 0.77 on real data** — computed facts (SVI scores, stage quartiles, valuation consensus, AU tax context, SaaS benchmark bands) are citable rows; the critic sees the whole submission (it had seen a 4,000-character slice and called real numbers fabricated); the auto-citer runs over the rendered section; short or typo'd ids are expanded; `--audit-dump` writes the per-section audit so the next gap is diagnosable. The 0.85 KPI is not yet met — the remaining sections are model-invented specifics the critic catches and revises.

## 2026-09-21 — v3.23.0: Report grounding, pilot proposals + conversion, ops hygiene (G23)

- **Report grounding to the KPI** — the showcase run had read `groundedShare` 0.41 against the 0.85 KPI. Every material number in a section is now auto-cited against the evidence register (never an invented id; ≤ 2 ids per sentence; unit-aware), the claim gate is shared between the llm-auditor and the auto-citer, a truncated model answer is salvaged on the first call instead of a repair pass, CMO / CFO / CPO get larger structured-output budgets, W4 verdicts are trimmed to the word cap instead of degrading the chapter, the executive-summary gate resolves ids against the whole register, ReportV2 read-time projections publish their real citation-gate share, and `budget_overruns` / `verdict_trimmed` / `auto_cited` counters land in `tbr-quality.jsonl`. The demo report audits at 1.00; `TBR_GROUNDED_SHARE_KPI` (0.85) is exported. New prompt-eval fixture `TBR-grounding-v2.3.0`.
- **Pilot proposals + conversion** — `/admin/validation` generates a written pilot proposal PDF per tracked organisation (problem in their words, scope + price from the pilot SKUs, deliverables, success metrics, timeline, data & consent, after-the-pilot path, acceptance block); a paid pilot converts to a Cohort annual plan with the pilot credit (`convert_from_pilot` on checkout, founder-minted coupons, conversion recorded on the pilot order — 0434 — and on the validation tracker), with a conversion card on `/workspace/accelerator/pilot`.
- **Ops hygiene** — partial live-qa runs write `live-qa-latest-partial.json` and never overwrite the full summary; weekly crons log to `content/reports/logs/` (pruned at 56 d); validation-ledger writes are serialised and `PATCH` honours `If-Match` (409 on a stale row, the form reloads the server copy); pilot copy reads catalogue keys in EN and VI; `/api/status.tbr_quality` carries `grounded_share` + `grounded_share_kpi` and `/admin/funnel` shows a Report grounding row.
- **Fix** — live-qa lane 42 and the post-deploy smoke tolerate the two Cloudflare-injected GTM inline scripts (blocked by design under the hash-mode CSP) and still fail on a first-party block or a React #4xx.
...
- **Calibration, honestly** — `npm run calibration` associates the SVI band / evidence confidence at T0 with confirmed outcomes ≥ 90 d later, per stage × quarter cohort, only under the n-rules (Wilson 95 % CI from n ≥ 30; "indicative" 10–29; nothing below 10); `/methodology/calibration` shows the honest empty state until the data exists and never an accuracy claim. **Trajectory timeline** (Day 0 / 60 / 180 — SVI, evidence confidence, evidence levels, outcomes) on the workspace score and the dossier. **Data-moat metrics** on `/admin/funnel` and `/api/status`.
- **Benchmark segments** — stage and stage × sector medians computed nightly from one latest score per company (0428), published only at n ≥ 10 (RLS enforces it), sector → stage fallback stated in the label; the Assessment Card, the Cohort Report (external benchmark; the cohort's own median is now labelled "Cohort median") and `/startup-index` read them.
- **Read-only institutional API** — `/api/v1/institutional/{cohorts, cohorts/[id], cohorts/[id]/snapshots, companies/[id], benchmarks, methodology}`: API-key auth on the `api.access` tiers, 600 reads / key / hour, ETag + private cache, an audit row per read, no founder PII, consent tiers respected; OpenAPI + `/developers/api` section + `docs/api/institutional.md`.
- **Organisation admin** — audit export CSV (formula-safe, 90 d default), retention setting (30–3650 d) with a weekly cron that deletes only org-owned artefacts (cohort snapshots, overrides, intake submissions); `docs/ops/retention.md`. SSO remains a planning note.
- **Connectors as evidence** — every sync writes EvidenceRecords (Stripe / Xero L5 transaction data; GitHub / GA4 / ABR L4 connected source; a named reviewer is still required for L6), each connector card states which claims it strengthens; not-offered integrations listed with the claim they would strengthen. **Freshness** (fresh ≤ 30 d · ageing · stale > 90 d) on cards, a stale-connector hint on the Assessment Card, live "stale connectors" trust metric. **Reviewer signature** (name, role, firm, date, methodology version, override count, "Humans make the decision") on dossiers and IC memos. `/methodology/versions` history page with a guard that fails on a version bump without a row.

## 2026-09-20 — v3.20.0: Cohort intelligence (G21 P2 — advisor-feedback upgrade, phase 3 of 4) + P1 review fixes

- **One BlockID Cohort** — `evaluation_batches` is the cohort (program, intake link, template, weights version, applicants cap, paid-pilot order; migration 0422); the legacy accelerator cohort pages redirect to it. **CSV import** (company, url, contact e-mail, stage, sector, deck; validation with line numbers, dedupe by domain / ABN / e-mail, pilot applicants cap, founder invites carrying the program's consent text). **Intake templates** (question builder, rubric weights, consent text) rendered on `/apply/[slug]`. **Cohort snapshots** on batch completion / on demand, "Re-score cohort", per-startup Δ.
- **The BlockID Cohort view** (0423) — columns company · stage · sector · SVI · Evidence Confidence · BlockID Verified · Δ · strongest · weakest · evidence gaps · review status · reviewer · decision (+ conviction) · shortlist; URL-synced filters (stage, sector, SVI / confidence / traction ranges, risk flags, verification, status, decision, shortlist, search); column chooser, saved views, density; compare drawer (≤ 4); **overrides with reason codes** logged beside the canonical SVI — the SVI itself is never altered; **program rubric weights** now rank the cohort (canonical SVI always shown beside the program score); reviewer / viewer roles with e-mail invites; decision log per startup; CSV export with the new columns. "Humans make the decision."
- **Program journey** — `/workspace/accelerator` is the six-stage BlockID Cohort journey (Intake → Assessment → Selection → Program → Demo day → Sponsor reporting) with data-derived stage chips, mentor-gap lists (weakest dimension × missing evidence), re-score, demo-day readiness table.
- **Cohort Report for sponsors** — HTML / PDF / CSV: cohort movement first vs latest snapshot, median improvement per dimension, benchmark only with `n`, evidence completion, outputs, top strengths / gaps, human-review note (override count), reviewer signature, disclaimer. **Demo-day pack** PDF (one Assessment Card page per selected startup). **Pilot delivery kit** (`/workspace/accelerator/pilot`): checklist + success-metric capture (review time, consistency, startups processed, evidence completion, satisfaction, repeat / renewal intent, willingness to pay) + case-study consent. Feedback letters to non-selected applicants with preview → confirm (k ≥ 3 anonymity kept).
- **P1 review fixes** — the Assessment Card's stage benchmark now counts companies (latest analysis per project), not analysis rows — it had published "n = 55" for three projects; one evidence-confidence number per report (card and executive line, web / PDF / DOCX / dossier / workspace read the stored value); every remaining percentile / median / "average" line (comparison card, chapters, PDFs, IC memo, dossier header, summary PDF, `/api/benchmarks` fallback) goes through the publication rules; the corrections data-ethics panel is project-scoped and owner-only; concurrent resolve guarded; claims re-sync after expiry mints a fresh active record; rejected hub rows withdraw their records; every new `app_users` FK is in the erasure map (0425, 0426).

## 2026-09-20 — v3.19.0: Evidence & explainability (G21 P1 — advisor-feedback upgrade, phase 2 of 4)

- **Claim ≠ Evidence** — every startup now carries an explicit claim register (`claims`, `evidence_records` L1–L6 with expiry / hash / visibility / consent scope, `claim_versions`; migration 0417), synced from every analysis and report, backfilled for existing projects; founders correct a claim through a versioned `PATCH` that never overwrites; a daily expiry job flips stale evidence and re-grades the claims (the Phase-3 state machine's `expire` finally has a caller).
- **BlockID Assessment Card** — SVI and **Evidence Confidence** side by side (one function, persisted on snapshots — 0419), BlockID Verified level, stage, sector, benchmark with `n`, top strength / top gap, unverified material claims, last updated — on the Trusted Business Report, `/workspace/score`, the evaluator dossier and IC memo, `/tbr/*`, the BlockID showcase, PDF and DOCX. Every dimension now shows **why · evidence · missing · benchmark · next action** instead of a bare number.
- **Benchmark discipline** — one publication module: n < 10 nothing, 10–29 "indicative", 30+ benchmark, 100+ segmented, `n` always visible; wired through the cohort percentile (the band-based fallback below the floor is gone), the backtest and `/methodology/calibration`, `/startup-index`, `/api/benchmarks` and the dashboard cohort block. "Australian average" wording retired.
- **Founder corrections** — `/workspace/evidence/corrections` (flag incorrect / stale / misunderstood / duplicate / wrong sector-stage / unsupported statement) with a data-ethics panel (what BlockID holds, who has access, what was shared, last refreshed, revoke); `/admin/corrections` accepts or rejects with a logged resolution that never silently overwrites; "Flag a problem with this report" in every report appendix (migration 0418).
- **Evidence checklist** on the founder dashboard and evidence hub (claimed · missing · what raises confidence · one action per dimension); Claimed / Evidence-backed / Verified / Unverified / Conflicting chips on evidence items.
- **Housekeeping** — all showcase pages have a `<main>` landmark and the report tables scroll inside their container at 375 px; `pilot_orders` joins the erasure map as an anonymised financial record (0421); FI analytics `evidence_added`, `evidence_verified`, `score_recalculated`.

## 2026-09-20 — v3.18.0: Trust & positioning (G21 P0 — advisor-feedback upgrade, phase 1 of 4)

- **Positioning** — BlockID is evidence-backed startup assessment infrastructure for accelerators, innovation programs and professional evaluators; founders own the data. Hero "Screen every startup on the same evidence-backed framework." with **Run a cohort pilot** / **Score my startup**; 7-item nav (Product · For Programs · For Investors · For Founders · Methodology · Startup Index · Pricing); home = problem → product sequence → Screen faster · Trust the evidence · Track improvement → Why not ChatGPT → Built for → trust band → one CTA. No prices, no agent counts, no A$3 on the home page; OG image + JSON-LD on the new hero.
- **One legal identity** — `lib/site/legal-entity.ts` is the only place the operator, ACN/ABN, marketing operator and e-mails live; ~100 files, the i18n bundles, the legal MDX documents, PDF covers, e-mail footers and Stripe invoice footers read it; a guard test fails on any new literal. `TrustBand` (operating entity · ACN/ABN · methodology version · privacy/evidence controls · score disclaimer · audit trail · founder consent · data ownership) on home, /product, /pricing, /methodology, every /solutions page and /pilot.
- **BlockID Cohort / Investor / Founder pages** — accelerator page rebuilt around the six-stage program workflow (only shipped bullets), "Humans make the decision", and the **Cohort Validation Pilot** offer; investor page "Standardise the first-pass review before human investment judgement begins."; founder page "See what an evaluator can verify — not only what your pitch says." with the nine-step workflow; /vi mirrors.
- **Paid pilot SKU** — `cohort_pilot_25` A$1,500 / `cohort_pilot_50` A$2,500 inc. GST, one-off Stripe checkout (quote dialog before pay), `pilot_orders` (migration 0416) + 90-day Cohort-tier entitlement + welcome e-mail + workspace banner; until the founder mints `STRIPE_PRICE_COHORT_PILOT_25/50` every buy control lands on `/contact?topic=pilot`. `/pilot` is the paid landing; the comped investor pilot moved to `/pilot/investor` (noindex). Pilot rung first on the Programs pricing tab; the A$3 Trusted Business Report is a pay-as-you-go footnote, not a card.
- **Trust documents** — `docs/product/score-governance.md` rendered at `/methodology/governance` (dimensions, weights policy, versioning, benchmark n rules, conflict handling, human review, corrections, re-scores); methodology version line + human-in-the-loop paragraph on /methodology; `docs/design/public-claims-policy.md` + `content/claims-register.json` (proven / observed / hypothesis, 22 rows) with a guard that fails on any unregistered number in public copy; an unsourced "70 %" statistic and the last public agent counts removed.
- **Pitch deck v4** — 10 slides + appendix (`npm run pitch:v4`), guard test (no agent counts, no A$3, no unregistered numbers, entity from config).
- **Analytics** — FI vocabulary (`website_imported`, `evidence_verified`, `score_recalculated`, `cohort_created`, `startup_added_to_cohort`, `batch_scored`, `pilot_started`, `subscription_renewed` + aliases) with an organisation / startup / plan / channel envelope; `/admin/funnel` institutional section and the North Star "startups assessed through paying institutional workflows per month".

## 2026-09-20 — v3.17.0: Ready for sale (G20) + report quality batches 1–2 (G19)

- **G20 feature inventory** (`docs/ops/feature-inventory.md`): every nav/menu/CTA-reachable feature classified per persona; 19 unfinished features hidden behind a "Not offered yet — talk to us" card (SSO, white-label, public listing submission, digests, portfolio, roster invite, cohort add, LP quarterly, innovator console, mentor tools, unprovisioned Stripe Connect/Xero/QuickBooks connectors, /es /ja forks, admin tokens) and removed from every nav, tour and the sitemap; dead `/score` + `/svi` pages removed (redirects kept); plan bullets ↔ flags ↔ pages reconciled (0414: Program/Fund/VC-ent carry `accelerator.cohort`); a guard test forbids any live link to a hidden route.
- **Signed-in page sweep** (`scripts/page-sweep.mjs`, live-qa lane 33, weekly cron): every page × founder/evaluator/accelerator — status, one h1, console, failed requests, 375 px overflow, alt, main, error boundary. Fixed: streamed `redirect()`s behind `workspace/loading.tsx` (CSP inline-script errors) → real 307s from the founder layout; `/workspace/projects/compare` React #441 (webpackIgnore import); the analyze page rendered the whole marketing landing inside the workspace; the cohort page queried columns that never existed (every Program subscriber saw an empty cohort); missing h1/main on dashboard, onboarding, score, strategy, ESOP, reports; `/dataset` api prefetch 400s; `/version`, `/changelog`, `/legal`, `/developers/api` and five `/tools` overflowed at 375 px; forecast page called a route that never existed; reports client 404 on `default`.
- **Purchase path** (live-qa lane 34, nothing paid): pricing → signup trial terms + card, checkout URL / `interval_unavailable` / `contact_sales`, evaluator Scout trial, credit packs and Startup Package from the catalogue, portal 404 on Free; buy-surface ↔ SKU mapping test; workspace copy on the messaging map (Beta chips, emoji icons, price literals → constants); onboarding trial days from the plan; member invite accept lands on `/dashboard`.
- **G19 (peer session)**: score ledger on every dimension, honest pre-revenue valuation (Berkus + scorecard + stage baseline), evidence & data CTAs, paid view = ReportV2.
- **feat(S46 showcase + telemetry)** `/showcase/blockid/report` — BlockID's own report from the same pipeline (weekly `run-self-analysis.mjs --report`, Mon 04:00 UTC); `tbr-quality.jsonl` per run → `/api/status.tbr_quality` + admin "Pipeline (24 h)"; prompt-eval fixtures for the valuation inputs and a second ledger dimension.
- **fix(S46 — found by running our own report)** Structured criterion calls now get the JSON contract in the system prompt (they carried the legacy markdown format → every first answer was prose, 3 calls per criterion, unvalidated); pipeline calls no longer sit behind the per-user AI fairness cap; W4 chapter and criterion output budgets sized to the contract (900 → 3,200 tokens); `phase_lens.floor` accepts null; migration 0415 — `assembled_reports.analysis_id` uuid → text (every assembled report insert had failed since slug ids; the table held 0 rows).

## 2026-09-19 — v3.16.0: Reliability, first dollar, unicorn homepage, truth sweep (G15 / G16 / G17 / G18)

Version alignment: `web/package.json` (written by the CEO implementing-plan loop) had reached v3.15.0 while `content/reports/version.json` — what `/api/status`, the footer badge, `/version` and `/roadmap` read — still said v3.13.2. Both now say **v3.16.0**; the sections below are the releases between the two (see `docs/plans/SOURCE-OF-TRUTH.md` §1 and Appendix A for the deploy SHAs).

### 2026-09-19 — G18 truth sweep (docs, version, API reference)
- **docs(release)** `version.json`, `package.json`, `project-state.json` and this file aligned at v3.16.0; `/version`, the footer badge and `/roadmap` now show the real release.
- **docs(readme)** `README.md` and `web/README.md` rewritten for the shipped stack: Next.js 16 standalone on bare metal, self-hosted Supabase, single Stripe account (GST-inclusive), the 12-gate `deploy-live.sh`, `npm run qa:live`, `scripts/link-check.mjs`, the restore drill and the three cron families. The Docker Compose / Caddy / GitLab CI instructions, "Growth A$99 / Pro A$299", "Founding 100" and the "50+ agents" claims are gone.
- **docs(index)** `docs/README.md` lists every document with a one-line purpose and a last-verified date; Docker/GitLab-era deployment guides, the removed goal loops, the Goal 5A–5D tracking docs and the v2/v3 implementation plans are retired into `docs/archive/` with a header note, never deleted.
- **docs(public /docs)** Pricing ladder reads Free → A$3 Trusted Business Report → Starter A$29 → Growth A$69 for founders and Scout A$79 / Firm A$149 / Program A$349 · Fund A$999 / Intake link A$249 / Index API A$299 · Cohort 25 / 100 for evaluators; "13 criteria" is described as the internal rubric behind the 8 public dimensions; the Founding 100 footnote, the A$99 reversion note and the 2026-07 plan deltas are replaced by the G11–G17 shipped list, the Evaluator API v1 + webhook destinations, the pilot programme and the link check.
- **docs(api)** `api-docs-registry.ts` / `GET /api/openapi.json` / `/developers/api` gain `POST /api/v1/analyze` (Bearer `bk_live_…`, `analyze` scope, credit-metered) and `GET /api/v1/id/{slug}` (public verified profile JSON, optional partner bearer); `docs/API-REFERENCE.md` gains §10 for the partner, session and public endpoints that had no prose (`/api/v1/svi`, `/api/v1/id/{slug}/vc`, `/api/v1/vc/{jti}/revoke`, `/api/webhooks*`, `/api/reports/access`, `/api/analytics/event`, `/api/pilot/apply`, `/api/index/*`) and its plan/credit appendices are current (the "Founding 50 A$49 / Founder A$99 / Growth A$499" table is gone).
- **docs(roadmap page)** `/roadmap` reflects G11–G17 closed, the founder-only queue, and the "This quarter" list no longer keys off a `v2.0.0-beta` regex that showed every phase as planned on a v3 release.

### 2026-09-19 — G17 unicorn homepage & site template (`fc155500c`, `55c71d258`, `97a2ad910`, `5a0c3dbb1`)
- **feat(home)** Evaluator-first hero ("Startup Value Index … by BlockID"), 5-entry nav (Product · Samples · Solutions · Pricing · Docs), colour-ring search box kept, no price tables on the home page; the home went from 448 to 187 lines / 6 sections; new `/product` and `/samples` intro pages.
- **feat(template)** One design contract for every marketing page and the `/vi` mirrors (`docs/design/unicorn-template.md`, template primitives `PageHero` / `Section` / `CtaBand` / `Prose`); pricing ladder untouched; partner logos no longer paint black on the dark footer; screenshots at 1440 and 375 px in `docs/design/screenshots/`.
- **feat(quality)** `scripts/link-check.mjs` — production 9 → 0 broken internal links, then wired into deploy gate 8 against the temp build (`DEPLOY_LINK_CHECK=0` skips) and a daily 03:40 UTC cron; private / loopback / metadata hosts are never probed from user-supplied hrefs; redirect chains fail the run. Final production run: 525 pages · 1,638 links · 0 broken · 0 chains. Homepage smoke in gate 12; live-qa `31-marketing` lane; elevated live-qa 191/0.
- **fix(review)** `/vi` business-report login redirect, duplicate ids on pricing / methodology, ISR-safe home stats keyed on file mtimes, `/solutions` in the sitemap, 44 px mobile nav targets, VI hero wording, migration 0412 (ACS Digital Pulse URL).

### 2026-09-19 — G16 first dollar (`388f73a82` / `1f0773d96` / `383e9740b` → deploy `8cbb6bdb8`, review `58406efa2`)
- **fix(paywall — P0 finding)** The A$3 Trusted Business Report checkout was mounted by no page and every free founder's web report had been lifted to `standard` — there was no reachable first purchase. Now: locked-chapter preview with one unlock rail → quote-then-pay, `GET /api/reports/access` (read-only quote, price and balance), feature-gate cards that show plan + price + one CTA, `feature_gate_hit.surface` populated, a +48 h "unlock for A$3" nudge (migration 0411).
- **feat(funnel truth)** `sign_up`, `svi_analyze`, `svi_score_computed`, `report_view`, `checkout` and `trust_report_purchased` are emitted server-side with `qa` flags (the 14-day dry-run before this was all zeros); `/admin/funnel` reads real data; `traction.funnel_7d_v2`; `scripts/funnel-report.mjs` daily 02:50 UTC + Monday 03:05 weekly summary.
- **feat(pilot)** One-click evaluator pilot comp — Program plan for 30 days, cap 5, never a Stripe payer, audited `pilot.started` / `pilot.ended`, `pilot-expiry` cron 04:20 UTC; public `/pilot` page + apply form (`POST /api/pilot/apply`, honeypot + per-IP limit) and `/admin/pilots`. A throw-away pilot was started, converted and ended on production before close.
- **fix(review)** Deck-analyser snapshots no longer bypass the paywall as `standard`; ingest limiter keyed on the server session; nudge and D1 drip cannot land in the same hour; stale-chunk error boundaries cover the app segments; `content/pilots.json` ignored by the dirty-tree gate. Elevated live-qa 184/0.

### 2026-09-18 — G15 reliability (`3ce353da8`, review `7e86fb700`, follow-ups `bac472af4`)
- **feat(ship safety)** Deploy manifest written at build start with the tree's real SHA, dirty-tree refusal, gate 11 verifies the *live* bundle SHA (first time the pipeline could tell a stale peer build from master); `--wait` lock etiquette shared by deploy and live-qa; rollback `--dry-run`; `docs/ops/deploy.md`. Evidence that drove it: 25 failed deploys in 14 days, a mid-merge build, a 120 s AI ladder answering as a Cloudflare 524.
- **feat(observability)** `/data/logs` append-mode production log with rotation (the previous `/tmp` log was lost on restart), `error-digest` and `latency-sample` every 10 min, `/api/status` v2 (`git_sha`, `errors_1h`, `ai`, `queues`, `backups_detail`, `slo.latency_p95_ms`, `crons_failed_24h`) with matching `/status` sections, nginx `blockid_timing` log format, `docs/ops/slo.md`.
- **feat(data safety + AI resilience)** Weekly restore drill (`scripts/db/restore-drill.sh`, Sun 03:45 UTC; first run 12/12 tables ≥ 99.8 %, audit-chain head ok, 15.6 s) replaces the legacy restore test; off-site backup failure alert once a day with the founder command; `getProviderHealthSnapshot()` + a `fully_degraded` counter in the report pipeline; `server-cleanup --procs` stray-session sweep; cron-runner `--timeout` ceiling 600 s.
- **fix(review)** Log fold-in had unlinked the inode the live server was writing to (fixed, lost lines recovered); status readers read the live checkout, not the release-dir copy; restore-drill allow-list no longer treats a missing relation as benign; the watchdog was found to be killing a healthy server under build load and was fixed. Telegram bot token is 401 → alerts fall back to e-mail until the founder mints a new token. Elevated live-qa 177/0 → 178/0.

### 2026-09-16 — G13 investor clarity (15 sprints in one day, W1 03:58 → W5 21:20 UTC; migrations 0390–0403)
- **feat(nav v4 + personas)** Founder groups Home · Prove · Money · Company, evaluator Home · Deal flow · Reports; 10 personas with their own post-login landing; every old route redirects (`legacy-redirects.ts`); founder phase-0 sidebar ≤ 10 links; Account moves to the avatar menu.
- **feat(Trusted Business Report v2)** Report follows the 8 dimensions with a named C-Level owner per dimension, 17 deterministic SVG visuals, chapters on `/workspace/business-report`, share view `/tbr/[token]` in four locales and `/tbr/demo`; "Trust BizReport" renamed "Trusted Business Report"; comparables count is the live number, not "500+".
- **feat(investor dossier + taxonomy)** `startup_taxonomy` (22 industries, 10 business models, 8 canonical stages) auto-suggested on every create / analyse; investor mandate v2 with fit scores refreshed nightly; Investor Dossier keyed on the evaluation, evaluator assessments with versions; three new live-qa lanes.

## 2026-09-10 → 2026-09-17 — v3.11.0 → v3.15.0: Money Finder + evaluator ladder (G11 / G12), release readiness (S24–S32), G14 investor feedback
...
- **security(auth — enumeration, P2-a)** `POST /api/auth/login-password` returns one identical 401 for an unknown email, a wrong password and a password-less (Google / magic-link) account; the reason stays in the server log. `POST /api/auth/register` no longer answers 409 "already exists": the caller gets the same generic "check your email" a pending signup gets, and the mailbox owner receives a "you already have an account" email with sign-in and reset links.
- **security(auth — password reset, P2-d)** Requesting a reset no longer overwrites the password (an anonymous caller could lock a founder out by submitting their email). It now mints a single-use 30-minute token (stored hashed in the new `password_reset_tokens` table, migration 0343), emails a link to `/auth/reset`, and rotates the hash only when `POST /api/auth/reset-password/confirm` consumes the token — which also signs the user out everywhere.
- **fix(api — 400 not 500, P2-b)** Empty or malformed bodies on `auth/reset-password`, `index/waitlist`, `unsubscribe/feedback`, `website-tech-audit` and `upload` return 400 through the shared `readJsonBody` guard instead of a logged 500; `upload` checks the session before parsing the multipart body, so anonymous callers get 401.
- **fix(api — dead owner checks, P2-e)** `DELETE /api/evidence/dim/[id]` (`projects.owner_id`) and the three `financial/forecast/[modelId]` readers (`projects.created_by`) queried columns that do not exist and refused every caller, owner included. All four now use `assertProjectAccess` (owner or accepted member; non-member 404).
- **security(status, P2-f)** The anonymous `/api/status` payload is `ok`, `version`, per-service up/down, 24h uptime and the last deploy's time and gate count. The audit-chain, OAuth-token-sealing and GA4-event verdicts, the cron catalogue and the git sha are trusted-caller only.

### 2026-09-11 — post-launch hardening & conversion (S6–S15)
- Money & access review fixes: atomic credit spend RPC, spend-before-run on paid reports and drafts, Startup Package buyers now receive Growth extras + Radar sweeps, batch-scoring leases, idempotent report retries.
- Security: SSRF guard on catalogue fetches, safe redirects, constant-time secret handling on all 80 cron routes and internal endpoints, edge CSRF gate for cookie-authenticated API mutations.
- Quality: ESLint error baseline 271 → 0; accessibility fixes (keyboard reorder, modal focus traps, tabs, contrast); SEO sweep of 163 public pages (titles, OG images, JSON-LD, sitemap coverage of 252 funding pages); Gate-12 smoke extended.
- Product: server-synced dashboard layout; `/docs/unlocks` generated matrix; consistent workspace sidebar phase gating; `/funding/report/demo` sample; evaluator trial = 1 included Trust BizReport + activation checklist; investor "Let matching founders see me" opt-in; enriched program/grant detail pages; lean grouped directories; Founder Radar activation nudges.
- Compliance: privacy policy v2.2 (Money Finder intake, evaluator-entered startup data, discoverability, retention) with a weekly retention sweep mirroring the policy table.

- **feat(evaluations — trial allowance, S7-C)** A card-required evaluator trial now includes exactly one Trust BizReport for the whole trial instead of the plan's monthly 10/30/100: `getReportQuota` reads `subscription_trial_state.status` (mirrored from Stripe by the webhook) and, while `trialing` with `trial_end` ahead, caps the included quota at `TRIAL_REPORT_ALLOWANCE = 1` counted since `trial_start`; the second report is charged 3 credits, never blocked, and the plan quota takes over automatically once the subscription is `active`. `POST /api/evaluations/batch` returns 402 `trial_limit` with the Program upgrade hint when a batch exceeds that one report. `/workspace/evaluations` gains a trial strip — days left, `used/1`, the conversion date and price, and a "Manage billing" button into the Stripe Billing Portal — and the report dialog says "Included in your trial" / "charged to credits" from the new `cost.trial` field. The T-3d trial reminder email adds one line with the unused included report when there is one. Before this a trialing Scout could run ten full 13-agent reports for A$0 on a cancellable card.
