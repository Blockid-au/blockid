-- 0464_erase_account_email_preferences_notifications.sql
--
-- G34 DC09 (2026-09-25): two address-keyed tables the user_id FK walk
-- cannot reach join the non-FK extras (deleted by the pre-erasure e-mail):
--   • email_preferences.email — a row written before sign-up or from an
--     unsubscribe link carries user_id NULL, so the FK entry misses it;
--   • svi_notifications.email — the sent-email / nurture log the daily cap
--     and dedupe read (the RPC skips it where the column is absent).
-- Body = 0442 byte-for-byte except this header and the generated
-- non-fk-extras VALUES block (32 extras), rendered from
-- web/src/lib/privacy/erasure-map.ts and pinned by erasure-map.test.ts.
-- NOT APPLIED — pending-authority/README.md; until applied, live erasure
-- runs the 0442 function (these two tables are not yet covered live).
