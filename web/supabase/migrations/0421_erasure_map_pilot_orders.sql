-- 0421_erasure_map_pilot_orders.sql
-- ---------------------------------------------------------------------------
-- G21 P1 merge (2026-09-20): erasure map + erase_account() gain pilot_orders.
-- ---------------------------------------------------------------------------
BEGIN;

-- ─── erase_account() re-emitted from the 139-entry erasure map ─────────────
-- G21 P0-C's pilot_orders.user_id (0416, CASCADE) was never mapped: paid
-- pilot orders are a financial record (7 y) and are anonymised (buyer e-mail
-- pseudonymised), not deleted. Body = 0417 §5 byte-for-byte except the
-- generated VALUES blocks (rendered from web/src/lib/privacy/erasure-map.ts,
-- pinned by erasure-map.test.ts whose ERASURE_MIGRATION_FILE now points here).
--
