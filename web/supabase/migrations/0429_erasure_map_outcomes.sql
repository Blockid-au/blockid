-- 0429_erasure_map_outcomes.sql
-- ---------------------------------------------------------------------------
-- G21 P3-A (2026-09-20): erasure map + erase_account() gain the two
-- 0427 startup_outcomes FKs (recorded_by / confirmed_by — both detach).
-- ---------------------------------------------------------------------------
BEGIN;

-- ─── erase_account() re-emitted from the 149-entry erasure map ─────────────
-- G21 P3-A (2026-09-20): 0427 added startup_outcomes.recorded_by and
-- startup_outcomes.confirmed_by (both detach — the outcome observation stays
-- with the project's longitudinal record; the actor pointers are cleared).
-- Body = 0426 byte-for-byte except the generated VALUES blocks
