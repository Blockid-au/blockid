-- 0351_erase_account_connector_snapshots.sql — erase_account() re-created for the 125th FK (S25-A, 2026-09-13)
-- ---------------------------------------------------------------------------
-- Migration 0349 added connector_snapshots(user_id → app_users) — the 125th
-- foreign key on app_users. erase_account() walks a generated VALUES map, so
-- the function body is re-emitted here from web/src/lib/privacy/erasure-map.ts
-- (mode: delete, order 30). Everything else is byte-identical to 0348; the
-- ALTER TABLE … ADD COLUMN IF NOT EXISTS statements are idempotent. The
-- header below is 0348's, kept for the reader.
--
-- (0348) -- ---------------------------------------------------------------------------
-- `DELETE FROM app_users` is impossible in production: 125 foreign keys point
