-- 0426_erasure_map_cohort_members.sql
-- ---------------------------------------------------------------------------
-- G21 P2-B merge (2026-09-20): erasure map + erase_account() gain the four
-- 0423 cohort overrides / members FKs.
-- ---------------------------------------------------------------------------
BEGIN;

-- ─── erase_account() re-emitted from the 147-entry erasure map ─────────────
-- G21 P2-B merge (2026-09-20): 0423 added assessment_overrides.reviewer_id,
-- evaluation_batch_items.reviewer_id, evaluation_batch_members.invited_by
-- (detach) and evaluation_batch_members.user_id (delete — the erased
-- account's own seats). Body = 0425 byte-for-byte except the generated VALUES blocks
