-- 0368_erase_account_secondary_sim_orders.sql — erase_account() re-created for the 127th FK (S27-B, 2026-09-13)
-- ---------------------------------------------------------------------------
-- Migration 0366 added secondary_sim_orders(user_id → app_users, ON DELETE
-- CASCADE) — the 127th foreign key on app_users. erase_account() walks a
-- generated VALUES map, so the function body is re-emitted here from
-- web/src/lib/privacy/erasure-map.ts (mode: delete, order 30 — the sandbox
-- orders are the user's own data; the trades tape keeps its labels because
-- secondary_sim_trades references orders with ON DELETE SET NULL, and the
-- labels are register names / free-text sandbox labels, not account PII).
-- Everything else is byte-identical to 0356 / 0351 / 0348; the ALTER TABLE …
-- ADD COLUMN IF NOT EXISTS statements are idempotent. Apply AFTER 0366. The
-- header below is 0348's, kept for the reader.
--
-- (0348) -- ---------------------------------------------------------------------------
-- `DELETE FROM app_users` is impossible in production: 127 foreign keys point
-- at app_users(id) — 14 NO ACTION, 6 RESTRICT (two of them append-only
-- ledgers that reject UPDATE and DELETE), 24 SET NULL, 83 CASCADE — and the
