-- 0406_founder_feedback_letters.sql
-- ---------------------------------------------------------------------------
-- G14-S34 — Founder feedback letter "What investors said"
-- (docs/plans/g14-investor-feedback-2026-09-16.md D3 / F-5; approved plan
-- §5 row S34). Extends G13 S-D2 (evaluation_assessments, 0392).
--
--   public.founder_feedback_letters — ONE anonymised letter per
--   (project, window_end): the k-aggregate of the submitted, shared,
--   not-opted-out assessments on the project once k >= 3 assessors from
--   >= 2 distinct organisations have recorded a verdict (a NULL org counts
--   as its own organisation). `aggregate` holds per-dimension mean ratings
--   and agree shares rounded to 25 % steps, risk buckets (counts + the risk
--   TITLE only), deduplicated questions — never an evaluator id, org,
--   decision, conviction, thesis fit, valuation view, private or shared
--   notes (web/src/lib/evaluations/feedback-letter.ts walks the JSON against
--   FOUNDER_FORBIDDEN_FIELDS before a row is written). `letter_md` /
--   `letter_md_vi` are the rendered EN / VI markdown; `next_actions` the
--   three CTO next-best-actions for the weakest evaluator-rated dimension.
--
--   evaluation_assessments.feedback_letter_id — which letter consumed the
--   row (SET NULL when the letter is erased with the founder).
--   evaluation_assessments.feedback_opt_out — the evaluator's per-assessment
--   opt-out ("Exclude my ratings from the founder's anonymised feedback
--   letter", POST /api/evaluations/[id]/assessment/opt-out-feedback).
--
-- House rules
--   * founder_user_id FKs public.app_users(id) ON DELETE CASCADE (never
--     auth.users) — 134th entry in the erasure map, mode delete; the map
--     and erase_account() are re-emitted in §4 below.
--   * RLS: the founder may SELECT own rows; service role all. Writes go
--     through the service-role client from the cron only.
--   * Reader (feedback-letter-store.ts) is 42P01-guarded: until this file
--     is applied the landing block is absent and the cron answers
--     `{ ok:true, reason:"table_missing" }`.
--
-- Idempotent. Apply by hand (never on deploy):
--   scripts/db/apply-migration.sh web/supabase/migrations/0406_founder_feedback_letters.sql
--
-- Rollback
--   alter table public.evaluation_assessments drop column if exists feedback_letter_id;
--   alter table public.evaluation_assessments drop column if exists feedback_opt_out;
--   drop table if exists public.founder_feedback_letters;
--   (then re-apply 0405 §6 for the 133-entry erase_account()).
-- ---------------------------------------------------------------------------

BEGIN;

-- ─── 1. founder_feedback_letters ─────────────────────────────────────────────
create table if not exists public.founder_feedback_letters (
  id                uuid primary key default gen_random_uuid(),
  project_id        uuid not null references public.projects(id) on delete cascade,
  founder_user_id   uuid not null references public.app_users(id) on delete cascade,
  -- the evaluations whose current submitted assessment fed this letter
  evaluation_ids    uuid[] not null default '{}',
  -- k-anonymity floor (D3 / F-5): >= 3 assessors from >= 2 organisations
  k                 smallint not null check (k >= 3),
  org_count         smallint not null check (org_count >= 2),
  window_start      timestamptz,
  window_end        timestamptz not null,
  -- {dimensions:[{key,mean,n,agreePct}], risks:[{dimension,count,titles}], questions:[{text,dimension}], weakestDim, k, orgCount}
  aggregate         jsonb not null check (jsonb_typeof(aggregate) = 'object'),
  letter_md         text not null,
  letter_md_vi      text,
  -- [{id,title,rationale,dimension,sviBenefit,effort,timeToComplete,href}]
  next_actions      jsonb not null default '[]'::jsonb check (jsonb_typeof(next_actions) = 'array'),
  status            text not null default 'draft' check (status in ('draft','sent','opened')),
  sent_at           timestamptz,
  opened_at         timestamptz,
  email_message_id  text,
  created_at        timestamptz not null default now(),
  unique (project_id, window_end)
);

comment on table public.founder_feedback_letters is
  'G14 S34: anonymised "What investors said" letter per (project, window_end) — k >= 3 assessors from >= 2 orgs; ratings / risk buckets / questions only, never evaluator id, org, decision, conviction or notes.';
comment on column public.founder_feedback_letters.aggregate is
  'Per-dimension mean rating + agree share (25 % steps), risk buckets (dimension, count, normalised titles), deduped questions. Walked against FOUNDER_FORBIDDEN_FIELDS before insert.';
comment on column public.founder_feedback_letters.evaluation_ids is
  'evaluations.id of every assessment that fed the aggregate — for the founder-side "based on N assessments" line and the cron''s nothing-new check.';

create index if not exists founder_feedback_letters_founder_idx
  on public.founder_feedback_letters (founder_user_id, created_at desc);
create index if not exists founder_feedback_letters_project_idx
  on public.founder_feedback_letters (project_id, window_end desc);

alter table public.founder_feedback_letters enable row level security;

drop policy if exists founder_feedback_letters_founder_select on public.founder_feedback_letters;
create policy founder_feedback_letters_founder_select on public.founder_feedback_letters
  for select using (founder_user_id = auth.uid());

drop policy if exists founder_feedback_letters_service_all on public.founder_feedback_letters;
create policy founder_feedback_letters_service_all on public.founder_feedback_letters
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

-- ─── 2. evaluation_assessments: letter pointer + evaluator opt-out ───────────
alter table public.evaluation_assessments
  add column if not exists feedback_letter_id uuid null references public.founder_feedback_letters(id) on delete set null;
alter table public.evaluation_assessments
  add column if not exists feedback_opt_out boolean not null default false;

comment on column public.evaluation_assessments.feedback_letter_id is
  'G14 S34: the founder_feedback_letters row that consumed this assessment (NULL = not yet lettered). SET NULL when the letter goes.';
comment on column public.evaluation_assessments.feedback_opt_out is
  'G14 S34: the assessor excluded this assessment from the founder''s anonymised feedback letter (POST …/assessment/opt-out-feedback).';

create index if not exists evaluation_assessments_feedback_pending_idx
  on public.evaluation_assessments (project_id)
  where status = 'submitted' and feedback_letter_id is null and feedback_opt_out = false;

-- ─── 3. schema note ──────────────────────────────────────────────────────────
-- The founder view v_assessment_founder_view (0392 §3) is unchanged: the
-- letter reads through the service-role client and masks in code.

-- ─── 4. erase_account() re-emitted from the 134-entry erasure map ───────────
-- One new app_users FK (founder_feedback_letters.founder_user_id — CASCADE,
-- mode delete, order 30). The function body below is 0405 §6's
