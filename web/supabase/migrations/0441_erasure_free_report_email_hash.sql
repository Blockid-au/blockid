-- 0441_erasure_free_report_email_hash.sql
--
-- Review v3.26.0 P3 (2026-09-21): free_report_grants rows are keyed by the
-- NORMALISED address hash (plus tags stripped, gmail dots removed) while the
-- typed address is stored as-is — an erasure by `lower(email)` left rows
-- behind for a.b+x@gmail.com when the account address is ab@gmail.com.
-- This migration (1) adds `public.free_report_email_hash(text)` mirroring
-- lib/reports/free-grants-rules.ts normaliseReportEmail() + free-grants.ts
-- emailHash(), (2) re-emits erase_account() from the erasure map with the new
-- `email_hash` keyed mode so the non-FK extras loop deletes by the hash too.
-- Body = 0440 except the generated VALUES blocks and the new CASE branch.
