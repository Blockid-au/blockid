-- 0385_erase_account_cap_table_fundraise.sql — erase_account() re-created: 129th FK (fundraise_rounds, 0384) + cap-table / dividend register extras keyed 'account' (live QA 2026-09-13, lane 2 P1-2)
-- ---------------------------------------------------------------------------
-- Live erasure QA left shareholders (third-party name + email), share_classes,
-- share_transactions and dividend_records behind: they are keyed on
-- account_id with NO foreign key and the column holds either the app_users id
-- or a legacy svi_accounts id. The non-FK extras loop gains keyed = 'account'
-- (matches both), the four tables are listed in dependency order (NO ACTION
-- FKs from share_transactions), and fundraise_rounds joins the FK map after
-- 0384 repointed its constraint. Otherwise identical to 0371. Idempotent.
--
-- (0371) -- ---------------------------------------------------------------------------
-- S27-B (0368: secondary_sim_orders.user_id, delete) and S27-C (0370:
-- sector_multiples_overrides.approved_by, detach) were built on parallel
-- branches; each re-emitted erase_account() with its own 127-entry map, so
-- whichever applied last dropped the other's row. This file is 0370 with the
-- VALUES blocks re-rendered from the merged web/src/lib/privacy/erasure-map.ts
-- (128 entries, pinned by erasure-map.test.ts). Idempotent.
--
-- (0370) -- ---------------------------------------------------------------------------
-- Migration 0369 added sector_multiples_overrides(approved_by → app_users,
-- ON DELETE SET NULL) — the 127th foreign key on app_users. erase_account()
-- walks a generated VALUES map, so the function body is re-emitted here from
-- web/src/lib/privacy/erasure-map.ts (mode: detach, order 15 — an approved
-- sector-multiple override is a platform-wide valuation input, not the
-- admin's data; only the approver pointer is cleared and the audit_events
-- row keeps the actor). Everything else is byte-identical to 0356 / 0351 /
-- 0348; the ALTER TABLE … ADD COLUMN IF NOT EXISTS statements are
-- idempotent. Apply AFTER 0369. The header below is 0348's, kept for the
-- reader.
--
-- (0348) -- ---------------------------------------------------------------------------
...
--
-- Apply by hand: scripts/db/apply-migration.sh web/supabase/migrations/0370_erase_account_sector_multiples_overrides.sql
...
-- END non-fk-extras
  ...
  LOOP
    ...
    v_where := CASE WHEN v_step.keyed = 'email'
                    ...
                    THEN format('(%I = %L OR %I IN (SELECT id FROM public.svi_accounts WHERE lower(email) = %L OR project_id IN (SELECT id FROM public.projects WHERE user_id = %L)))',
                                v_step.col, p_user_id, v_step.col, lower(v_email), p_user_id)
