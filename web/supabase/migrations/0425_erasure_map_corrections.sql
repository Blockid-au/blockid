-- 0425_erasure_map_corrections.sql
-- ---------------------------------------------------------------------------
-- G21 P1 post-ship review (2026-09-20): erasure map + erase_account() gain
-- corrections.submitted_by / corrections.resolved_by.
-- ---------------------------------------------------------------------------
BEGIN;

-- ─── erase_account() re-emitted from the 141-entry erasure map ─────────────
-- G21 P1-C's corrections.submitted_by / corrections.resolved_by (0418, both
-- SET NULL, applied in prod) were never mapped. Both are actor pointers on a
-- row that belongs to the project → detach (order 15, pointer nulled). The
-- correction `message` is free text the founder wrote about their own
-- startup — not third-party PII — so it is kept, exactly like the other
-- detached rows' content. Body = 0421 §5 byte-for-byte except the generated
-- VALUES blocks (rendered from web/src/lib/privacy/erasure-map.ts, pinned by
