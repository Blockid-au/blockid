...
-- ---------------------------------------------------------------------------
-- G21 P1/P2 merge (2026-09-20): erasure map + erase_account() gain
-- intake_templates, cohort_snapshots (0422) and corrections (0418).
-- ---------------------------------------------------------------------------
BEGIN;

-- ─── erase_account() re-emitted from the 143-entry erasure map ─────────────
-- G21 P1/P2 merge (2026-09-20): the union of 0422 (intake_templates.owner_user_id
-- delete, cohort_snapshots.created_by detach) and the P1 post-ship review fix
-- (corrections.submitted_by / resolved_by detach — 0418 had shipped without a
-- map entry). Body = 0421 §5 byte-for-byte except the generated VALUES blocks
