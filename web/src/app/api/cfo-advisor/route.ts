...
/** Adapter: ADK ModelCaller (system, user, maxTokens) → DeepInfra customer policy. */
const adkModel = async (system: string, user: string, maxTokens: number): Promise<string> =>
  (await callAI({ providerPolicy: "deepinfra-only", agentId: "cfo-advisor", system, user, maxTokens, timeoutMs: 90_000, interactive: true })).text;
