// LEGACY GENERATOR — kept for ONE release behind `REPORT_GENERATOR=legacy_stream`
// (S-R3, spec §F S-R3 rollback). Not a Next.js route file (route.legacy.ts is
// ignored by the App Router); ./route.ts delegates here only when the env flag
// is set. Delete once the S-R4 smoke passes on the unified pipeline.
...
export const legacyStreamPOST = POST_handler;
