...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (connErr) {
      console.error("[oauth:ga4] oauth_connections upsert failed", { code: connErr.code, message: connErr.message });
      return NextResponse.redirect(`${siteUrl}/workspace/evidence?error=ga4_save_failed`);
    }
