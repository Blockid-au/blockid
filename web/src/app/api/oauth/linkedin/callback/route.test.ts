// Colocated tests for GET /api/oauth/linkedin/callback — S18-A review P1-2.
//
...
function run(email?: string) {
  return GET(new Request(`http://x/api/oauth/linkedin/callback?code=one-shot&state=${stateParam(email)}`));
}
...
beforeEach(() => {
  ...
  fetchSpy.mockReset().mockImplementation(async (url: string) => {
    const u = String(url);
    if (u.includes("linkedin.com/oauth/v2/accessToken")) return { ok: true, json: async () => ({ access_token: "li-token" }) };
    if (u.includes("api.linkedin.com/v2/userinfo")) {
      return { ok: true, json: async () => ({ sub: "abc123", name: "Ava Founder", email_verified: true }), text: async () => "" };
    }
...
describe("GET /api/oauth/linkedin/callback — session binding (S18-A P1-2)", () => {
  it("state email ≠ session email: redirected with error=linkedin_email_mismatch; no exchange, no writes", async () => {
    const res = await run("victim@x.test");
    expect(res.status).toBeGreaterThanOrEqual(300);
    expect(res.status).toBeLessThan(400);
    const loc = location(res);
    expect(loc.pathname).toBe("/workspace/evidence");
    expect(loc.searchParams.get("error")).toBe("linkedin_email_mismatch");
  ...
  it("unauthenticated: redirected with error=linkedin_unauthenticated; no exchange, no writes", async () => {
    auth.user = null;
    const res = await run();
    expect(location(res).searchParams.get("error")).toBe("linkedin_unauthenticated");
    expect(fetchSpy).not.toHaveBeenCalled();
    expect(keyCalls(scopeState, "findOrCreateSVIAccount")).toEqual([]);
    expect(db.sb!.calls).toEqual([]);
  });
  ...
  it("CSRF mismatch still wins first: redirected before the session check", async () => {
    ...
    auth.user = null;
    const res = await run();
    expect(location(res).searchParams.get("error")).toBe("linkedin_csrf_mismatch");
    expect(fetchSpy).not.toHaveBeenCalled();
  });

  it("happy path (owner): exchange runs, connection + evidence written on the owner's own record", async () => {
    const res = await run();
    expect(location(res).searchParams.get("connected")).toBe("linkedin");
  ...
  it("editor: redirected with error=linkedin_forbidden_role after the session check, before the exchange", async () => {
    scopeState.role = "editor";
    const res = await run();
    expect(location(res).searchParams.get("error")).toBe("linkedin_forbidden_role");
    expect(fetchSpy).not.toHaveBeenCalled();
    expect(db.sb!.calls).toEqual([]);
  });
