    ...
    return NextResponse.json(
      {
        ok: true,
        message:
          "The Founding 100 promo ended on 2026-08-31. See /pricing for the current plans.",
        promo_ended: true,
      },
    );
  }

  // G18-A (2026-09-19): the Stripe Checkout + payment-link e-mail this route
  // used to mint for `source === "founding50"` is gone. The promo guard above
  // has answered every such request since the 2026-09-01 cutover, so the
  // block could never run again — and it was the last consumer of
  // `STRIPE_PRICE_FOUNDING50`, which is now safe to archive in Stripe.
