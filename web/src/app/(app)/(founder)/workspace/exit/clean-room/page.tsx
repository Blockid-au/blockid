...
export default async function CleanRoomPage() {
  ...
  await requireTierForPage({ minTier: "growth", fromPath: "/workspace/exit/clean-room" });

  const user = await getCurrentUser();
  if (!user) redirect("/auth/login?next=/workspace/exit/clean-room");
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-ink-700 mt-1 max-w-2xl">
            ...
            <Link href="/workspace/documents/data-room" className="text-brand-600 underline">
              data room
            </Link>{" "}
            ...
            and{" "}
            <Link href="/workspace/exit/listing" className="text-brand-600 underline">
              Listing readiness
            </Link>
            .
          </p>
