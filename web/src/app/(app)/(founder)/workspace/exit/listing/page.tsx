...
export default async function ListingReadinessPage() {
  ...
  await requireTierForPage({ minTier: "growth", fromPath: "/workspace/exit/listing" });

  const user = await getCurrentUser();
  if (!user) redirect("/auth/login?next=/workspace/exit/listing");
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-ink-700 mt-1 max-w-2xl">
            ...
            <Link href="/workspace/exit/clean-room" className="text-brand-600 underline">
