...
describe("EvaluatorActivationChecklist", () => {
  it("fresh trial: 4 steps, 0 of 4, days chip, step 1 primary CTA, step 2 blocked, step 3 links to the mandate", () => {
    ...
    expect(out).toMatch(/data-testid="evaluator-checklist-cta-1"[^>]*class="[^"]*bg-action/); // primary = next step (G26: navy primary)
  ...
  it("progress: done steps carry the tick and drop their CTA; the next undone step is primary", () => {
    ...
    expect(out).not.toContain('data-testid="evaluator-checklist-cta-2"');
    expect(out).toMatch(/data-testid="evaluator-checklist-cta-3"[^>]*class="[^"]*bg-action/);
