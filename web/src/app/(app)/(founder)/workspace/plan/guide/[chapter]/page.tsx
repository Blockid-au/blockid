/**
 * /workspace/plan/guide/[chapter] — Track B B2 + B3 + B4 in-app surface for the
 ...
 * /auth/login?next=/workspace/plan/guide/<slug>. Rendered inside WorkspaceLayout
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-surface-200 bg-white p-4">
        ...
        {c.ctaHref ? (
          <Link
            href={c.ctaHref}
            className="mt-3 inline-flex items-center gap-2 rounded-lg bg-bull px-4 py-2 text-sm font-semibold text-on-action transition hover:bg-emerald-800"
          >
...
export default async function WorkspaceGuideChapterPage({
  ...
  if (!user) redirect(`/auth/login?next=/workspace/plan/guide/${c.slug}`);
    ...
    <WorkspaceLayout user={user} isSandbox={isSandbox}>
      ...
      <div className="mx-auto max-w-3xl p-6">
        ...
        <nav className="mt-10 grid gap-3 border-t border-surface-200 pt-6 md:grid-cols-2">
          {previous ? (
            <Link
              href={`/workspace/plan/guide/${previous.slug}`}
          ...
          {next ? (
            <Link
              href={`/workspace/plan/guide/${next.slug}`}
