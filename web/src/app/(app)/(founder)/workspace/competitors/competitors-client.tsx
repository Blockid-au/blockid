...
export function CompetitorsClient({ initial, disabled }: Props) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {aiSuggestions.map((s, idx) => (
              <button
                ...
                className="w-full text-left rounded-xl border border-line-subtle bg-surface-sunken hover:bg-surface-hover px-4 py-3 transition-colors"
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="text-sm font-semibold text-secondary">{s.name}</span>
                  {typeof s.websiteScore === "number" && (
                    <span className="text-xs text-muted shrink-0">
                      Web score: <span className="text-action font-medium">{s.websiteScore}/100</span>
                    </span>
                  )}
                </div>
                {s.positioning && (
                  <p className="text-xs text-muted mt-0.5 line-clamp-1">{s.positioning}</p>
                )}
                {/* Tech signal chips */}
                {s.techSignals && s.techSignals.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {s.techSignals.map((chip, ci) => (
                      <span
                        key={ci}
                        className="bg-surface-hover border border-line-subtle text-muted rounded-full px-2 py-0.5 text-xs"
                      >
                        {chip}
                      </span>
                    ))}
                    {s.websiteScore !== undefined && s.websiteScore > 75 && (
                      <span className="bg-action/10 border border-action/25 text-action rounded-full px-2 py-0.5 text-xs">
                        Strong web presence
                      </span>
                    )}
                    {s.hasPricing && (
                      <span className="bg-warn/10 border border-warn/25 text-amber-300 rounded-full px-2 py-0.5 text-xs">
                        Has pricing page
                      </span>
                    )}
                  </div>
      ...
      {techComparison && (
        <section className="rounded-2xl border border-action/25 bg-action/5 px-5 py-4 space-y-3">
          <h2 className="text-sm font-semibold text-action">Tech Comparison</h2>
          <div className="flex flex-wrap gap-6 text-sm">
            {techComparison.ownTechScore != null ? (
              <div>
                <span className="text-muted text-xs uppercase tracking-wider">Your tech score</span>
              ...
              <div>
                <span className="text-muted text-xs uppercase tracking-wider">Your tech score</span>
                <div className="text-sm text-tertiary mt-0.5 italic">
