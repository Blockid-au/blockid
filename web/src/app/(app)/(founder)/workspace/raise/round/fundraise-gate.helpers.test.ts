...
describe("capTableDeadEnd (lane-1 F8)", () => {
  ...
  it("a founder who can write the cap table is sent to /workspace/equity/cap-table", () => {
    const d = capTableDeadEnd(API_MSG, (f) => f === "cap_table.write");
    expect(d).toMatchObject({ href: "/workspace/equity/cap-table", linkLabel: "Set up your cap table", upgrade: false });
