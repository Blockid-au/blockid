...
// S18-B render test for /workspace/team — member-aware founder-feature page.
...
vi.mock("@/lib/platform-config", () => ({
  getPlatformConfig: async () => ({
    founder_features_copy: { team_intro: "intro", team_suggested_advisors: 2 },
  }),
}));
...
vi.mock("@/lib/founder-features", async (importOriginal) => {
  ...
  return { ...actual, listTeamMembers: (scope: unknown) => listMock(scope) };
});
vi.mock("./team-planner-client", () => ({
  TeamPlannerClient: (p: { initial: unknown[]; disabled: boolean }) => (
...
beforeEach(() => {
  ...
  listMock.mockReset();
  listMock.mockResolvedValue([{ id: "t1", role_title: "CTO" }]);
});

describe("/workspace/team (S18-B)", () => {
