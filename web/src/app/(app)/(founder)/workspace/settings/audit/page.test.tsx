...
// Render test for /workspace/settings/audit (S20-A) with the data layer mocked.
...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/workspace/settings/audit",
}));
...
describe("/workspace/settings/audit", () => {
  it("redirects to login when signed out", async () => {
    getCurrentUserMock.mockResolvedValue(null);
    await expect(html()).rejects.toThrow("REDIRECT:/auth/login?next=/workspace/settings/audit");
  });
  ...
  it("paginates preserving filters; empty state", async () => {
    ...
    expect(out).toContain('href="/workspace/settings/audit?action=svi&amp;page=2"');
