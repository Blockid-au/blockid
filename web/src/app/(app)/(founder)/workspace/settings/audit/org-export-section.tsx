  ...
  return (
    <section className="mt-8 rounded-2xl border border-surface-200 bg-white p-6 shadow-sm" aria-labelledby="org-audit-export" data-testid="org-audit-export">
      ...
      <p className="mt-1 text-sm text-ink-700">
        The organisation rows written by the {admin.seats.length} seat{admin.seats.length === 1 ? "" : "s"} of <span className="font-medium">{admin.org.name}</span> — cohort, intake, override, pilot and institutional-API actions, plus anything on the cohorts and intake links created for this organisation — as a CSV. A seat holder&apos;s work for another organisation or their own startup is never included. Default window: the last {EXPORT_DEFAULT_DAYS} days; at most {EXPORT_MAX_DAYS} days per file. Cells are guarded against spreadsheet formulas; the export itself is recorded on the log.
      </p>
