...
export function FounderSignalsClient({ initial, readOnly = false }: Props) {
  ...
  const tab = (m: typeof mode, label: string) => (
    <button
      type="button"
      onClick={() => setMode(m)}
      className={`rounded-full px-3 py-1 text-sm ${mode === m ? "bg-brand-600 text-white" : "border border-surface-300 text-ink-700"}`}
      aria-pressed={mode === m}
    >
      {label}
    </button>
    ...
    <div className="space-y-8">
      {!readOnly && (
        <form onSubmit={submit} className="rounded-2xl border border-surface-300 bg-white p-5 space-y-4" data-testid="founder-evidence-form">
          ...
          <button type="submit" disabled={busy} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
