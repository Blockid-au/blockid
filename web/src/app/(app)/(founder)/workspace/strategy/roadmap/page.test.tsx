...
// S18-B render test for /workspace/strategy/roadmap — member-aware founder-feature page.
...
vi.mock("@/lib/platform-config", () => ({
  getPlatformConfig: async () => ({
    founder_features_copy: { roadmap_intro: "intro", roadmap_quarters_ahead: 4 },
  }),
}));
...
vi.mock("@/lib/founder-features", async (importOriginal) => {
  ...
  return { ...actual, listRoadmapMilestones: (scope: unknown) => listMock(scope) };
});
vi.mock("@/components/founder/conference-recommender", () => ({
  ConferenceRecommender: () => <div data-conferences />,
}));
vi.mock("./roadmap-builder-client", () => ({
  RoadmapBuilderClient: (p: { initial: unknown[]; disabled: boolean }) => (
    <div data-planner data-count={p.initial.length} data-disabled={String(p.disabled)} />
  ),
}));
...
beforeEach(() => {
  ...
  listMock.mockReset();
  listMock.mockResolvedValue([{ id: "m1", quarter: "2026-Q4" }]);
});

describe("/workspace/strategy/roadmap (S18-B)", () => {
