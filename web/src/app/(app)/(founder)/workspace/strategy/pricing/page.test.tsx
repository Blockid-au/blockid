...
// S18-B render test for /workspace/strategy/pricing — member-aware founder-feature page.
...
vi.mock("@/lib/platform-config", () => ({
  getPlatformConfig: async () => ({
    founder_features_copy: { pricing_intro: "intro", pricing_suggested_tiers: 3 },
  }),
}));
...
vi.mock("@/lib/founder-features", async (importOriginal) => {
  ...
  return { ...actual, listPricingTiers: (scope: unknown) => listMock(scope) };
});
vi.mock("./pricing-tiers-client", () => ({
  PricingTiersClient: (p: { initial: unknown[]; disabled: boolean }) => (
...
beforeEach(() => {
  ...
  listMock.mockReset();
  listMock.mockResolvedValue([{ id: "p1", name: "Pro" }]);
});

describe("/workspace/strategy/pricing (S18-B)", () => {
