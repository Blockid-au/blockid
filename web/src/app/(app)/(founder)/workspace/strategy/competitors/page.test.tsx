...
// S18-B render test for /workspace/strategy/competitors — member-aware founder-feature page.
...
vi.mock("@/lib/platform-config", () => ({
  getPlatformConfig: async () => ({
    founder_features_copy: { competitors_intro: "intro", competitors_suggested_direct: 3 },
  }),
}));
...
vi.mock("@/lib/founder-features", async (importOriginal) => {
  ...
  return { ...actual, listCompetitors: (scope: unknown) => listMock(scope) };
});
vi.mock("./competitors-client", () => ({
  CompetitorsClient: (p: { initial: unknown[]; disabled: boolean }) => (
...
beforeEach(() => {
  ...
  listMock.mockReset();
  listMock.mockResolvedValue([{ id: "c1", name: "Rival" }]);
});

describe("/workspace/strategy/competitors (S18-B)", () => {
