      ...
      <div className="mx-auto max-w-2xl p-6">
        <div className="rounded-2xl border border-dashed border-surface-300 bg-white px-6 py-10 text-center" data-testid="dossier-interstitial">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            {isEvaluator ? (
              <Link href="/workspace/evaluations" className="inline-flex min-h-11 items-center rounded-xl bg-action px-4 py-2.5 text-sm font-semibold text-on-action hover:bg-action-hover">
                Add to my evaluations
              </Link>
            ) : (
              <Link href="/pricing?segment=evaluator" className="inline-flex min-h-11 items-center rounded-xl bg-action px-4 py-2.5 text-sm font-semibold text-on-action hover:bg-action-hover">
                See evaluator plans
              </Link>
            )}
