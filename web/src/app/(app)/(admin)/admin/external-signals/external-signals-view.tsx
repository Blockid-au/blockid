...
const STATUS_TONE: Record<string, string> = {
  ...
  cite_only: "border-amber-200 bg-amber-50 text-amber-800",
  disabled: "border-surface-300 bg-surface-200 text-ink-700",
};
...
export function ExternalSignalsAdminView({ data }: { data: ExternalSignalsAdminData }) {
  ...
  return (
    <div className="min-h-svh bg-surface-100 px-4 py-8" data-testid="admin-external-signals">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">CDO · open AU data</p>
            <h1 className="text-2xl font-bold text-ink-900">External signals</h1>
          ...
          <div className="flex gap-2">
            <Link href="/methodology" className="rounded-lg border border-surface-300 px-3 py-1.5 text-sm text-ink-700 hover:bg-surface-200">
              /methodology
            </Link>
        ...
        <section className="grid gap-3 sm:grid-cols-4">
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Register rows</p>
            <p className="mt-2 text-2xl font-semibold tabular-nums text-ink-900" data-testid="external-signals-total">{n(totalRows)}</p>
            <p className="mt-1 text-xs text-ink-500">across {ingestable.length} ingestable sources · target ≥ 2,000</p>
          </div>
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Last run</p>
            <p className="mt-2 text-lg font-semibold text-ink-900">{summary ? when(summary.ran_at) : "never"}</p>
            <p className="mt-1 text-xs text-ink-500">{summary ? (summary.ok ? "ok" : "errors") + (summary.dry ? " · dry" : "") : (summaryError ?? "")}</p>
          </div>
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Last run inserted</p>
            <p className="mt-2 text-2xl font-semibold tabular-nums text-ink-900">{summary ? n(summary.totals.inserted) : "n/a"}</p>
            <p className="mt-1 text-xs text-ink-500">{summary ? `${n(summary.totals.duplicates)} duplicates · ${n(summary.totals.refused)} refused` : "no summary"}</p>
          </div>
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Allow-set · runs</p>
            <p className="mt-2 text-2xl font-semibold tabular-nums text-ink-900">{summary ? n(summary.allow_set_size) : "n/a"}</p>
            <p className="mt-1 text-xs text-ink-500">ABNs in the ABR allow-set · {n(historyLines)} runs logged</p>
          </div>
        </section>

        <section className="rounded-2xl border border-surface-200 bg-white p-5">
          <p className="text-sm font-medium text-ink-800">Sources {fromDb ? "(external_sources)" : "(code catalogue)"}</p>
            ...
            <table className="w-full text-sm" data-testid="external-signals-sources">
              ...
              <tbody className="text-ink-800 [&>tr:nth-child(even)]:bg-surface-sunken">
                {[...ingestable, ...citeOnly].map((s) => (
                  <tr key={s.id} className="border-t border-surface-100 align-top" data-source-id={s.id} data-source-status={s.status}>
                    <td className="py-2 pr-3">
                      <a href={s.url} target="_blank" rel="noopener noreferrer" className="font-medium underline decoration-dotted">
                        {s.name}
                      </a>
                      <p className="mt-1 max-w-md text-xs text-ink-500">{s.attribution_text}</p>
                      <p className="mt-1 font-mono text-[11px] text-ink-400">{s.id}</p>
                    </td>
        ...
        <section className="rounded-2xl border border-surface-200 bg-white p-5">
          <p className="text-sm font-medium text-ink-800">Last ingest summary</p>
                ...
                <tbody className="text-ink-800 [&>tr:nth-child(even)]:bg-surface-sunken">
                  {summary.sources.map((s) => (
                    <tr key={s.id} className="border-t border-surface-100">
                      ...
                      <td className="py-1.5 pr-3 text-xs text-ink-500">{s.error ?? s.file ?? ""}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <details className="mt-3">
                <summary className="cursor-pointer text-xs text-ink-500">Raw JSON</summary>
                <pre className="mt-2 max-h-96 overflow-auto rounded-lg border border-line-subtle bg-surface-sunken p-3 text-[11px] leading-relaxed text-ink-900" data-testid="external-signals-summary-json">
                  {JSON.stringify(summary, null, 2)}
                </pre>
              </details>
            </>
          ) : (
            <p className="mt-3 text-sm text-ink-500" data-testid="external-signals-no-run">{summaryError ?? "No run recorded yet."}</p>
          )}
        </section>
      </div>
    </div>
