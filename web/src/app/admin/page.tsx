...
export default async function AdminPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-svh bg-ink-950 flex items-center justify-center">
  ...
  return (
    <div className="min-h-svh bg-ink-950 text-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-50">BlockID Admin</h1>
          <p className="text-sm text-slate-500 mt-1">Manage users, SVI accounts, and system configuration.</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "App Users", value: stats.users, icon: Users, color: "text-brand-400" },
            ...
            <div key={label} className="rounded-2xl border border-ink-700 bg-ink-900 p-5">
        ...
        <div className="rounded-2xl border border-ink-700 bg-ink-900 overflow-hidden">
          <div className="px-6 py-4 border-b border-ink-700 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-100">SVI Accounts</h2>
            <span className="text-xs text-slate-500">{sviAccounts.length} accounts</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink-700">
                  ...
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">Startup</th>
                  <th className="text-right px-4 py-3 text-xs text-slate-500 font-medium">SVI</th>
                  <th className="text-right px-4 py-3 text-xs text-slate-500 font-medium">Stage</th>
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">Plan</th>
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">Enrolled</th>
                </tr>
              </thead>
              <tbody>
                {sviAccounts.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-600 text-sm">No SVI accounts yet</td></tr>
                ) : sviAccounts.map((acc) => (
                  <tr key={acc.email} className="border-b border-ink-700/50 hover:bg-ink-800/30 transition-colors">
                    <td className="px-6 py-3 text-slate-300 font-mono text-xs">{acc.email}</td>
                    <td className="px-4 py-3 text-slate-400 text-xs">{acc.startup_name ?? "—"}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={`font-mono font-bold text-sm ${acc.current_svi >= 140 ? "text-green-400" : acc.current_svi >= 120 ? "text-brand-400" : acc.current_svi >= 100 ? "text-amber-400" : "text-red-400"}`}>
                        {acc.current_svi}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-slate-400 text-xs">{acc.current_stage}</td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-medium rounded px-1.5 py-0.5 ${acc.plan === "founding50" ? "bg-brand-900/50 text-brand-300" : acc.plan === "pro" ? "bg-green-900/40 text-green-300" : "bg-ink-700 text-slate-500"}`}>
                        {acc.plan}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-500 text-xs">{new Date(acc.enrolled_at).toLocaleDateString("en-AU")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-2xl border border-ink-700 bg-ink-900 overflow-hidden">
          ...
          <div className="divide-y divide-ink-700/50">
            {recentAnalyses.length === 0 ? (
              <p className="px-6 py-6 text-center text-slate-600 text-sm">No analyses yet</p>
            ) : recentAnalyses.map((a, i) => (
              <div key={i} className="px-6 py-3 flex items-center justify-between">
                <span className="text-slate-400 font-mono text-xs">{a.email}</span>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-slate-600 text-xs">{new Date(a.created_at).toLocaleString("en-AU")}</span>
                </div>
              </div>
            ))}
          </div>
