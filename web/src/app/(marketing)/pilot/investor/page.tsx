/**
 * /pilot/investor — the comped evaluator pilot ("Free cohort scoring for one
 * intake", G16-C). G21 P0-C moved it here from /pilot, which is now the paid
 * Cohort Validation Pilot landing; this page is `robots: noindex` (the comp
 * is admin-only per G14 — invited programs get the link) and its API route
 * `POST /api/pilot/apply` is unchanged.
 *
...
const PILOT_PATH = "/pilot/investor";
...
export async function generateMetadata(): Promise<Metadata> {
  return pageMetadata({
    ...
    path: PILOT_PATH,
    // G21 P0-C: invitation-only comp — the public pilot is the paid one at /pilot.
    index: false,
  });
}
  ...
  return (
    <MarketingShell>
      <PageHero
        ...
        ctas={[
          { href: "#apply", label: "Apply for a pilot", ctaId: "pilot_investor_hero_apply" },
          { href: "/pilot", label: "The paid Cohort Validation Pilot" },
        ]}
        align="start"
      />
