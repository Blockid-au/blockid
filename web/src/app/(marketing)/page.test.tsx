...
describe("homepage v7 — sections in order", () => {
  it("<main> holds hero + problem · sequence · messages · why-not-chatgpt · built-for · cta, in that order, and the TrustBand placeholder for lane P0-A", () => {
    ...
    // + the hero section (aria-labelledby, no id) = seven.
    expect((main.match(/<section\b/g) ?? []).length).toBe(HOME_SECTION_IDS.length + 1);
    expect(main).toContain('aria-labelledby="page-hero-heading"');
    // The P0-A mount point is a marked comment between built-for and the close.
    const src = readFileSync(resolve(__dirname, "page.tsx"), "utf8");
    expect(src).toMatch(/G21 P0-A: <TrustBand \/> mounts here after merge/);
    expect(src.indexOf("G21 P0-A: <TrustBand />")).toBeGreaterThan(src.indexOf('id="built-for"'));
    expect(src.indexOf("G21 P0-A: <TrustBand />")).toBeLessThan(src.indexOf("<CtaBand"));
  });
...
describe("homepage v7 — chrome", () => {
  ...
  it("one header + one footer landmark; the footer is the shared one", () => {
