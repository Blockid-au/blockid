...
// Read the published roster at request time. Its generation timestamp is not
// stored, so the UI describes snapshot activity rather than asserting freshness.
...
export const metadata: Metadata = pageMetadata({
  ...
  description:
    "Meet BlockID’s founder and published AI advisor roles, with stored activity records. Founder-led, Sydney NSW.",
  path: "/team",
});
  ...
  return (
    <MarketingShell>
      <PageHero
        ...
        sub="AI advisor roles support platform work and customer reports. Cards show the published activity snapshot; its collection window may be historical."
        ctas={[
      ...
      <Section
        ...
        title="Published AI advisor roster"
        lede="These are published operational roles; report generation selects its own role set. Cards show the published activity snapshot; its collection window may be historical."
      >
        <StatStrip
          ariaLabel="Fleet activity"
          stats={[
            { value: String(roster.length), label: "Published roles" },
            { value: String(activeRoles), label: "Roles with stored activity" },
            { value: String(totalShips), label: "Stored activity records" },
          ]}
          caption="Published snapshot from team-roster.json. Counts describe its original 30-day collection window; refresh time is unavailable. Activity records are not verified deployments."
        />
                ...
                >
                  ...
                  <p className="mt-auto pt-4 text-xs text-muted">
                    {r.latest_activity_date
                      ? `Latest recorded activity ${r.latest_activity_date}`
