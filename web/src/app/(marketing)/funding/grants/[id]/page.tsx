...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!g || g.exclude_from_matching) return { title: "Grant not found · BlockID.au", robots: { index: false } };
  const range = formatAudRange(g.amount_min_aud, g.amount_max_aud, g.amount_note);
  const title = `${g.name} — ${range} · BlockID.au`;
  const description =
    g.summary ??
    `${g.name}: ${fundingTypeLabel(g.funding_type)} from ${g.provider ?? levelLabel(g.level)} for Australian startups (${stateLabel(g.state)}). Eligibility, evidence needed, how to apply and the official link.`;
  const path = `/funding/grants/${encodeURIComponent(g.id)}`;
  return {
    title,
    description,
