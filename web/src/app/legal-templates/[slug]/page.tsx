/**
 * /legal-templates/[slug] — public preview of a single AU legal template.
 *
 * Server-rendered from the raw markdown in `web/content/templates/legal/…`.
 * Uses the platform's minimal markdown renderer (same shape as /legal/[doc])
 * so we don't introduce a client-side dep.
 *
 * Includes:
 *   - prominent disclaimer at top
 *   - placeholder token list
 *   - "Download raw template" CTA → /api/templates/legal/[slug]?download=1
 */

import type { Metadata } from "next";
import { brandedOrAbsolute, fitDescription, pageMetadata } from "@/lib/seo/page-meta";
import Link from "next/link";
import { notFound } from "next/navigation";
import {
  getTemplate,
  listTemplates,
  readTemplateRaw,
} from "@/lib/templates/legal-templates";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export function generateStaticParams(): { slug: string }[] {
  return listTemplates().map((t) => ({ slug: t.slug }));
}

export async function generateMetadata({
  params,
}: {
  params: Promise<{ slug: string }>;
}): Promise<Metadata> {
  const { slug } = await params;
  const tpl = getTemplate(slug);
  if (!tpl) return {};
  return pageMetadata({
    title: brandedOrAbsolute(tpl.title),
    description: fitDescription([tpl.summary], { min: 70, max: 160 }),
    path: `/legal-templates/${tpl.slug}`,
  });
}

// ---------------------------------------------------------------------------
// Minimal markdown → HTML renderer (paragraphs, headings, lists, hr, code,
// bold, italics). Sufficient for hand-authored legal templates.
// ---------------------------------------------------------------------------

function escapeHtml(input: string): string {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function renderInline(input: string): string {
  let s = escapeHtml(input);
  s = s.replace(
    /`([^`]+)`/g,
    '<code class="rounded bg-surface-sunken px-1.5 py-0.5 text-[0.85em]">$1</code>',
  );
  s = s.replace(
    /\*\*([^*]+)\*\*/g,
    '<strong class="font-semibold text-primary">$1</strong>',
  );
  s = s.replace(/(^|[^*])\*([^*\n]+)\*/g, "$1<em>$2</em>");
  return s;
}

function renderMarkdown(md: string): string {
  const lines = md.replace(/\r\n?/g, "\n").split("\n");
  const out: string[] = [];
  let inUl = false;
  let inOl = false;
  let paraBuf: string[] = [];
  let inBlockquote = false;

  const flushList = () => {
    if (inUl) {
      out.push("</ul>");
      inUl = false;
    }
    if (inOl) {
      out.push("</ol>");
      inOl = false;
    }
  };
  const flushPara = () => {
    if (paraBuf.length > 0) {
      out.push(
        `<p class="mt-3 leading-relaxed text-slate-700">${renderInline(
          paraBuf.join(" "),
        )}</p>`,
      );
      paraBuf = [];
    }
  };
  const flushQuote = () => {
    if (inBlockquote) {
      out.push("</blockquote>");
      inBlockquote = false;
    }
  };

  for (const line of lines) {
    const trimmed = line.trim();

    if (trimmed === "") {
      flushPara();
      flushList();
      flushQuote();
      continue;
    }

    if (trimmed === "---") {
      flushPara();
      flushList();
      flushQuote();
      out.push('<hr class="my-8 border-line-subtle" />');
      continue;
    }

    // Headings
    const h = /^(#{1,6})\s+(.*)$/.exec(trimmed);
    if (h) {
      flushPara();
      flushList();
      flushQuote();
      const level = h[1]!.length;
      const text = renderInline(h[2]!);
      const cls =
        level === 1
          ? "mt-10 text-2xl font-bold text-primary"
          : level === 2
            ? "mt-8 text-xl font-semibold text-primary"
            : level === 3
              ? "mt-6 text-lg font-semibold text-primary"
              : "mt-5 text-base font-semibold text-primary";
      out.push(`<h${level} class="${cls}">${text}</h${level}>`);
      continue;
    }

    // Blockquote
    if (/^>\s?/.test(trimmed)) {
      flushPara();
      flushList();
      if (!inBlockquote) {
        out.push(
          '<blockquote class="mt-4 border-l-4 border-line bg-surface-sunken px-4 py-2 text-sm italic text-slate-700">',
        );
        inBlockquote = true;
      }
      out.push(
        `<p>${renderInline(trimmed.replace(/^>\s?/, ""))}</p>`,
      );
      continue;
    } else {
      flushQuote();
    }

    // Numbered list
    const ol = /^(\d+)\.\s+(.*)$/.exec(trimmed);
    if (ol) {
      flushPara();
      if (inUl) {
        out.push("</ul>");
        inUl = false;
      }
      if (!inOl) {
        out.push(
          '<ol class="mt-3 list-decimal space-y-1 pl-6 text-slate-700">',
        );
        inOl = true;
      }
      out.push(`<li>${renderInline(ol[2]!)}</li>`);
      continue;
    }

    // Bulleted list
    if (/^[-*]\s+/.test(trimmed)) {
      flushPara();
      if (inOl) {
        out.push("</ol>");
        inOl = false;
      }
      if (!inUl) {
        out.push(
          '<ul class="mt-3 list-disc space-y-1 pl-6 text-slate-700">',
        );
        inUl = true;
      }
      out.push(`<li>${renderInline(trimmed.replace(/^[-*]\s+/, ""))}</li>`);
      continue;
    }

    // Paragraph accumulator
    paraBuf.push(trimmed);
  }

  flushPara();
  flushList();
  flushQuote();
  return out.join("\n");
}

// ---------------------------------------------------------------------------
// Page
// ---------------------------------------------------------------------------

export default async function LegalTemplateDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const tpl = getTemplate(slug);
  if (!tpl) notFound();

  const raw = await readTemplateRaw(slug);
  if (raw == null) notFound();

  const html = renderMarkdown(raw);

  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      <nav className="mb-6 text-sm text-muted">
        <Link href="/legal-templates" className="hover:underline">
          ← All legal templates
        </Link>
      </nav>

      <header className="mb-8">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted">
          {tpl.category}
        </p>
        <h1 className="mt-2 text-3xl font-bold text-primary">
          {tpl.title}
        </h1>
        <p className="mt-2 text-sm text-secondary">
          {tpl.summary}
        </p>
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <a
            href={`/api/templates/legal/${tpl.slug}?download=1`}
            className="inline-flex items-center rounded-md bg-action px-4 py-2 text-sm font-medium text-on-action hover:bg-action-hover"
          >
            Download this template (.md)
          </a>
          <span className="text-xs text-muted">
            v{tpl.version} · revised {tpl.revision_date}
          </span>
        </div>
      </header>

      <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
        <strong>Template only — not legal advice.</strong> {tpl.disclaimer}
      </div>

      <aside className="mb-8 rounded-lg border border-line-subtle bg-surface-sunken p-4">
        <h2 className="text-sm font-semibold text-primary">
          Placeholders to fill in
        </h2>
        <p className="mt-1 text-xs text-secondary">
          Every <code className="rounded bg-slate-200 px-1">{`{{token}}`}</code>
          {" "}below must be replaced before the document is signed. Tokens
          wrapped like <code className="rounded bg-slate-200 px-1">{`{{#name}}…{{/name}}`}</code>
          {" "}are variant toggles.
        </p>
        <ul className="mt-3 flex flex-wrap gap-2">
          {tpl.placeholders.map((p) => (
            <li
              key={p}
              className="rounded bg-white px-2 py-0.5 text-xs font-mono text-slate-700 shadow-sm ring-1 ring-slate-200"
            >
              {`{{${p}}}`}
            </li>
          ))}
        </ul>
      </aside>

      <article
        className="prose prose-slate max-w-none"
        // Rendered HTML is produced by our own renderer over trusted
        // source-controlled markdown, not user input.
        dangerouslySetInnerHTML={{ __html: html }}
      />

      <footer className="mt-12 border-t border-line-subtle pt-6 text-xs text-muted">
        Sources:
        <ul className="mt-2 space-y-1">
          {tpl.sources.map((s) => (
            <li key={s.url}>
              <a
                href={s.url}
                target="_blank"
                rel="noopener noreferrer"
                className="hover:underline"
              >
                {s.label}
              </a>
            </li>
          ))}
        </ul>
      </footer>
    </main>
  );
}
