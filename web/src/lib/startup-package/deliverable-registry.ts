...
function parseNumericAnswer(
  ...
  for (const key of keys) {
    const hit = ctx.interviewAnswers.find((a) => a.step_key === key);
    if (!hit?.answer_text) continue;
    // Strip currency symbols, commas, whitespace.
    const cleaned = hit.answer_text.replace(/[^0-9.\-]/g, "");
...
function inferStageFromSvi(
  ctx: DeliverableInputContext,
): "pre-seed" | "seed" | "series-a" | "series-b" {
  const s = ctx.sviAnalysis?.stage;
  if (typeof s !== "number") return "pre-seed";
  if (s <= 1) return "pre-seed";
  if (s <= 3) return "seed";
  if (s <= 5) return "series-a";
  return "series-b";
}
...
const REGISTRY: DeliverableEntry[] = [
  ...
  {
    key: "financial_projection_3yr",
    phaseId: "revenue_model",
    label: "3-year financial projection",
    blurb: "P&L + cash burn + runway for 12 quarters, from your revenue-model inputs.",
    pdfGenerator: "financial-projection",
    featureKey: "financial_projection",
    dataroomFolder: "financial",
    templateSlug: "package_revenue_model_financial_projection_3yr",
    inputBuilder: (ctx) => ({
      startupName: ctx.project.name,
      stage: inferStageFromSvi(ctx),
      sector: ctx.project.industry ?? undefined,
      mrrAud: parseNumericAnswer(ctx, ["rm_mrr", "revenue_mrr"]),
      monthlyBurnAud: parseNumericAnswer(ctx, ["rm_burn", "monthly_burn"]),
      cashAud: parseNumericAnswer(ctx, ["rm_cash", "cash_bank"]),
      grossMarginPct: parseNumericAnswer(ctx, ["rm_gross_margin", "gross_margin"]),
      monthlyGrowthPct: parseNumericAnswer(ctx, ["rm_growth", "monthly_growth"]),
      email: ctx.founderEmail,
    }),
  },
  ...
  {
    key: "gtm_strategy_doc",
    phaseId: "go_to_market",
    label: "GTM strategy doc (CMO agent)",
    blurb: "12-page positioning + channel mix + 90-day plan + KPIs, generated by the CMO agent.",
    pdfGenerator: "gtm-strategy",
    featureKey: "gtm_doc",
    dataroomFolder: "gtm",
    templateSlug: "package_go_to_market_gtm_strategy_doc",
    inputBuilder: (ctx) => ({
      startupName: ctx.project.name,
      sector: ctx.project.industry ?? "startup",
      stage: ctx.sviAnalysis?.stage ?? 1,
      description: ctx.project.description ?? undefined,
      targetCustomer: firstAnswer(ctx, ["cd_persona", "target_customer"], ""),
      email: ctx.founderEmail,
      founderName: ctx.founderName,
    }),
  },
