...
export async function loadActiveGrant(
  ...
  if (projectId) {
    // Match grants scoped to this project OR grants that cover all projects.
 ...
 * used by the founder-side /dashboard/settings/mentor-access page. Callers
 * are expected to filter revoked_at / expires_at at render time so the
 * UI can show a distinct "expired" row style.
 */
