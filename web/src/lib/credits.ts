...
export const FEATURE_COSTS: Record<string, number> = {
  ...
  investor_pack: 5.00,         // A$5.00 — full investor pack PDF (SVI + valuation + checklist + comps + team + cap-table + ask)
  ...
  // ── Startup Package — Phase 3.1 auto-fill deliverables ──────────────
  // Mirrored in PACKAGE_FEATURE_COST_DEFAULTS (client-safe map).
  ...
  // ── Evaluator Trusted Business Report (T0271, G12 §3b) ──────────────────────
  ...
  trust_report: 3.00,               // A$3 — full Trusted Business Report per startup entered
...
//
// The binding cost is `enhanced_report_standard` (3.00 credits) — the only key
...
//
// Mirrors plans.csv `usage_limits.monthly_credits`; regenerate plans.generated.ts
// with `npx tsx scripts/build-plans.ts` after changing either.
export const PLAN_CREDITS: Record<string, { amount: number; recurring: boolean }> = {
  // ── Legacy (grandfathered — do NOT reduce) ──────────────────────────────
  ...
  // is ~378 runs; 1000 credits ≈ 333 runs sits just inside that.
  founder_enterprise: { amount: 1000, recurring: true  },
};
