...
const DEMO_DIMS: Record<string, SnapshotDimState> = {
  ...
  tre: {
    ...
    score: 78,
    priority: "medium",
    insights: ["A$1.2M ARR (A$100k MRR) growing 4.5 % MoM; 120 paying SMEs, churn 2.1 %.", "NRR 104 % on the last four cohorts; top customer is 9 % of revenue."],
...
const DEMO_CRITERIA: SnapshotCriterionState[] = [
  ...
  { key: "code_git", title: "Code & Git Repository", primary_dimension: "ptd", weight: 8, score: 79, verdict: "Repo linked, audit pending (unverified): 210 commits in 90 days and 71 % test coverage are founder-reported; CI on every PR.", strengths: ["CI green for 12 weeks", "Dependency freshness 92 %"], gaps: ["Bus factor 2 on the billing service"], next_action: "Pair a second engineer on billing." },
...
export function demoEvidenceRows(): EvidenceRow[] {
  return [
    // G23-A: the register carries every number the sample narrates (the demo is
    // the grounding reference: groundedShare ≥ 0.85 is asserted in report.test).
    { evidence_id: "ev-connected-revenue-stripe", source: "stripe", label: "Stripe revenue (last sync)", status: "evidenced", observedAt: "2026-09-10T00:00:00.000Z", value: "mrr_aud = 100000; arr_aud = 1200000; prior_mrr_aud = 95700; mom_growth_pct = 4.5; active_customers = 120; net_new_per_month = 9; churn_90d_pct = 2.1; nrr_pct = 104; top_customer_share_pct = 9", dims: ["tre", "iri", "cgh"], confidence: "transaction_data" },
    { evidence_id: "ev-connected-xero-pnl", source: "xero", label: "Xero P&L (last sync)", status: "evidenced", observedAt: "2026-09-10T00:00:00.000Z", value: "gross_margin_pct = 74; ltv_cac = 3.4; cac_payback_months = 11; burn_multiple = 1.4; rule_of_40 = 52; expansion_share_of_nrr_pct = 6", dims: ["tre", "cgh", "iri"], confidence: "transaction_data" },
    { evidence_id: "ev-connected-ga4-acquisition", source: "ga4", label: "GA4 acquisition (last 90 days)", status: "evidenced", observedAt: "2026-09-11T00:00:00.000Z", value: "sessions_90d = 41200; partner_channel_share_of_new_logos_pct = 60; signup_conversion_pct = 3.1", dims: ["mpc", "tre"], confidence: "connected_source" },
    { evidence_id: "ev-market-anchor-abs", source: "external", label: "AU market anchor (ABS / IBISWorld)", status: "evidenced", observedAt: "2026-09-01T00:00:00.000Z", value: "ANZSIC 7000 Computer System Design; TAM A$4.2bn; SAM A$310M (41,000 eligible SMEs × A$7.5k ACV); SOM A$15.5M; CAGR 6.4 %", dims: ["mpc", "svm", "tre"], confidence: "public_url" },
    { evidence_id: "ev-hub-data-room", source: "upload", label: "Data room (Evidence Hub, 8 folders)", status: "evidenced", observedAt: "2026-09-12T00:00:00.000Z", value: "folders = 8; completeness_pct = 78; dd_checklist_pct = 80; deck = current; one_pager = current; model = actual-vs-plan", dims: ["iri", "cgh", "lco"], confidence: "connected_source" },
