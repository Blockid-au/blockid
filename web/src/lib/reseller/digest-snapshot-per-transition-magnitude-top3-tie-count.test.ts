import { describe, expect, it } from "vitest";

import type { KnownKpiSection } from "./digest-snapshot";
import type {
  DigestSnapshotPerPairHotCells,
  PerPairHotCellRow,
} from "./digest-snapshot-per-pair-hot-cells";
import {
  MAGNITUDE_MEDIUM_MAX,
  MAGNITUDE_SMALL_MAX,
} from "./digest-snapshot-per-transition-magnitude-drilldown";
import {
  computeDigestSnapshotPerTransitionMagnitudeTop3TieCount,
  formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection,
  TOP_N,
} from "./digest-snapshot-per-transition-magnitude-top3-tie-count";

type TransitionToken = PerPairHotCellRow["transition"];

function cell(
  code: string,
  key: KnownKpiSection,
  transition: TransitionToken,
  hot_score: number,
  overrides: Partial<PerPairHotCellRow> = {},
): PerPairHotCellRow {
  const base: PerPairHotCellRow = {
    reseller_code: code,
    key,
    metric_name: `${key} name`,
    unit: "cents",
    transition,
    from_verdict: transition === "first_classification" ? null : "flat",
    to_verdict: "sustained_both_axes",
    delta_rank: (() => {
      switch (transition) {
        case "improved":
          return hot_score;
        case "degraded":
          return -hot_score;
        case "rotated":
        case "stable":
          return 0;
        case "undecidable":
        case "first_classification":
          return null;
      }
    })(),
    summary: `stub ${code} × ${key} ${transition}`,
    hot_score,
  };
  return { ...base, ...overrides };
}

function envelope(
  rows: PerPairHotCellRow[],
  overrides: Partial<Omit<DigestSnapshotPerPairHotCells, "rows">> = {},
): DigestSnapshotPerPairHotCells {
  return {
    window_size: 4,
    first_week: "2026-W28",
    last_week: "2026-W31",
    sustained_p90_threshold: 3,
    threshold: 0.25,
    rows,
    ...overrides,
  };
}

describe("computeDigestSnapshotPerTransitionMagnitudeTop3TieCount — envelope passthrough", () => {
  it("carries window_size / first_week / last_week / sustained_p90_threshold / threshold verbatim", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("ACME", "attributed_mrr", "improved", 2)], {
        window_size: 7,
        first_week: "2026-W25",
        last_week: "2026-W31",
        sustained_p90_threshold: 5,
        threshold: 0.4,
      }),
    );
    expect(out.window_size).toBe(7);
    expect(out.first_week).toBe("2026-W25");
    expect(out.last_week).toBe("2026-W31");
    expect(out.sustained_p90_threshold).toBe(5);
    expect(out.threshold).toBe(0.4);
  });

  it("keeps null first_week / last_week when the source envelope carries null", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([], { first_week: null, last_week: null }),
    );
    expect(out.first_week).toBeNull();
    expect(out.last_week).toBeNull();
  });

  it("emits band_thresholds pinned to MAGNITUDE_SMALL_MAX and MAGNITUDE_MEDIUM_MAX constants", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([]),
    );
    expect(out.band_thresholds.small_max).toBe(MAGNITUDE_SMALL_MAX);
    expect(out.band_thresholds.medium_max).toBe(MAGNITUDE_MEDIUM_MAX);
  });

  it("top_n scalar equals the exported TOP_N constant (3)", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([]),
    );
    expect(out.top_n).toBe(TOP_N);
    expect(out.top_n).toBe(3);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3TieCount — envelope shape stability", () => {
  it("empty rows → all four transitions present with zero counts across all three bands", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([]),
    );
    expect(out.total_hot_cells).toBe(0);
    for (const t of ["improved", "degraded", "rotated", "undecidable"] as const) {
      for (const b of ["small", "medium", "large"] as const) {
        const band = out.transitions[t].bands[b];
        expect(band.partner_top1_tie_count).toBe(0);
        expect(band.metric_top1_tie_count).toBe(0);
        expect(band.partner_top1_cells).toBeNull();
        expect(band.metric_top1_cells).toBeNull();
        expect(band.partner_count).toBe(0);
        expect(band.metric_count).toBe(0);
      }
    }
  });

  it("transitions object always ships all four transition keys", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 1)]),
    );
    expect(Object.keys(out.transitions).sort()).toEqual([
      "degraded",
      "improved",
      "rotated",
      "undecidable",
    ]);
  });

  it("bands object always ships all three band keys within every transition", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 1)]),
    );
    for (const t of ["improved", "degraded", "rotated", "undecidable"] as const) {
      expect(Object.keys(out.transitions[t].bands).sort()).toEqual([
        "large",
        "medium",
        "small",
      ]);
    }
  });

  it("rotated + undecidable ship zero tie counts in medium + large (hot_score=1 by P11.139 design)", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("A", "attributed_mrr", "rotated", 1),
        cell("B", "attributed_net_contribution", "undecidable", 1),
      ]),
    );
    expect(out.transitions.rotated.bands.small.partner_top1_tie_count).toBeGreaterThan(0);
    expect(out.transitions.rotated.bands.medium.partner_top1_tie_count).toBe(0);
    expect(out.transitions.rotated.bands.large.partner_top1_tie_count).toBe(0);
    expect(out.transitions.undecidable.bands.medium.metric_top1_tie_count).toBe(0);
    expect(out.transitions.undecidable.bands.large.metric_top1_tie_count).toBe(0);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3TieCount — tie counting", () => {
  it("clear leader (single partner) → partner_top1_tie_count === 1", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("SOLO", "attributed_mrr", "improved", 4)]),
    );
    const band = out.transitions.improved.bands.medium;
    expect(band.partner_top1_tie_count).toBe(1);
    expect(band.partner_top1_cells).toBe(1);
    expect(band.partner_count).toBe(1);
  });

  it("clear leader (4/1/1 split) → partner_top1_tie_count === 1 with partner_top1_cells === 4", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("BIG", "attributed_mrr", "improved", 4),
        cell("BIG", "attributed_net_contribution", "improved", 4),
        cell("BIG", "commission_cleared_mtd", "improved", 4),
        cell("BIG", "budget_utilization", "improved", 4),
        cell("MID", "attributed_mrr", "improved", 4),
        cell("LOW", "attributed_mrr", "improved", 4),
      ]),
    );
    const band = out.transitions.improved.bands.medium;
    expect(band.partner_top1_tie_count).toBe(1);
    expect(band.partner_top1_cells).toBe(4);
    expect(band.partner_count).toBe(3);
  });

  it("3-way tie (3/3/3) → partner_top1_tie_count === 3", () => {
    const rows: PerPairHotCellRow[] = [];
    for (const code of ["ALPHA", "BETA", "GAMMA"] as const) {
      for (const k of [
        "attributed_mrr",
        "attributed_net_contribution",
        "commission_cleared_mtd",
      ] as const) {
        rows.push(cell(code, k, "improved", 4));
      }
    }
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope(rows),
    );
    const band = out.transitions.improved.bands.medium;
    expect(band.partner_top1_tie_count).toBe(3);
    expect(band.partner_top1_cells).toBe(3);
    expect(band.partner_count).toBe(3);
  });

  it("2-way tie at the top with a lower runner-up (2/2/1) → partner_top1_tie_count === 2", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("ALPHA", "attributed_mrr", "improved", 4),
        cell("ALPHA", "attributed_net_contribution", "improved", 4),
        cell("BETA", "attributed_mrr", "improved", 4),
        cell("BETA", "attributed_net_contribution", "improved", 4),
        cell("GAMMA", "attributed_mrr", "improved", 4),
      ]),
    );
    const band = out.transitions.improved.bands.medium;
    expect(band.partner_top1_tie_count).toBe(2);
    expect(band.partner_top1_cells).toBe(2);
    expect(band.partner_count).toBe(3);
  });

  it("metric tie counting mirrors partner formula", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        // attributed_mrr: 2 cells
        cell("A", "attributed_mrr", "degraded", 6),
        cell("B", "attributed_mrr", "degraded", 6),
        // attributed_net_contribution: 2 cells
        cell("C", "attributed_net_contribution", "degraded", 6),
        cell("D", "attributed_net_contribution", "degraded", 6),
        // commission_cleared_mtd: 1 cell
        cell("E", "commission_cleared_mtd", "degraded", 6),
      ]),
    );
    const band = out.transitions.degraded.bands.large;
    expect(band.metric_top1_tie_count).toBe(2);
    expect(band.metric_top1_cells).toBe(2);
    expect(band.metric_count).toBe(3);
  });

  it("empty cell → tie count 0 and top1_cells null", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 4)]),
    );
    const emptyBand = out.transitions.degraded.bands.small;
    expect(emptyBand.partner_top1_tie_count).toBe(0);
    expect(emptyBand.partner_top1_cells).toBeNull();
    expect(emptyBand.metric_top1_tie_count).toBe(0);
    expect(emptyBand.metric_top1_cells).toBeNull();
  });

  it("counts are scoped per (transition, band) — improved and degraded stay independent", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("A", "attributed_mrr", "improved", 10),
        cell("A", "attributed_net_contribution", "improved", 10),
        cell("B", "attributed_mrr", "degraded", 10),
      ]),
    );
    expect(out.transitions.improved.bands.large.partner_top1_tie_count).toBe(1);
    expect(out.transitions.improved.bands.large.partner_top1_cells).toBe(2);
    expect(out.transitions.degraded.bands.large.partner_top1_tie_count).toBe(1);
    expect(out.transitions.degraded.bands.large.partner_top1_cells).toBe(1);
    expect(out.transitions.improved.bands.small.partner_top1_tie_count).toBe(0);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3TieCount — total_hot_cells parity", () => {
  it("total_hot_cells counts every alert-worthy row in the source envelope", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("A", "attributed_mrr", "improved", 4),
        cell("B", "attributed_mrr", "degraded", 6),
        cell("C", "attributed_mrr", "rotated", 1),
        cell("D", "attributed_mrr", "undecidable", 1),
      ]),
    );
    expect(out.total_hot_cells).toBe(4);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3TieCount — determinism", () => {
  it("re-runs on reversed input produce identical transitions map", () => {
    const rows: PerPairHotCellRow[] = [
      cell("A", "attributed_mrr", "improved", 3),
      cell("B", "attributed_net_contribution", "degraded", 7),
      cell("C", "attributed_mrr", "improved", 1),
      cell("A", "attributed_net_contribution", "improved", 3),
      cell("B", "attributed_mrr", "improved", 3),
    ];
    const first = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope(rows),
    );
    const second = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([...rows].reverse()),
    );
    expect(first.transitions).toEqual(second.transitions);
  });
});

describe("formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection — suppression", () => {
  it("returns '' when window_size < 3", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 2)], { window_size: 2 }),
    );
    expect(
      formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(out),
    ).toBe("");
  });

  it("returns '' when total_hot_cells === 0", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([]),
    );
    expect(
      formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(out),
    ).toBe("");
  });

  it("renders non-empty HTML for window_size >= 3 and >= 1 hot cell", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("ACME", "attributed_mrr", "improved", 4)]),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).not.toBe("");
    expect(html).toContain("Per-transition magnitude TOP-3 tie count");
  });
});

describe("formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection — content", () => {
  it("caption carries window_size / first_week / last_week / threshold pct / sustained bar", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 3)], {
        window_size: 5,
        first_week: "2026-W27",
        last_week: "2026-W31",
        threshold: 0.25,
        sustained_p90_threshold: 3,
      }),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).toContain("5-week window");
    expect(html).toContain("2026-W27");
    expect(html).toContain("2026-W31");
    expect(html).toContain("25.0%");
    expect(html).toContain("p90 &ge; 3");
    expect(html).toContain("TOP-3");
  });

  it("caption names band cutoffs from the envelope", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("A", "attributed_mrr", "improved", 4)]),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).toContain("small (1..2)");
    expect(html).toContain("medium (3..5)");
    expect(html).toContain("large (6+)");
  });

  it("omits empty (transition, band) cells from the visual table but keeps them on the envelope with count 0", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([cell("ACME", "attributed_mrr", "improved", 4)]),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).toContain("1 @ 1");
    expect(out.transitions.degraded.bands.small.partner_top1_tie_count).toBe(0);
    expect(out.transitions.rotated.bands.large.metric_top1_tie_count).toBe(0);
  });

  it("labels a solo entry as clear and a 3/3/3 top-3 as tied", () => {
    const clearOut = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("BIG", "attributed_mrr", "improved", 4),
        cell("BIG", "attributed_net_contribution", "improved", 4),
        cell("BIG", "commission_cleared_mtd", "improved", 4),
        cell("BIG", "budget_utilization", "improved", 4),
        cell("MID", "attributed_mrr", "improved", 4),
      ]),
    );
    const clearHtml =
      formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(clearOut);
    expect(clearHtml).toContain("clear");

    const tiedRows: PerPairHotCellRow[] = [];
    for (const code of ["ALPHA", "BETA", "GAMMA"] as const) {
      for (const k of [
        "attributed_mrr",
        "attributed_net_contribution",
        "commission_cleared_mtd",
      ] as const) {
        tiedRows.push(cell(code, k, "improved", 4));
      }
    }
    const tiedOut = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope(tiedRows),
    );
    const tiedHtml =
      formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(tiedOut);
    expect(tiedHtml).toContain("tied");
  });

  it("HTML-escapes week labels", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope(
        [cell("ACME", "attributed_mrr", "improved", 4)],
        { first_week: "2026-<W>", last_week: "2026-W31" },
      ),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).toContain("2026-&lt;W&gt;");
    expect(html).not.toContain("2026-<W>");
  });

  it("renders transition arrow labels for each transition that fires", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("A", "attributed_mrr", "improved", 3),
        cell("B", "attributed_mrr", "degraded", 4),
        cell("C", "attributed_mrr", "rotated", 1),
        cell("D", "attributed_mrr", "undecidable", 1),
      ]),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    expect(html).toContain("&uarr;");
    expect(html).toContain("&darr;");
    expect(html).toContain("&harr;");
    expect(html).toContain("undecidable ?");
  });

  it("renders `N @ C` cell for a non-empty (transition, band)", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3TieCount(
      envelope([
        cell("BIG", "attributed_mrr", "improved", 4),
        cell("BIG", "attributed_net_contribution", "improved", 4),
        cell("MID", "attributed_mrr", "improved", 4),
      ]),
    );
    const html = formatDigestSnapshotPerTransitionMagnitudeTop3TieCountSection(
      out,
    );
    // BIG=2 cells clear leader; format `1 @ 2` for partner
    expect(html).toContain("1 @ 2");
  });
});
