...
} from "./digest-snapshot-per-reseller-direction-streak-length-histogram";
...
describe("computeDigestSnapshotPerResellerDirectionStreakLengthHistogram — shape", () => {
  it("passes through window metadata + min_streak_length", () => {
    ...
    const hist =
      computeDigestSnapshotPerResellerDirectionStreakLengthHistogram(trend);
  ...
  it("returns zero groups on empty trend", () => {
    const hist = computeDigestSnapshotPerResellerDirectionStreakLengthHistogram(
...
describe("computeDigestSnapshotPerResellerDirectionStreakLengthHistogram — grouping", () => {
...
describe("computeDigestSnapshotPerResellerDirectionStreakLengthHistogram — buckets", () => {
  it("emits a dense per-partner bucket set from min_streak_length to that partner's max_length", () => {
    // ACME: two qualifying streaks: mrr length-2 (going up 3 snapshots) + churn length-4 (going down 5 snapshots).
    // Expect ACME buckets to be 2/3/4 with zero at length 3.
    const trend = computeDigestSnapshotPerResellerRollingTrend([
  ...
  it("scopes max_length per partner (sibling's deeper tail does not pad this partner's axis)", () => {
    // ACME: length-2 mrr streak (3 snapshots). ZEBRA: length-4 churn streak (5 snapshots).
    // Expect ACME buckets to run only 2..2, ZEBRA buckets to run 2..4.
    const trend = computeDigestSnapshotPerResellerRollingTrend([
      bothSnap(
        0,
...
describe("formatDigestSnapshotPerResellerDirectionStreakLengthHistogramSection", () => {
  it("returns '' when window_size < 3", () => {
    ...
    const hist =
      computeDigestSnapshotPerResellerDirectionStreakLengthHistogram(trend);
    expect(
      formatDigestSnapshotPerResellerDirectionStreakLengthHistogramSection(hist),
    ).toBe("");
  });
  ...
  it("renders one table per partner group with a bar column", () => {
    const trend = computeDigestSnapshotPerResellerRollingTrend([
