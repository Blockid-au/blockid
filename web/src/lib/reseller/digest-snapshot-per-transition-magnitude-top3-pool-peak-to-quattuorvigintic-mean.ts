// Weekly digest PER-TRANSITION MAGNITUDE TOP-3 POOL PEAK-TO-QUATTUORVIGINTIC-MEAN
// pure-lib (P11.302).
//
...
//
//   ptqvim = (max - min) / quattuorvigintic_mean
//
...
//
// Composite regime labels emitted by joining PTQVIM+PTRVM+PTDVIM+PTUVM+PTVIM+PTSOM+PTSPM+PTSDM:
//
//   * PTQVIM tight + PTRVM tight + PTDVIM tight + PTUVM tight + PTVIM tight + PTSOM tight + PTSPM tight + PTSDM tight
//                                     -> SYMMETRIC POOL or MODERATE-
//                                     SKEW pool where every power-
//                                     mean anchor sits close enough
//                                     to the range to dampen.
//   * PTQVIM tight + PTRVM tight + PTDVIM tight + PTUVM tight + PTVIM tight + PTSOM tight
...
//                                     PTRVM + PTQVIM absorb by raising
...
//                                     [1x99, 100] reads PTQVIM 1.1994
//                                     wide.
//   * PTQVIM spread + PTRVM spread + PTDVIM spread + PTSOM tight
...
// quattuorvigintic_mean approaches x_max / n^(1/24), so ptqvim
...
// in a 10-partner pool reads ptqvim just past 1.10 (spread but
...
// PTQVIM an even CLEANER outlier-tolerance read than PTRVM in the
...
//                                     guarded for future upstream
//                                     robustness).
//   * pool_count >= 2 and           -> ptqvim in [0, +Inf) rounded to
...
//   * flat [k,k,...,k]              -> QVIM = k, range 0, ptqvim 0
//                                     (tight).
//   * uniform ramp [1..10]          -> QVIM ~= 9.1160, range 9, ptqvim
//                                     ~= 0.9873 (tight -- well under
//                                     the 1.005 tight/spread boundary).
//   * upper-outlier [1x9, 10]       -> QVIM ~= 9.0852, range 9, ptqvim
...
//                                     regime in spread).
//   * small [10, 1, 1]              -> QVIM ~= 9.5526, range 9, ptqvim
//                                     ~= 0.9422 (TIGHT --
...
//
// Bands on raw ptqvim (fixed cutoffs, calibrated against the n=10
...
//                          unreachable)
//   * tight                ptqvim < 1.005 (flat, uniform ramp, upper-
...
//                          anchor)
//   * spread               ptqvim in [1.005, 1.09) (extreme-outlier
...
//   * wide                 ptqvim >= 1.09 (RUNAWAY-OUTLIER regime with
...
//
// Both cutoffs are exposed on the envelope as tight_ptqvim_max /
// wide_ptqvim_min so downstream JSONL consumers render the label
// vocabulary without importing the TS module.
//
// LABEL ORIENTATION follows the range-based DISPERSION framing
// (HIGH ptqvim = MORE range against quattuorvigintic centre = MORE
...
// -> PEAK-TO-QUATTUORVIGINTIC-MEAN (this module) -> per-pair
...
// Bands on raw ptqvim (fixed cutoffs since quattuorvigintic_mean scales
...
// reach it).
const TIGHT_PTQVIM_MAX = 1.005;
const WIDE_PTQVIM_MIN = 1.09;

// PTQVIM rounded to 4 decimals so weekly digests are stable under
// float-noise re-runs. Same precision as every other pool-shape
// sibling.
const PTQVIM_DECIMALS = 4;

export interface PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBand {
  readonly partner_pool_count: number;
  readonly partner_pool_cells: number;
  readonly partner_peak_to_quattuorvigintic_mean: number | null;
  readonly metric_pool_count: number;
  readonly metric_pool_cells: number;
  readonly metric_peak_to_quattuorvigintic_mean: number | null;
}

export interface PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBands {
  readonly small: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBand;
  readonly medium: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBand;
  readonly large: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBand;
}

export interface PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry {
  readonly bands: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBands;
}

export interface PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanMap {
  readonly improved: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry;
  readonly degraded: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry;
  readonly rotated: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry;
  readonly undecidable: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry;
}

export interface DigestSnapshotPerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMean {
  ...
  readonly tight_ptqvim_max: number;
  readonly wide_ptqvim_min: number;
  readonly band_thresholds: BandThresholds;
  readonly transitions: PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanMap;
}
...
function foldMap(cellsByKey: Map<string, number>): {
  pool_count: number;
  pool_cells: number;
  peak_to_quattuorvigintic_mean: number | null;
} {
  const values = Array.from(cellsByKey.values());
  const pool_count = values.length;
  const pool_cells = values.reduce((a, b) => a + b, 0);
  if (pool_count === 0) {
    return { pool_count, pool_cells, peak_to_quattuorvigintic_mean: null };
  }
  if (pool_count === 1) {
  ...
  const range = max - min;
  const ptqvim = range / quattuorvigintic_mean;
  // Clamp tiny negative float-noise to 0; ptqvim is non-negative by
  // construction because range >= 0 and quattuorvigintic_mean > 0.
  const clamped = ptqvim < 0 ? 0 : ptqvim;
  return {
    pool_count,
    pool_cells,
    peak_to_quattuorvigintic_mean: roundTo(clamped, PTQVIM_DECIMALS),
  };
}

function finaliseBand(
  buckets: BandBuckets,
): PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanBand {
  ...
  return {
    ...
    partner_peak_to_quattuorvigintic_mean:
      partner.peak_to_quattuorvigintic_mean,
    metric_pool_count: metric.pool_count,
    metric_pool_cells: metric.pool_cells,
    metric_peak_to_quattuorvigintic_mean: metric.peak_to_quattuorvigintic_mean,
  };
}

function finaliseTransition(
  buckets: TransitionBuckets,
): PerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanEntry {
  return {
    bands: {
...
export function computeDigestSnapshotPerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMean(
  ...
  return {
    ...
    total_hot_cells,
    top_n: TOP_N,
    tight_ptqvim_max: TIGHT_PTQVIM_MAX,
    wide_ptqvim_min: WIDE_PTQVIM_MIN,
    band_thresholds: {
...
function labelForPtqvim(
  ...
  if (ptqvim === null) return "degenerate";
  if (ptqvim >= wide_min) return "wide";
  if (ptqvim < tight_max) return "tight";
  return "spread";
}
...
function renderPtqvimCell(
  ...
  const ptqvimText = ptqvim === null ? "-" : ptqvim.toFixed(4);
  return `PTQVIM ${ptqvimText} / pool ${pool_count} across ${pool_cells} cells (${label})`;
}

export function formatDigestSnapshotPerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanSection(
  snapshot: DigestSnapshotPerTransitionMagnitudeTop3PoolPeakToQuattuorviginticMean,
): string {
  ...
  const { tight_ptqvim_max, wide_ptqvim_min } = snapshot;
  ...
  const rowsHtml = TRANSITION_KEYS.flatMap((t) => {
    ...
    return BAND_KEYS.filter((b) => {
      ...
      const band = entry.bands[b];
      return `<tr><td style="padding:4px 8px;border-bottom:1px solid #eee;vertical-align:top">${transitionLabel(t)}</td><td style="padding:4px 8px;border-bottom:1px solid #eee;vertical-align:top">${bandRangeLabel(b, small_max, medium_max)}</td><td style="padding:4px 8px;border-bottom:1px solid #eee;vertical-align:top">${renderPtqvimCell(band.partner_pool_count, band.partner_pool_cells, band.partner_peak_to_quattuorvigintic_mean, tight_ptqvim_max, wide_ptqvim_min)}</td><td style="padding:4px 8px;border-bottom:1px solid #eee;vertical-align:top">${renderPtqvimCell(band.metric_pool_count, band.metric_pool_cells, band.metric_peak_to_quattuorvigintic_mean, tight_ptqvim_max, wide_ptqvim_min)}</td></tr>`;
    });
  }).join("");

  return `
    <h3 style="margin-top:16px;font-family:Arial,sans-serif;font-size:14px">Per-transition magnitude TOP-${snapshot.top_n} pool PEAK-TO-QUATTUORVIGINTIC-MEAN across the ${snapshot.window_size}-week window (${firstWeek} &rarr; ${lastWeek}) at the ${thresholdPct}% magnitude threshold, sustained bar p90 &ge; ${snapshot.sustained_p90_threshold}</h3>
    <p style="font-family:Arial,sans-serif;font-size:13px">WHOLE-POOL RANGE-AGAINST-QUATTUORVIGINTIC-CENTER scalar over the P11.161 pool &mdash; ptqvim = (max - min) / quattuorvigintic_mean where quattuorvigintic_mean = ((sum x_i^24) / n)^(1/24). Reads the pool's total RANGE in units of its QUATTUORVIGINTIC (power-mean-of-order-24, M_24) centre so a LARGE-VALUE-DOMINATED pool reads TIGHTER here than under P11.300 PTRVM because raising the large values to the TWENTY-FOURTH power before averaging lifts the anchor MORE than raising to the twenty-third does, dampening the ratio against the range even harder. Unique DISPERSION-axis contribution: reads range in units of the QUATTUORVIGINTIC (POWER-MEAN-OF-ORDER-24) CENTER &mdash; extends the (harmonic M_-1, geometric M_0, arithmetic M_1, quadratic M_2, cubic M_3, quartic M_4, quintic M_5, sextic M_6, septic M_7, octic M_8, nonic M_9, decic M_10, undecic M_11, duodecic M_12, tredecic M_13, quattuordecic M_14, quindecic M_15, sedecic M_16, septendecic M_17, octodecic M_18, vigintic M_20, unvigintic M_21, duovigintic M_22, tresvigintic M_23) power-mean vigesimoquintet into a VIGESIMOSEXTET with the M_24 quattuorvigintic mean. Asymptotic ceiling: for a pool of size n with a dominant outlier x_max &gt;&gt; x_min, ptqvim approaches n^(1/24) so 10-partner pools cap near 1.1007 and only pools with pool_count &gt;= 11 escape into wide with a modest outlier (11^(1/24) ~= 1.1051 is past the wide floor). Composite regime labels: PTQVIM tight + PTRVM tight + PTDVIM tight + PTUVM tight + PTVIM tight + PTSOM tight + PTSPM tight + PTSDM tight + PTRMS spread = MILD OUTLIER absorbed by quattuorvigintic ([1x9, 10] reads PTQVIM 0.9906 tight); PTQVIM spread + PTRVM spread + PTDVIM spread = EXTREME OUTLIER only partially absorbed ([1x9, 100] reads PTQVIM 1.0897 spread); PTQVIM wide = RUNAWAY OUTLIER with a large pool ([1x99, 100] reads 1.1994 wide); PTQVIM spread + PTRVM spread + PTDVIM spread + PTSOM tight = ISOLATED HIGH PARTNER ([1, 100] reads 1.0190 spread — the 1.005 tight/spread boundary at M_24 keeps this two-partner regime spread that P11.294 PTVIM first caught at 1.01). Labels: solo = pool_count == 1 (structural single partner), degenerate = pool_cells == 0 OR quattuorvigintic_mean == 0 (guarded but unreachable), tight = ptqvim &lt; ${tight_ptqvim_max} (flat, uniform ramp, upper-outlier, two-shoulders, bimodal-split, two-partner, small regimes), spread = ptqvim in [${tight_ptqvim_max}, ${wide_ptqvim_min}) (extreme-outlier regime), wide = ptqvim &ge; ${wide_ptqvim_min} (runaway-outlier regime with pool_count &gt;= 11). Bands: small (1..${small_max}), medium (${small_max + 1}..${medium_max}), large (${medium_max + 1}+). Rotated + undecidable ship empty pools in medium + large bands by P11.129 design. Empty cells omitted from this table but stay on the JSONL envelope with pool_count 0 + ptqvim null.</p>
