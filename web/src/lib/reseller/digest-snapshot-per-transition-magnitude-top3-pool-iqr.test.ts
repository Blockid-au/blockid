...
import {
  ...
  TOP_N,
} from "./digest-snapshot-per-transition-magnitude-top3-pool-iqr";
...
describe("computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr — envelope passthrough", () => {
  it("carries scalars verbatim from source envelope", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr(
  ...
  it("exposes tight_iqr_max=0.5 + wide_iqr_min=2 + min_pool_count_for_iqr=4 on envelope", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr(
      envelope([]),
    );
    expect(out.tight_iqr_max).toBe(0.5);
    expect(out.wide_iqr_min).toBe(2);
    expect(out.min_pool_count_for_iqr).toBe(4);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr — envelope shape stability", () => {
  it("empty envelope emits pool_count 0 + null q1/q3/iqr in every cell", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr(
      envelope([]),
    );
    for (const t of [
      ...
      for (const b of ["small", "medium", "large"] as const) {
        ...
        expect(band.partner_iqr).toBeNull();
        expect(band.metric_pool_count).toBe(0);
        expect(band.metric_iqr).toBeNull();
      }
    }
...
describe("computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr — small_pool structural null (pool_count < 4)", () => {
  it("solo cell (1 partner) → iqr null (small_pool — quartiles undefined for n<4)", () => {
    const out = computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr(
      envelope([cell("ACME", "attributed_mrr", "improved", 2)]),
    );
  ...
  it("two-cell pool [3,1] → iqr null (small_pool — would collapse to P11.181 range)", () => {
  ...
  it("three-cell pool [3,2,1] → iqr null (small_pool — n=3 Tukey excludes middle → range clone)", () => {
    const rows: PerPairHotCellRow[] = [
...
describe("computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr — arithmetic (pool_count >= 4)", () => {
  it("flat 4-partner pool [1,1,1,1] → q1 1 / q3 1 / iqr 0 (tight)", () => {
    const rows: PerPairHotCellRow[] = [
      cell("A", "attributed_mrr", "improved", 4),
      cell("B", "attributed_mrr", "improved", 4),
      cell("C", "attributed_mrr", "improved", 4),
    ...
    expect(band.partner_iqr).toBe(0);
    expect(band.partner_iqr!).toBeLessThan(out.tight_iqr_max);
  });

  it("even 4-partner pool [4,3,2,1] → q1 1.5 / q3 3.5 / iqr 2 (wide floor)", () => {
    const rows: PerPairHotCellRow[] = [];
    ...
    // Sorted [1,2,3,4]. Lower half [1,2] → Q1 = 1.5. Upper half [3,4] → Q3 = 3.5. iqr = 2.
    expect(band.partner_q1_cells).toBe(1.5);
    expect(band.partner_q3_cells).toBe(3.5);
    expect(band.partner_iqr).toBe(2);
    expect(band.partner_iqr!).toBeGreaterThanOrEqual(out.wide_iqr_min);
  });

  it("odd 5-partner pool [5,4,3,2,1] → Tukey exclude middle 3 → q1 1.5 / q3 4.5 / iqr 3 (wide)", () => {
    const rows: PerPairHotCellRow[] = [];
    const kpiCatalog: KnownKpiSection[] = [
      "attributed_mrr",
      "commission_cleared_mtd",
    ...
    // Sorted [1,2,3,4,5]. Exclude middle 3. Lower [1,2] → Q1 = 1.5. Upper [4,5] → Q3 = 4.5. iqr = 3.
    expect(band.partner_q1_cells).toBe(1.5);
    expect(band.partner_q3_cells).toBe(4.5);
    expect(band.partner_iqr).toBe(3);
  });

  it("6-partner pool [6,5,4,3,2,1] → q1 2 / q3 5 / iqr 3 (wide)", () => {
    const rows: PerPairHotCellRow[] = [];
    const kpiCatalog: KnownKpiSection[] = [
      "attributed_mrr",
      "commission_cleared_mtd",
    ...
    // Sorted [1,2,3,4,5,6]. Lower half [1,2,3] → Q1 = 2. Upper half [4,5,6] → Q3 = 5. iqr = 3.
    expect(band.partner_q1_cells).toBe(2);
    expect(band.partner_q3_cells).toBe(5);
    expect(band.partner_iqr).toBe(3);
  });

  it("single-outlier robust: pool [10,1,1,1,1,1] (n=6) → q1 1 / q3 1 / iqr 0 (tight — outlier tucked into upper half's max, IQR ignores it)", () => {
    const rows: PerPairHotCellRow[] = [];
    ...
    expect(band.partner_pool_count).toBe(6);
    expect(band.partner_pool_cells).toBe(15);
    // Sorted [1,1,1,1,1,10]. Lower half [1,1,1] → Q1 = 1. Upper half [1,1,10] → Q3 = 1. iqr = 0.
    expect(band.partner_q1_cells).toBe(1);
    expect(band.partner_q3_cells).toBe(1);
    expect(band.partner_iqr).toBe(0);
    expect(band.partner_iqr!).toBeLessThan(out.tight_iqr_max);
  ...
  it("two-outlier crossover: pool [10,10,1,1,1,1] (n=6) → q1 1 / q3 10 / iqr 9 (wide — two outliers cross the hinge boundary)", () => {
    const rows: PerPairHotCellRow[] = [];
    ...
    // Sorted [1,1,1,1,10,10]. Lower half [1,1,1] → Q1 = 1. Upper half [1,10,10] → Q3 = 10. iqr = 9.
    expect(band.partner_q1_cells).toBe(1);
    expect(band.partner_q3_cells).toBe(10);
    expect(band.partner_iqr).toBe(9);
    expect(band.partner_iqr!).toBeGreaterThanOrEqual(out.wide_iqr_min);
  });

  it("even 4-partner pool [10,1,1,1] → q1 1 / q3 5.5 / iqr 4.5 (wide — outlier pulled into upper-half hinge on n=4)", () => {
    const rows: PerPairHotCellRow[] = [];
    const aKpis: KnownKpiSection[] = [
      "commission_cleared_mtd",
      "attributed_net_contribution",
    ...
    // Sorted [1,1,1,10]. Lower half [1,1] → Q1 = 1. Upper half [1,10] → Q3 = 5.5. iqr = 4.5.
    expect(band.partner_q1_cells).toBe(1);
    expect(band.partner_q3_cells).toBe(5.5);
    expect(band.partner_iqr).toBe(4.5);
    expect(band.partner_iqr!).toBeGreaterThanOrEqual(out.wide_iqr_min);
  });
});

describe("computeDigestSnapshotPerTransitionMagnitudeTop3PoolIqr — band + transition bucketing", () => {
...
describe("formatDigestSnapshotPerTransitionMagnitudeTop3PoolIqrSection", () => {
  it("returns empty string when window_size < 3 (P11.139 guard)", () => {
    ...
    expect(
      formatDigestSnapshotPerTransitionMagnitudeTop3PoolIqrSection(out),
    ).toBe("");
  });
  ...
  it("renders INTERQUARTILE RANGE heading + iqr cells + Q1/Q3 labels for populated bands", () => {
    const rows: PerPairHotCellRow[] = [];
    const kpiCatalog: KnownKpiSection[] = [
      "attributed_mrr",
      "commission_cleared_mtd",
    ...
    const html =
      formatDigestSnapshotPerTransitionMagnitudeTop3PoolIqrSection(out);
    expect(html).toContain("INTERQUARTILE RANGE");
    expect(html).toContain("iqr");
    expect(html).toContain("Q1");
    expect(html).toContain("Q3");
    expect(html).toContain("wide");
    expect(html).toContain("partner iqr");
    expect(html).toContain("KPI iqr");
  });
