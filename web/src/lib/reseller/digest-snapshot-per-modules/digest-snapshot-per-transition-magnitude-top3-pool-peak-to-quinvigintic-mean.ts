...
// pure-lib (P11.302).
//
// WHOLE-POOL RANGE-AGAINST-QUINVIGINTIC-CENTER dispersion scalar
// over the P11.161 pool. Folds every cell into ONE dispersion read
// that reports the pool's total RANGE (max - min) in units of the
// pool's QUATTUORVIGINTIC MEAN (a.k.a. power mean of order 24, M_24):
//
//   ptqivm = (max - min) / quinvigintic_mean
//
// where quinvigintic_mean = ((sum x_i^25) / n)^(1/25). Reads the
...
// under P11.302 PTQVIM, because raising to the TWENTY-FIFTH power
...
//
// PTQIVM's unique DISPERSION-axis contribution: reads range in units
// of the QUINVIGINTIC (POWER-MEAN-OF-ORDER-25) CENTER. Every
...
// quinvigintic_mean; equality iff all values equal). PTQIVM's
...
// mean centre-anchor VIGESIMOSEXTET into a VIGESIMOSEPTET (harmonic,
...
// unvigintic, duovigintic, tresvigintic, quattuorvigintic, quinvigintic), and lets
...
//                                     the quinvigintic mean cannot
//                                     absorb fully; range still lifts
//                                     PTQIVM into spread. Reference:
//                                     [1x9, 100] reads PTQIVM 1.0855
//                                     spread, PTRVM 1.0942 spread,
//                                     PTDVIM 1.0992 spread.
//   * PTQIVM wide + PTRVM wide + PTDVIM wide
//                                     -> RUNAWAY OUTLIER with a
//                                     large pool: raising to the 25th
//                                     power STILL leaves range
//                                     dominant over the anchor
//                                     because the outlier ratio
//                                     exceeds n^(1/25). Reference:
//                                     [1x99, 100] reads PTQIVM 1.1902
//                                     wide.
//   * PTQIVM spread + PTRVM spread + PTDVIM spread + PTSOM tight
//                                     -> ISOLATED HIGH PARTNER (two-
//                                     partner pool). The 1.005 tight/
//                                     spread boundary at M_25 continues
//                                     to catch this regime that P11.294
//                                     PTVIM first caught at 1.01. Ref:
//                                     [1, 100] reads PTQIVM 1.01780
//                                     spread, PTRVM 1.0203 spread,
//                                     PTDVIM 1.0217 spread.
//   * PTQIVM wide + PTRVM tight       -> unreachable because
//                                     quinvigintic_mean is ALWAYS
//                                     >= tresvigintic_mean by Power
//                                     Mean inequality (M_25 >= M_24),
//                                     so ptqivm = range/quinvigintic_mean
//                                     <= ptrvm = range/tresvigintic_mean
//                                     by construction. Guarded on the
//                                     reference distributions below
//                                     as a documented invariant.
//
// PTQIVM's asymptotic ceiling: for a pool of size n with a single
// dominant outlier x_max and every other cell at x_min << x_max,
// quinvigintic_mean approaches x_max / n^(1/25), so ptqivm
// approaches (x_max - x_min) / (x_max / n^(1/25)) = n^(1/25) *
// (1 - x_min/x_max) -> n^(1/25) as x_max -> +Inf. For n=10 the
// ceiling is 10^(1/25) ~= 1.0965, so even the most extreme outlier
// in a 10-partner pool reads ptqivm just past 1.10 (spread but
// barely above 1.10). For n=100 the ceiling climbs to 100^(1/25)
// ~= 1.2023, so a large pool with a dominant outlier reads wide.
// Pools with pool_count >= 11 escape into wide (since 11^(1/25)
// ~= 1.1007 > wide_min = 1.09 so pool_count >= 11 pools can reach
...
//                                     robustness).
//   * pool_count >= 2 and           -> ptqivm null (quinvigintic_mean_zero
//     quinvigintic_mean == 0        -- unreachable since quinvigintic_mean
...
//     quinvigintic_mean > 0         4 decimals. Zero iff max == min
//                                     (flat pool). Non-negative by
//                                     construction because range >= 0
//                                     and quinvigintic_mean > 0.
//
// Reference distributions (pool_count 10 unless noted):
//   * flat [k,k,...,k]              -> QIVM = k, range 0, ptqivm 0
//                                     (tight).
//   * uniform ramp [1..10]          -> QIVM ~= 9.1468, range 9, ptqivm
//                                     ~= 0.9840 (tight -- well under
//                                     the 1.005 tight/spread boundary).
//   * upper-outlier [1x9, 10]       -> QIVM ~= 9.1201, range 9, ptqivm
//                                     ~= 0.9868 (tight -- MILD-SINGLE-
//                                     OUTLIER absorbed by the
//                                     quinvigintic mean where P11.252
//                                     PTRMS reads spread + P11.250
//                                     PTH + P11.248 PTGM read wide;
//                                     even softer than P11.302 PTQVIM's
//                                     0.9948 tight landing; drops
//                                     FURTHER BELOW the arithmetic-max
//                                     dampening threshold of 1.0).
//   * two-shoulders [1x8, 5x2]      -> QIVM ~= 4.6883, range 4, ptqivm
//                                     ~= 0.8532 (tight).
//   * 50/50 split [1x5, 10x5]       -> QIVM ~= 9.7265, range 9, ptqivm
//                                     ~= 0.9253 (tight -- BIMODAL
//                                     SPLIT well-absorbed).
//   * extreme outlier [1x9, 100]    -> QIVM ~= 91.2011, range 99,
//                                     ptqivm ~= 1.0855 (SPREAD --
//                                     EXTREME OUTLIER approaches
//                                     n^(1/25) ~ 1.0965 asymptote and
//                                     lands well above the tight
//                                     boundary but under the wide
//                                     cutoff).
//   * two-partner [1, 9]            -> QIVM ~= 8.7539, range 8, ptqivm
//                                     ~= 0.9139 (tight).
//   * two-partner [1, 100]          -> QIVM ~= 97.2655, range 99,
//                                     ptqivm ~= 1.0178 (SPREAD --
//                                     ISOLATED HIGH PARTNER remains
//                                     above the 1.005 tight/spread
//                                     boundary at M_25; the boundary
//                                     tightening from P11.294 through
//                                     P11.300 keeps this two-partner
//                                     regime in spread).
//   * small [10, 1, 1]              -> QIVM ~= 9.5701, range 9, ptqivm
//                                     ~= 0.9404 (TIGHT --
//                                     SMALL-VALUE-DOMINATED with
//                                     LARGE-PARTNER DAMPENING; PTQIVM
//                                     approaches the 3-partner
//                                     asymptote 3^(1/25) ~= 1.0449 as
//                                     the outlier grows).
//   * pool_count=100 [1x99, 100]    -> QIVM ~= 83.1764, range 99,
//                                     ptqivm ~= 1.1902 (WIDE --
...
// (HIGH ptqivm = MORE range against quinvigintic centre = MORE
...
// P11.298 PTDVIM + P11.300 PTRVM + P11.302 PTQVIM tight/spread/wide vocabulary).
...
// P11.145.
//
// Splice placement rule for a follow-up cron-wiring tick (P11.305):
// IMMEDIATELY BELOW perTransitionMagnitudeTop3PoolPeakToQuattuorviginticMeanSection
// (P11.302) AND IMMEDIATELY ABOVE perPairHotCellsSection so the
// DISPERSION axis continues with range-against-quinvigintic-center
...
// Bands on raw ptqivm (fixed cutoffs since quinvigintic_mean scales
// with cell counts and typical quinvigintic-center emissions land
...
// P11.302 PTQVIM's 1.005 -- MILD-OUTLIER at M_25 is 0.9868 (already
...
// regime ([1, 100] reads 1.0178 spread). Wide boundary tightens
// P11.302 PTQVIM's 1.10 down to 1.09 in a full 0.01 step matching
// the M_22 -> M_23 (1.11 -> 1.10) step so only pool_count >= 11
// pools reach wide with a modest outlier (11^(1/25) ~= 1.1007 is
// past the wide floor while 10^(1/25) ~= 1.0965 stays only marginally
...
// Peak-to-quinvigintic-mean of a discrete distribution:
//   PTQIVM = (max - min) / quinvigintic_mean
// where quinvigintic_mean = ((sum x_i^25) / n)^(1/25). Returns null
// on empty, solo, and degenerate (zero quinvigintic_mean or non-
// finite twenty-fifth-power sum) so downstream labels fire from
...
function foldMap(cellsByKey: Map<string, number>): {
  ...
  if (pool_count === 1) {
    // Solo -- range = 0 and QIVM = the sole cell. The ratio would
  ...
  let twentyfifthSum = 0;
  for (const v of values) {
    const sq = v * v;
    const quad = sq * sq;
    const oct = quad * quad;
    // x^25 = x^8 * x^8 * x^8 * x
    twentyfifthSum += oct * oct * oct * v;
  }
  if (!Number.isFinite(twentyfifthSum) || twentyfifthSum <= 0) {
    // Belt-and-braces: sum of twenty-fifth-power non-negative counts
  ...
  const quinvigintic_mean = Math.pow(twentyfifthSum / pool_count, 1 / 25);
  if (!Number.isFinite(quinvigintic_mean) || quinvigintic_mean <= 0) {
    return { pool_count, pool_cells, peak_to_quinvigintic_mean: null };
  }
  const range = max - min;
  const ptqivm = range / quinvigintic_mean;
  // Clamp tiny negative float-noise to 0; ptqivm is non-negative by
  // construction because range >= 0 and quinvigintic_mean > 0.
...
export function formatDigestSnapshotPerTransitionMagnitudeTop3PoolPeakToQuinviginticMeanSection(
  ...
  return `
    ...
    <p style="font-family:Arial,sans-serif;font-size:13px">WHOLE-POOL RANGE-AGAINST-QUINVIGINTIC-CENTER scalar over the P11.161 pool &mdash; ptqivm = (max - min) / quinvigintic_mean where quinvigintic_mean = ((sum x_i^25) / n)^(1/25). Reads the pool's total RANGE in units of its QUINVIGINTIC (power-mean-of-order-25, M_25) centre so a LARGE-VALUE-DOMINATED pool reads TIGHTER here than under P11.302 PTQVIM because raising the large values to the TWENTY-FIFTH power before averaging lifts the anchor MORE than raising to the twenty-fourth does, dampening the ratio against the range even harder. Unique DISPERSION-axis contribution: reads range in units of the QUINVIGINTIC (POWER-MEAN-OF-ORDER-25) CENTER &mdash; extends the (harmonic M_-1, geometric M_0, arithmetic M_1, quadratic M_2, cubic M_3, quartic M_4, quintic M_5, sextic M_6, septic M_7, octic M_8, nonic M_9, decic M_10, undecic M_11, duodecic M_12, tredecic M_13, quattuordecic M_14, quindecic M_15, sedecic M_16, septendecic M_17, octodecic M_18, vigintic M_20, unvigintic M_21, duovigintic M_22, tresvigintic M_23, quattuorvigintic M_24) power-mean vigesimosextet into a VIGESIMOSEPTET with the M_25 quinvigintic mean. Asymptotic ceiling: for a pool of size n with a dominant outlier x_max &gt;&gt; x_min, ptqivm approaches n^(1/25) so 10-partner pools cap near 1.0965 and only pools with pool_count &gt;= 11 escape into wide with a modest outlier (11^(1/25) ~= 1.1007 is past the wide floor). Composite regime labels: PTQIVM tight + PTRVM tight + PTDVIM tight + PTUVM tight + PTVIM tight + PTSOM tight + PTSPM tight + PTSDM tight + PTRMS spread = MILD OUTLIER absorbed by quinvigintic ([1x9, 10] reads PTQIVM 0.9868 tight); PTQIVM spread + PTRVM spread + PTDVIM spread = EXTREME OUTLIER only partially absorbed ([1x9, 100] reads PTQIVM 1.0855 spread); PTQIVM wide = RUNAWAY OUTLIER with a large pool ([1x99, 100] reads 1.1902 wide); PTQIVM spread + PTRVM spread + PTDVIM spread + PTSOM tight = ISOLATED HIGH PARTNER ([1, 100] reads 1.0178 spread — the 1.005 tight/spread boundary at M_25 keeps this two-partner regime spread that P11.294 PTVIM first caught at 1.01). Labels: solo = pool_count == 1 (structural single partner), degenerate = pool_cells == 0 OR quinvigintic_mean == 0 (guarded but unreachable), tight = ptqivm &lt; ${tight_ptqivm_max} (flat, uniform ramp, upper-outlier, two-shoulders, bimodal-split, two-partner, small regimes), spread = ptqivm in [${tight_ptqivm_max}, ${wide_ptqivm_min}) (extreme-outlier regime), wide = ptqivm &ge; ${wide_ptqivm_min} (runaway-outlier regime with pool_count &gt;= 11). Bands: small (1..${small_max}), medium (${small_max + 1}..${medium_max}), large (${medium_max + 1}+). Rotated + undecidable ship empty pools in medium + large bands by P11.129 design. Empty cells omitted from this table but stay on the JSONL envelope with pool_count 0 + ptqivm null.</p>
    <table style="font-family:Arial,sans-serif;font-size:13px;border-collapse:collapse;margin-top:4px">
      <thead>
        <tr><th style="padding:4px 8px;text-align:left;border-bottom:2px solid #333">transition</th><th style="padding:4px 8px;text-align:left;border-bottom:2px solid #333">magnitude band</th><th style="padding:4px 8px;text-align:left;border-bottom:2px solid #333">partner PTQIVM</th><th style="padding:4px 8px;text-align:left;border-bottom:2px solid #333">KPI PTQIVM</th></tr>
      </thead>
      <tbody>${rowsHtml}
      </tbody>
    </table>`;
