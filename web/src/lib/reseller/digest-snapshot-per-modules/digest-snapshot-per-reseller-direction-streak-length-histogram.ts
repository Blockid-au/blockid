// Weekly digest per-reseller sustained-direction streak length-frequency histogram (P11.81).
//
// The direction-streak family already ships six surfaces at increasing grain:
...
//
// The histogram axis so far only exists at the portfolio grain (P11.77). Ops
// still asks the reciprocal per-partner question when scanning P11.32: "for
// THIS partner, what's the SHAPE of their sustained runs — mostly length-2
// blips clearing the threshold, or a fat tail of length-5+ persistence?" A
// per-partner coverage rate (P11.59) or a per-partner top-N leaderboard
// (P11.73) can't answer that: coverage collapses shape into a single ratio
// and the leaderboard caps at top-N so the distribution beyond the top-N
// entries is invisible. Two partners with identical coverage% and identical
...
// exposes that.
//
// This module lands the per-partner analogue of P11.77: for each partner with
// at least one qualifying streak, emit an independent length-frequency
// histogram over that partner's own streak rows. Bucket set is DENSE from
// min_streak_length to max(row.length) FOR THAT PARTNER so the axis reads
// continuously — a partner whose deepest run is length 4 does not carry a
// phantom length-5 bucket sourced from a different partner's tail. Shape is
// the direct-analogue of P11.73/P11.74 leaderboard grouping: groups sorted by
// reseller_code asc, one histogram per partner group, only partners with at
// least one qualifying row emit a group. This closes the histogram family's
// per-partner axis symmetric with P11.59 (coverage) and P11.73 (leaderboard).
//
...
// (P11.82) so this shape can be exercised in isolation before touching the
...
//     the P11.73 leaderboard's delegation posture.
...
//     collapsed against neighbouring bars — matches P11.77's dense-bucket
//     contract.
...
//     right shortest → longest — matches P11.77's x-axis convention).
//   • Group ordering: reseller_code asc (deterministic + alphabetical — matches
//     P11.59 / P11.73 posture; partners have no canonical spec order the way
//     HEADLINE_METRICS does for KPIs).
//   • Formatter returns "" when window_size < 3 (a 2-week window cannot host
//     a length-2 streak — matches P11.30/P11.32 posture) OR when zero groups
//     qualify (matches P11.73's quiet-when-flat posture at the per-partner
...
export interface PerResellerDirectionStreakLengthBucket {
  readonly length: number;
  readonly count: number;
  readonly pct: number;
}

export interface PerResellerDirectionStreakLengthHistogramGroup {
  readonly reseller_code: string;
  readonly total_streaks: number;
  readonly max_length: number;
  readonly buckets: readonly PerResellerDirectionStreakLengthBucket[];
}

export interface DigestSnapshotPerResellerDirectionStreakLengthHistogram {
  readonly window_size: number;
  readonly first_week: string | null;
  readonly last_week: string | null;
  readonly min_streak_length: number;
  readonly groups: readonly PerResellerDirectionStreakLengthHistogramGroup[];
}
 ...
 * length-frequency histogram. For each partner with at least one qualifying
 * streak, emits an independent histogram over that partner's own streak
 * rows with a dense bucket set from min_streak_length to max(row.length) FOR
 * THAT PARTNER — sibling partners' deeper tails do not pad this partner's
 * axis. Reuses computeDigestSnapshotPerResellerDirectionStreaks so histogram
 * groups cannot diverge from the P11.32 spotlight rows they summarise.
 *
 ...
 */
export function computeDigestSnapshotPerResellerDirectionStreakLengthHistogram(
  ...
  const groups: PerResellerDirectionStreakLengthHistogramGroup[] = [];
  for (const code of sortedCodes) {
    ...
    const buckets: PerResellerDirectionStreakLengthBucket[] = [];
...
/**
 * Render a per-partner length-frequency histogram HTML section. One
 * independent histogram table per partner, sorted reseller_code asc. Each
 * table's Distribution column is a plain-text 20-cell unicode-block gauge
 * (█ filled + ░ empty) proportional to bucket.count / max(bucket.count)
 * WITHIN THAT PARTNER — the gauge scales per-partner so a small partner's
 * tail is not visually crushed by a dominant partner's peak bucket.
 *
 * In the P11.82 cron wiring this lands directly BELOW the P11.73/P11.74
 * per-partner leaderboard and ABOVE the P11.32 per-(metric × partner)
 * spotlight so ops reads the partner topline (coverage P11.59) → per-partner
 * leaderboard top-N (P11.73) → per-partner shape-of-persistence tail (this
 * module) → per-partner spotlight detail (P11.32). This closes the histogram
 * family's per-partner axis symmetric with the leaderboard family.
 *
 * Returns "" when window_size < 3 (a 2-week window cannot host a length-2
 * streak — matches P11.32/P11.77 posture) OR when zero groups qualify.
 */
export function formatDigestSnapshotPerResellerDirectionStreakLengthHistogramSection(
  histogram: DigestSnapshotPerResellerDirectionStreakLengthHistogram,
): string {
  ...
  const groupsHtml = histogram.groups
    .map((group) => {
      ...
      const caption = `${escapeHtml(group.reseller_code)} &mdash; ${total} qualifying streak${total === 1 ? "" : "s"}, tail runs to length ${group.max_length}`;
      return `
  ...
  return `
    <h3 style="margin-top:24px;font-family:Arial,sans-serif;font-size:14px">Per-partner sustained-direction streak length distributions across the ${histogram.window_size}-week window (${firstWeek} &rarr; ${lastWeek})</h3>
    <p style="font-family:Arial,sans-serif;font-size:13px">One length-frequency histogram per partner so ops can answer "what's the SHAPE of THIS partner's persistence" at a glance &mdash; two partners with identical P11.59 coverage% and identical P11.73 #1 leaderboard entries can still differ sharply in tail (one clustering at length ${histogram.min_streak_length} while another sits on a fat tail) and neither of those surfaces exposes that. Buckets within a partner group are dense from length ${histogram.min_streak_length} to that partner's own max_length so zero-count bands between populated bands render explicitly. The Distribution gauge scales per-partner so a small partner's tail is not visually crushed by a dominant partner's peak.</p>${groupsHtml}`;
}
