// Weekly digest per-partner persistence scorecard verdict TRANSITION
// DELTA-RANK DISTRIBUTION scalar-summary (P11.127).
//
// The P11.117 / P11.118 per-partner verdict-transition pair opened the
// week-over-week regression-detection surface at the per-partner grain — every
// reseller_code now carries a single transition token (improved / degraded /
// rotated / undecidable / stable / first_classification) beneath the P11.111
// per-partner verdict badge each Monday. But ops reading the per-partner
// transition table still had to eyeball each row to answer the roster-level
// question 'across THIS week, how many partners improved by 2 ranks vs 1, and
// how many degraded?'. A two-partner roster answers itself; a
// twenty-partner roster does not.
//
// This module closes that per-grain aggregation gap by folding the P11.117
// per-partner transition envelope into a scalar-summary DISTRIBUTION over the
...
// single-line caption in the digest email without walking the per-partner
// transition table row-by-row.
//
// Pure derivation of the P11.117 envelope — no new folds, no scorecard
// replay, no new inputs. The distribution is the (input, aggregation)
// duality: the P11.117 module answered 'PER PARTNER, what changed?'; this
// module answers 'ACROSS PARTNERS, how much changed by how much?' — the
// natural scalar summary that lives beneath the row-per-partner table.
//
// Mirrors the P11.125 per-metric-grain distribution module exactly — same
// bucket set (first_classification / undecidable / stable / rotated /
// improved_by_1 / improved_by_2 / improved_by_other / degraded_by_1 /
// degraded_by_2 / degraded_by_other), same alert_worthy scalar, same
// net_delta_rank barometer, same formatter suppression trio, same fixed
// bullet order — so ops learn ONE distribution vocabulary and apply it at
// per-metric AND per-partner grains without switching mental models between
// grains.
//
// Pure-lib-first per the P11.14 → P11.15 / P11.20 → P11.21 / ... / P11.115 →
// P11.116 / P11.117 → P11.118 / P11.123 → P11.124 / P11.125 → P11.126
// cadence — cron-route wiring intentionally deferred to a follow-up tick
// (P11.128) so the distribution shape can be exercised in isolation before
// touching the hot Monday cron path (already hosting four grains of
// scorecard + four grains of verdict caption + four grains of transition
// caption + one grain of transition distribution caption).
//
// Design notes:
//   • Buckets are named for direct grep-ability: `improved_by_1`,
//     `improved_by_2`, `degraded_by_1`, `degraded_by_2` split by
//     |delta_rank| so ops distinguishes 'ACME clawed back from
//     sustained_direction_only → sustained_both_axes' (+1) from
//     'ZEBRA leapt from volatile → sustained_both_axes' (+2). The rank
//     table only spans [-1, 2] (see P11.117 VERDICT_RANK), so the maximum
//     absolute delta_rank is 2 and no further buckets are needed. Rows with
//     |delta_rank| outside this range are impossible from valid P11.117
...
//     zero-rank flips together — matches P11.117 posture that a rotation
...
//     but retained on the envelope so ops can spot 'the partner roster grew
//     from 8 → 11 this week' (first_classification=3) or 'the roster is
//     fully described by the per-partner verdict block above' (stable=all).
//   • `alert_worthy = improved + degraded + rotated + undecidable` — matches
//     the formatter suppression on the underlying P11.117 module (which
...
//     + first_classification) as 0 — a positive integer means the per-partner
...
//     the per-partner transitions above to see which specific partners drove
//     the shift.
...
//     a JSONL consumer joining the distribution to the per-partner transition
//     envelope on the same weekly row aligns by row index without extra keys.
//   • Formatter suppresses on window_size < 3 (matches P11.117 formatter
//     suppression) AND on total == 0 (empty envelope) AND on alert_worthy ==
//     0 (every row is first_classification or stable — the per-partner
...
import type { DigestSnapshotPerResellerPersistenceScorecardVerdictTransition } from "./digest-snapshot-per-reseller-persistence-scorecard-verdict-transition";

export interface PerResellerPersistenceScorecardVerdictTransitionDistribution {
...
export interface DigestSnapshotPerResellerPersistenceScorecardVerdictTransitionDistribution {
  readonly window_size: number;
  readonly first_week: string | null;
  readonly last_week: string | null;
  readonly sustained_p90_threshold: number;
  readonly threshold: number;
  readonly distribution: PerResellerPersistenceScorecardVerdictTransitionDistribution;
}

/**
 * Fold the P11.117 per-partner verdict-transition envelope into a scalar
 ...
 * output distribution is the aggregation dual of the input row-per-partner
 * envelope: 'ACROSS PARTNERS, how much changed by how much?' vs. the input's
 * 'PER PARTNER, what changed?'.
 *
 ...
 */
export function computeDigestSnapshotPerResellerPersistenceScorecardVerdictTransitionDistribution(
...
/**
 * Render the per-partner verdict-transition delta-rank distribution as a
 * single-line caption + inline bullet list of the non-zero buckets. Splices
 * directly BELOW the P11.118 per-partner verdict-transition table so ops
 * reads the row-per-partner transitions above and the collapsed scalar
 * summary inline below without re-summing rows in their head.
 *
 * Returns "" when window_size < 3 (matches the P11.117 / P11.118 formatter
 * suppression on the same short-window guard) OR when total == 0 (empty
 * envelope) OR when alert_worthy == 0 (every row resolves to
 * first_classification or stable — the per-partner transition table above
 ...
 */
export function formatDigestSnapshotPerResellerPersistenceScorecardVerdictTransitionDistributionSection(
  snapshot: DigestSnapshotPerResellerPersistenceScorecardVerdictTransitionDistribution,
): string {
  ...
  return `
    <h3 style="margin-top:16px;font-family:Arial,sans-serif;font-size:14px">Per-partner verdict-transition distribution across the ${snapshot.window_size}-week window (${firstWeek} &rarr; ${lastWeek}) at the ${thresholdPct}% magnitude threshold, sustained bar p90 &ge; ${snapshot.sustained_p90_threshold}</h3>
    <p style="font-family:Arial,sans-serif;font-size:13px">Scalar summary of the per-partner transition table above. Aggregates the row-per-partner transitions into bucket counts split by <code>|delta_rank|</code> so ops distinguishes a single-rank claw-back (<code>improved_by_1</code>) from a two-rank leap (<code>improved_by_2</code>) without walking the row grid. n=<strong>${d.total}</strong> partners classified this week &mdash; <strong>${d.alert_worthy}</strong> alert-worthy, <strong>${d.stable}</strong> stable, <strong>${d.first_classification}</strong> first-classification. Net &Delta;rank across the roster = <strong>${escapeHtml(formatSignedInt(d.net_delta_rank))}</strong> (${netMoveLabel}).</p>
