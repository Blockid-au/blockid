...
describe("erasure map ↔ live-schema fixture", () => {
  it("fixture is the 133-FK production dump (129 live + the three 0393 investor FKs + the 0405 program_intakes FK added by hand until applied)", () => {
    expect(fixture.referenced).toBe("public.app_users(id)");
    expect(fks.length).toBe(133);
    expect(fixture.count).toBe(fks.length);
    const by = fks.reduce<Record<string, number>>((acc, f) => ({ ...acc, [f.on_delete]: (acc[f.on_delete] ?? 0) + 1 }), {});
    expect(by).toEqual({ CASCADE: 88, "NO ACTION": 14, "SET NULL": 25, RESTRICT: 6 });
  });
  ...
  it("summary matches the classification", () => {
    ...
    expect(s.entries).toBe(133);
    expect(s.delete + s.anonymise + s.detach).toBe(133);
    expect(s).toMatchObject({ delete: 83, anonymise: 34, detach: 16, immutable: 3, tables: 119 });
...
describe("erasure migration ↔ map parity", () => {
  ...
  it("the SQL block parses back to the same 133 entries", () => {
    const parsed = parseMapBlock(extractBlock(sql, MAP_BEGIN, MAP_END)!);
    expect(parsed.length).toBe(133);
    const ord = orderedEntries();
    parsed.forEach((p, i) => {
      const e = ord[i];
