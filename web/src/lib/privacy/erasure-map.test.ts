...
describe("erasure map ↔ live-schema fixture", () => {
  it("fixture is the 147-FK production dump (129 live + the three 0393 investor FKs + the 0405 intake FK + the 0406 feedback-letter FK + the four 0417 claims/evidence_records/claim_versions FKs + the 0416 pilot_orders FK + the two 0422 intake_templates/cohort_snapshots FKs + the two 0418 corrections FKs + the four 0423 cohort overrides/members FKs)", () => {
    expect(fixture.referenced).toBe("public.app_users(id)");
    expect(fks.length).toBe(147);
    expect(fixture.count).toBe(fks.length);
    const by = fks.reduce<Record<string, number>>((acc, f) => ({ ...acc, [f.on_delete]: (acc[f.on_delete] ?? 0) + 1 }), {});
    expect(by).toEqual({ CASCADE: 92, "NO ACTION": 14, "SET NULL": 35, RESTRICT: 6 });
  });
  ...
  it("summary matches the classification", () => {
    ...
    expect(s.entries).toBe(147);
    expect(s.delete + s.anonymise + s.detach).toBe(147);
    expect(s).toMatchObject({ delete: 86, anonymise: 35, detach: 26, immutable: 3, tables: 130 });
...
describe("erasure migration ↔ map parity", () => {
  ...
  it("the SQL block parses back to the same 147 entries", () => {
    const parsed = parseMapBlock(extractBlock(sql, MAP_BEGIN, MAP_END)!);
    expect(parsed.length).toBe(147);
    const ord = orderedEntries();
    parsed.forEach((p, i) => {
      const e = ord[i];
