...
describe("erasure map ↔ live-schema fixture", () => {
  it("fixture is the 141-FK production dump (129 live + the three 0393 investor FKs + the 0405 intake FK + the 0406 feedback-letter FK + the four 0417 claims/evidence_records/claim_versions FKs + the 0416 pilot_orders FK + the two 0418 corrections FKs)", () => {
    ...
    expect(by).toEqual({ CASCADE: 90, "NO ACTION": 14, "SET NULL": 31, RESTRICT: 6 });
  });
  ...
  it("summary matches the classification", () => {
    ...
    expect(s).toMatchObject({ delete: 84, anonymise: 35, detach: 22, immutable: 3, tables: 125 });
