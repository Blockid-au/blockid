...
describe("resolveBatchAccess", () => {
  const BATCH_ROW = { id: "b-1", user_id: "owner", name: "Cohort 5", rubric_weights: null, status: "done", total: 3, done_count: 3, failed_count: 0, created_at: "2026-09-01", started_at: null, finished_at: null };

  it("owner via evaluation_batches.user_id", async () => {
    getBatchForUserMock.mockResolvedValue({ id: "b-1", userId: "owner", name: "Cohort 5" });
    const r = await resolveBatchAccess("owner", "b-1", fakeDb({}));
    expect(r?.role).toBe("owner");
    expect(r?.batch.name).toBe("Cohort 5");
  });

  it("reviewer via P2-B evaluation_batch_members; unknown role / missing table → null", async () => {
    getBatchForUserMock.mockResolvedValue(null);
    const reviewer = await resolveBatchAccess("rev", "b-1", fakeDb({ evaluation_batch_members: { data: { role: "reviewer" } }, evaluation_batches: { data: BATCH_ROW } }));
    expect(reviewer?.role).toBe("reviewer");
    expect(reviewer?.batch).toMatchObject({ id: "b-1", userId: "owner", status: "done", total: 3 });
    expect(await resolveBatchAccess("rev", "b-1", fakeDb({ evaluation_batch_members: { data: { role: "janitor" } }, evaluation_batches: { data: BATCH_ROW } }))).toBeNull();
    expect(await resolveBatchAccess("rev", "b-1", fakeDb({}))).toBeNull();
    expect(await resolveBatchAccess("rev", "b-1", null)).toBeNull();
  });
});
