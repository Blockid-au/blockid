 ...
 */
export async function resolveBatchAccess(userId: string, batchId: string, client?: DbLike | null): Promise<{ batch: EvaluationBatch; role: BatchRole } | null> {
  const own = await getBatchForUser(userId, batchId);
  if (own) return { batch: own, role: "owner" };
  const sb = db(client);
  if (!sb) return null;
  try {
    const { data, error } = await sb.from("evaluation_batch_members").select("role").eq("batch_id", batchId).eq("user_id", userId).maybeSingle();
    if (error || !data) return null;
    const role = String((data as Row).role ?? "");
    if (role !== "reviewer" && role !== "viewer" && role !== "owner") return null;
    const { data: b, error: bErr } = await sb.from("evaluation_batches").select("id, user_id, name, rubric_weights, status, total, done_count, failed_count, created_at, started_at, finished_at").eq("id", batchId).maybeSingle();
    if (bErr || !b) return null;
    return { batch: mapBatchRow(b as Row), role: role as BatchRole };
  } catch {
    return null;
  }
}
