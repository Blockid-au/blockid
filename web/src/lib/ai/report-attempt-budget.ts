...
// G33-T06: `input` = certified input-token ceiling for the attempt (the request's
// UTF-8 byte count — a token is never shorter than one byte), absent on entries
// written before 24/09 (their ceiling was the full context window). `retryOf` =
// the settled attempt this one re-dispatches (same payload, new identity).
...
/** The entry's certified input ceiling: its byte bound, or the context window for pre-G33 entries. */
function inputCeiling(e: { model: string; input?: number }): number {
  const context = PRICES[e.model]?.context ?? 0;
  return typeof e.input === "number" ? Math.min(e.input, context) : context;
}
function validate(raw: string, scope: string): Ledger {
  ...
  for (const e of ledger.entries) {
    ...
    if (!PRICES[e.model] || e.maximum !== cost(e.model, inputCeiling(e), e.output) || e.held > e.maximum || (e.state !== "reported_usage" && e.held !== e.maximum)) return fail("ledger integrity mismatch");
  }
  ...
  return {
    ...
    async reserve(request: AttemptRequest) {
      ...
      return transaction(ledger => {
        ...
        // maximum (US$0.094 for one DeepSeek-V4-Flash attempt) and exhausted the
    ...
    async settle(outcome) {
      ...
      await transaction(ledger => {
        ...
        const valid = outcome.state === "reported_usage" && typeof input === "number" && Number.isSafeInteger(input) && input >= 0 && typeof output === "number" && Number.isSafeInteger(output) && output >= 0 && input + output > 0 && input + output <= PRICES[entry.model].context && input <= inputCeiling(entry) && output <= entry.output;
