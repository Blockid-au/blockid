...
const manifestSchema = z.object({
  ...
  entries: z.array(z.object({ modelId: id, endpointTag: tag, metadataSha256: sha, qualitySha256: sha, zeroCostContractSha256: sha.optional(),
    reviewedAt: timestamp, reviewer: z.string().min(1), task: z.literal("scoped_assessment") }).strict()).max(30),
}).strict();
...
const qualitySchema = z.object({
  suiteVersion: z.literal(FREE_REPORT_SUITE), modelId: id, endpointTag: tag, task: z.literal("scoped_assessment"),
...
export interface FreeFallbackCandidate {
  ...
  requestConstraints: { model: string; max_tokens: number; provider: {
    ...
    max_price: { prompt: 0; completion: 0; request: 0 }; data_collection: "deny"; zdr: true;
...
export async function loadQualifiedFreeFallbacks(manifest: unknown, readArtifact: (sha256: string) => Promise<string | null>, demand: FreeFallbackDemand): Promise<FreeFallbackQualification> {
  ...
  for (const entry of parsed.data.entries) {
    ...
    if (quality.modelId !== entry.modelId || quality.endpointTag !== entry.endpointTag || !fresh(quality.evaluatedAt, demand.now, 7 * 86400_000) || Date.parse(entry.reviewedAt) < Date.parse(quality.evaluatedAt)) { reject(entry.modelId, "quality_binding_or_freshness"); continue; }
    ...
    result.eligible.push({ modelId: entry.modelId, endpointTag: entry.endpointTag, metadataSha256: entry.metadataSha256, qualitySha256: entry.qualitySha256,
      executionAllowed: false, requiredNext: "atomic_account_quota_reservation_and_scoped_transport_integration",
      requestConstraints: { model: entry.modelId, max_tokens: demand.outputTokens, provider: { only: [entry.endpointTag], order: [entry.endpointTag], allow_fallbacks: false, require_parameters: true, max_price: { prompt: 0, completion: 0, request: 0 }, data_collection: "deny", zdr: true }, plugins: [], stream: false },
    });
  }
  return result;
}
