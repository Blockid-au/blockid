...
//   - web/src/app/workspace/plan/guide/[chapter]/page.tsx (workspace surface)
//
...
const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    ctaHref: "/workspace/strategy/competitors",
    agentLinks: [
      { label: "Competitive Intelligence", href: "/workspace/strategy/competitors" },
    ],
  },
  {
    slug: "04-mvp",
  ...
  },
  {
    ...
    ctaHref: "/workspace/finance/invoices",
    agentLinks: [
      { label: "Tax Invoice Checker", href: "/workspace/finance/invoices" },
    ],
    ...
    // path from Chapter 5 into the /workspace/finance/invoices surface.
    ...
    sections: [
      {
        ...
        body: {
          en: [
            ...
            "How BlockID.au surfaces this: /workspace/finance/invoices is the founder-facing wizard — paste or type the invoice fields, hit Assess, and the pure checker (web/src/lib/compliance/tax-invoice-checker.ts) returns the 3-band classification (under_threshold / standard / large) plus missing-field slots + warnings + the ATO disclaimer. Hit Save to snapshot the assessment into compliance_tax_invoice_checks; the /workspace/documents/compliance panel then shows the latest-band chip so a future auditor sees the audit trail. The ABN checksum reuses the same P1g modulus-89 validator that /api/abr/lookup runs, so a typo'd ABN gets caught before the invoice ships.",
            "Founder next step for Chapter 5: run one real invoice you've raised (or the template you plan to raise from) through /workspace/finance/invoices. Fix any red slots today, save the snapshot for the data-room audit trail, and re-run the checker whenever you rev the template. This is a 5-minute Chapter-5 task that removes a Chapter-9 diligence question the reviewer would otherwise flag.",
          ],
          vi: [
            ...
            "BlockID.au bề mặt hoá thế nào: /workspace/finance/invoices là wizard cho founder — dán hoặc nhập các trường hoá đơn, bấm Assess, và bộ kiểm thuần (web/src/lib/compliance/tax-invoice-checker.ts) trả về phân loại 3 dải (under_threshold / standard / large) kèm ô trường thiếu + cảnh báo + disclaimer ATO. Bấm Save để lưu snapshot vào compliance_tax_invoice_checks; bảng /workspace/documents/compliance sau đó hiển thị chip dải mới nhất để auditor tương lai thấy dấu vết audit. Kiểm tra checksum ABN dùng lại validator modulus-89 P1g cùng /api/abr/lookup, nên ABN gõ nhầm bị bắt trước khi hoá đơn phát đi.",
            "Bước tiếp theo cho founder Chương 5: chạy một hoá đơn thật đã phát (hoặc template dự định phát) qua /workspace/finance/invoices. Sửa ô đỏ hôm nay, lưu snapshot cho dấu vết audit data-room, và chạy lại checker mỗi lần đổi template. Đây là task 5 phút Chương 5 loại bỏ câu hỏi diligence Chương 9 mà reviewer sẽ nêu.",
          ],
        },
      },
    ],
  },
  {
    ...
    ctaHref: "/workspace/strategy/gtm",
    agentLinks: [
      { label: "GTM Strategy", href: "/workspace/strategy/gtm" },
      { label: "Pricing Tiers", href: "/workspace/strategy/pricing" },
    ],
  ...
  },
  {
    ...
    ctaHref: "/workspace/strategy/roadmap",
    agentLinks: [
      { label: "Roadmap Builder", href: "/workspace/strategy/roadmap" },
      { label: "GTM Strategy", href: "/workspace/strategy/gtm" },
    ],
    ...
    // template row) but had no companion guide section. Anchors on Privacy Act
    // 1988 (Cth) APP1/APP3/APP5/APP8/APP11 + Google Consent Mode v2 + GA4
    // event taxonomy + GST Act s9-70 (splitting Stripe `amount` into `value` +
    // `tax` before send). General information only, not personal advice.
    sections: [
      {
        id: "ga4-measurement-plan",
        heading: {
          en: "GA4 measurement plan — the event taxonomy investors read before your pitch",
          vi: "Kế hoạch đo lường GA4 — bộ sự kiện nhà đầu tư đọc trước khi bạn pitch",
        },
        body: {
          en: [
            "Why founders skip it and regret it: GA4's default install ships you 'engagement' events that mean nothing at diligence. Investors don't ask 'how many pageviews last month' — they ask 'what is your activation event, what percentage of signups reach it in week one, and how has that moved for the last four cohorts?'. Without a measurement plan you don't have those numbers; you have a dashboard.",
            "Write the plan before the property: the one-page measurement plan names (a) your primary conversion event, (b) the activation event that gates week-one retention, (c) the referral / partner attribution model, and (d) the PII fields you will never send. BlockID ships this as `web/content/templates/legal/au-ga4-measurement-plan.md` (10 sections, 56 placeholders, 7 statutory + industry sources) — pull it from the data-room and fill it in during the 90-minute Chapter 7 session.",
            "Privacy Act 1988 (Cth) is the gate — not a footnote: an APP5 collection notice must fire before GA4 sees a first visitor, APP1 requires you publish an open + transparent privacy policy, APP8 governs the cross-border disclosure to Google's US servers, APP11 requires reasonable security. Turn on Google Consent Mode v2 (`ad_storage` + `analytics_storage` + `ad_user_data` + `ad_personalization` all defaulting to `denied` on load, flipping to `granted` only after the founder-controlled banner) so a pre-consent visitor still contributes conversion modelling without triggering an APP breach + Notifiable Data Breaches Scheme (Privacy Act Part IIIC) exposure.",
            "Event taxonomy across five funnels: acquisition (`page_view`, `first_visit`, `campaign_hit`), activation (`signup_completed`, `first_project_created`, `first_svi_calculated` — pick the one that predicts week-one retention), retention (`weekly_active`, `feature_used`, `report_generated`), revenue (`purchase` — see the GST rule below — `subscription_created`, `upgrade`), referral (`referral_link_clicked`, `referral_converted`, `partner_attributed`). Keep the parameter shape stable across the funnel — a rename mid-quarter breaks the cohort chart you'll paste into the pitch deck.",
            "GST-hygienic `purchase` mapping (GST Act s9-70): Stripe's `amount` is GST-inclusive. GA4's `purchase.value` should be the GST-EXCLUSIVE line (revenue you keep) and `purchase.tax` the GST component (÷11 of the inclusive total, per GST Act s9-70(1)). If you send the raw Stripe amount into `value` you overstate revenue by ~9.09% — an investor spotting the discrepancy against your Stripe reconciliation is the fastest way to lose diligence trust.",
            "Cohort-floor + Wilson-CI decision rules (avoid s18 ACL + s1041H exposure): before quoting a conversion delta in an investor communication, confirm (a) the cohort has ≥ 40 users (Wilson 95% CI on a proportion is unstable below that), (b) the compared cohorts share the same acquisition channel (mixing organic + paid destroys the read), (c) the read window matches Google's default 30-day retention floor (shorter windows privilege paid channels). Marketing a 'we grew 3x' claim outside those guardrails is a s18 Australian Consumer Law misleading-or-deceptive-conduct risk + s1041H Corporations Act 2001 (Cth) misleading-conduct-in-financial-services risk when the same claim appears in a fundraise pitch.",
            "Quarterly data-quality audit — the audit investors want to see: every quarter, reconcile GA4 `purchase.value` against Stripe live-mode reporting (tolerance ± 2% by count, ± 5% by revenue — anything wider is a tagging bug, not a Stripe fee); confirm no PII in the last 90 days of BigQuery export (email / phone / name fields must be Consent-Mode-blocked or hashed); confirm the event taxonomy on the property matches the versioned measurement plan (a founder-only Confluence page + a signed change log). Attach the last four audit sign-offs to data-room folder 4 (Product & Technology) so a diligence reader sees discipline, not just dashboards.",
            "How BlockID.au surfaces this: data-room folder 4 template `au-ga4-measurement-plan` is the artefact slot; `/guide/07-growth` renders the Ga4PlanPanel preview so a founder can inspect the filled-in template before publishing; the CMO agent (growth playbook) reads the plan on invoke so a 12-week acquisition plan targets the events you actually measure. For property-level configuration (custom dimensions + retention windows + BigQuery export) engage a GA4-certified partner before Chapter 9 — the two-week lead time to backfill custom dimensions catches founders who try to add them the week of the raise.",
          ],
          vi: [
            "Tại sao founder bỏ qua rồi hối tiếc: bản cài GA4 mặc định gửi cho bạn các sự kiện 'engagement' không nói lên điều gì lúc thẩm định. Nhà đầu tư không hỏi 'tháng trước có bao nhiêu pageview' — họ hỏi 'sự kiện kích hoạt của bạn là gì, bao nhiêu phần trăm signup đạt được nó trong tuần đầu, và nó đã dịch chuyển thế nào qua bốn cohort gần nhất?'. Không có kế hoạch đo lường bạn không có mấy con số đó; bạn chỉ có một dashboard.",
            "Viết kế hoạch trước property: kế hoạch đo lường một trang gọi tên (a) sự kiện chuyển đổi chính, (b) sự kiện kích hoạt gác cửa retention tuần đầu, (c) mô hình attribution referral / đối tác, và (d) các trường PII bạn sẽ không bao giờ gửi. BlockID cấp bản mẫu `web/content/templates/legal/au-ga4-measurement-plan.md` (10 mục, 56 placeholder, 7 nguồn luật + ngành) — kéo nó từ data-room và điền trong phiên Chương 7 kéo dài 90 phút.",
            "Privacy Act 1988 (Cth) là cửa gác — không phải chú thích: thông báo thu thập APP5 phải bắn trước khi GA4 nhìn thấy khách truy cập đầu tiên, APP1 yêu cầu bạn công bố chính sách quyền riêng tư mở + minh bạch, APP8 quản lý việc truyền dữ liệu xuyên biên giới sang server US của Google, APP11 yêu cầu bảo mật hợp lý. Bật Google Consent Mode v2 (`ad_storage` + `analytics_storage` + `ad_user_data` + `ad_personalization` đều mặc định `denied` khi tải trang, chuyển `granted` chỉ sau banner do founder kiểm soát) để khách trước-consent vẫn đóng góp cho mô hình chuyển đổi mà không kích hoạt vi phạm APP + rủi ro Notifiable Data Breaches Scheme (Privacy Act Part IIIC).",
            "Bộ sự kiện qua năm phễu: acquisition (`page_view`, `first_visit`, `campaign_hit`), activation (`signup_completed`, `first_project_created`, `first_svi_calculated` — chọn cái dự đoán retention tuần đầu), retention (`weekly_active`, `feature_used`, `report_generated`), revenue (`purchase` — xem quy tắc GST dưới đây — `subscription_created`, `upgrade`), referral (`referral_link_clicked`, `referral_converted`, `partner_attributed`). Giữ shape tham số ổn định xuyên phễu — đổi tên giữa quý phá vỡ biểu đồ cohort bạn sẽ dán vào pitch deck.",
            "Ánh xạ `purchase` chuẩn GST (GST Act s9-70): `amount` của Stripe đã bao gồm GST. `purchase.value` của GA4 nên là dòng KHÔNG-BAO-GỒM GST (doanh thu bạn giữ) và `purchase.tax` là phần GST (÷11 của tổng bao gồm, theo GST Act s9-70(1)). Nếu bạn gửi thẳng `amount` của Stripe vào `value` bạn thổi phồng doanh thu ~9.09% — nhà đầu tư phát hiện sai lệch so với đối soát Stripe của bạn là cách nhanh nhất để mất niềm tin thẩm định.",
            "Sàn cohort + quy tắc quyết định Wilson-CI (tránh phơi nhiễm s18 ACL + s1041H): trước khi trích một delta chuyển đổi trong giao tiếp nhà đầu tư, xác nhận (a) cohort có ≥ 40 người dùng (Wilson 95% CI trên một tỷ lệ không ổn định dưới ngưỡng đó), (b) các cohort so sánh cùng kênh acquisition (trộn organic + paid phá kết quả đọc), (c) cửa sổ đọc khớp sàn retention mặc định 30 ngày của Google (cửa sổ ngắn hơn ưu ái kênh paid). Tiếp thị một tuyên bố 'chúng tôi tăng 3x' bên ngoài các hộ lan đó là rủi ro hành vi lừa dối hoặc gây hiểu lầm theo s18 Australian Consumer Law + rủi ro s1041H Corporations Act 2001 (Cth) khi cùng tuyên bố xuất hiện trong pitch gọi vốn.",
            "Audit chất lượng dữ liệu hàng quý — audit nhà đầu tư muốn thấy: mỗi quý, đối soát `purchase.value` của GA4 với báo cáo live-mode của Stripe (dung sai ± 2% theo số lượng, ± 5% theo doanh thu — rộng hơn là bug tagging, không phải phí Stripe); xác nhận không có PII trong 90 ngày gần nhất của BigQuery export (email / phone / name phải bị Consent-Mode chặn hoặc hash); xác nhận bộ sự kiện trên property khớp kế hoạch đo lường đã đánh số phiên bản (một trang Confluence chỉ founder xem + change log đã ký). Đính kèm bốn phiếu ký-duyệt audit gần nhất vào data-room folder 4 (Product & Technology) để người thẩm định thấy kỷ luật, không chỉ dashboard.",
            "BlockID.au bề mặt hoá thế nào: template `au-ga4-measurement-plan` trong data-room folder 4 là ô artefact; `/guide/07-growth` hiển thị Ga4PlanPanel để founder xem trước bản mẫu đã điền trước khi công bố; đại lý CMO (playbook tăng trưởng) đọc kế hoạch khi được gọi để kế hoạch acquisition 12 tuần nhắm đúng sự kiện bạn thực đo. Với cấu hình cấp property (custom dimensions + retention windows + BigQuery export) hãy tương tác đối tác chứng nhận GA4 trước Chương 9 — thời gian dẫn 2 tuần để backfill custom dimensions bắt gọn các founder cố thêm nó vào tuần gọi vốn.",
          ],
        },
      },
    ],
  ...
  },
  {
    ...
    agentLinks: [
      { label: "Data Room", href: "/workspace/documents/data-room" },
    ],
  },
  {
    slug: "10-fundraise",
    ...
    ctaHref: "/workspace/valuation",
    agentLinks: [
      { label: "Valuation Dashboard", href: "/workspace/valuation" },
    ],
    sections: [
      {
        ...
        body: {
          en: [
            ...
            "s708 exemption re-analysis. The Corporations Act 2001 (Cth) s707 on-sale rule kicks in the moment shares originally issued under a s708 exemption are re-sold within 12 months — the seller must self-classify whether the re-sale is itself a personal offer under s708(1), a wholesale-client transfer under s761G(7), a sophisticated-investor sale under s708(8), or triggers a fresh disclosure document. The s708(1) 20-investor / A$2M / 12-month counter applies to primary AND secondary re-sales — do not double-count exemption headroom, and log each transferee row via the /workspace/investors/access Folder 2 Secondary Transactions Register (data-room-templates.ts item 2.9).",
            "Right of First Refusal (ROFR) + pre-emptive rights. Every AU shareholders' agreement drafted from a Blackbird / AirTree public template contains a Right of First Refusal clause: an existing shareholder wanting to sell must first offer the shares to the company / other shareholders on the same terms before hitting the market. Skipping ROFR in a secondary is the fastest way to blow up an investor relationship — run the ROFR waiver process through /workspace/investors/access (Folder 1 board minutes + Folder 2 shareholders agreement) BEFORE the Share Purchase Agreement is signed, not after.",
