...
function ctaButtonClasses(variant: PanelVariant): string {
  return variant === "workspace"
    ? "mt-3 inline-flex items-center rounded-md bg-action px-4 py-2 text-sm font-semibold text-on-action transition hover:bg-action-hover"
    : "mt-3 inline-flex items-center rounded-md bg-action px-4 py-2 text-sm font-semibold text-on-action transition hover:bg-action-hover";
}
...
const BAND_CLASS: Record<
  ...
  string
> = {
  "not-started": "bg-surface-sunken text-slate-700",
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-3">
        <div className="flex-1">
          <div className="mb-1 flex items-baseline justify-between text-xs text-secondary">
        ...
        <ul className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-5">
          {STAGE_ORDER.map((stage) => {
            ...
            return (
              <li
                key={stage}
                className="rounded-md border border-line-subtle bg-surface-sunken px-3 py-2 text-xs"
        ...
        <ul className="mt-2 space-y-2">
          {GA4_CANONICAL_EVENTS.map((ev) => {
            ...
            return (
              <li
                key={ev.id}
                className="rounded-md border border-line-subtle bg-white p-3 text-sm"
              ...
              >
                <label className="flex cursor-pointer items-start gap-3">
                  <input
                    ...
                    className="mt-1 h-4 w-4 rounded border-line text-bull focus:ring-emerald-500"
                    aria-label={ev.name}
                  />
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="rounded-full bg-surface-sunken px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                        {stageLabel}
                      </span>
                    </div>
