    ...
    >
      <div className="w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl border border-line-subtle overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div
          ref={scrollRef}
          onScroll={onScroll}
          className="flex-1 overflow-y-auto px-6 py-4 text-sm leading-relaxed text-slate-700"
        >
        ...
        {error && (
          <div
            role="alert"
            className="mx-6 mb-2 rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-xs text-rose-800"
          >
            {error}
          </div>
        )}

        <div className="px-6 py-4 border-t border-line-subtle flex flex-col sm:flex-row-reverse gap-2 sm:items-center sm:justify-between">
          <button
            type="button"
            onClick={onAccept}
