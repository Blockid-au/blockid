...
const SIGNAL_STYLE: Record<AreaSignal, { label: string; cls: string }> = {
  ...
  mixed: { label: "Mixed", cls: "bg-amber-100 text-amber-900" },
  weak: { label: "Thin", cls: "bg-rose-100 text-rose-900" },
  not_public: { label: "Not public", cls: "bg-slate-200 text-secondary" },
};
...
export function PhaseBenchmarkPanel({ ordinal }: { ordinal: PhaseKey }) {
  ...
  return (
    <div
      ...
      data-phase={p.phase}
      data-stage={p.stage}
      className="mt-3 border-t border-surface-100 pt-2"
    >
      <div className="flex items-center gap-1">
        <span className="rounded border border-line bg-surface-sunken px-1.5 py-0.5 text-[9px] font-mono text-primary">
          {p.stage}
        </span>
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-baseline gap-2">
        ...
        <span className="text-xs text-ink-500">{b.period}</span>
        ...
        <table className="mt-2 w-full text-left text-[10px]">
          ...
          <tbody>
            {ANALYSIS_AREAS.map((area) => {
              ...
              return (
                <tr key={area.id} className="border-t border-surface-100 align-top">
                  <td className="py-1 pr-2 text-ink-800">{area.label}</td>
                  <td className="py-1 pr-2 text-ink-500">{area.pillar.replace(/_/g, " ")}</td>
      ...
      {milestones.length > 0 ? (
        <details className="mt-2">
          ...
          <ul className="mt-2 space-y-2">
            {milestones.map((m) => (
              <li key={m.id} className="border-l-2 border-surface-200 pl-2">
                <p className="text-[11px] font-semibold text-ink-900">
                  <span className="font-mono text-ink-500">{m.date}</span> {m.headline}{" "}
                  <EvidenceTag grade={m.evidence} />
                </p>
                <p className="mt-0.5 text-[10px] leading-snug text-ink-700">{m.detail}</p>
                {m.figures.length > 0 ? (
                  <ul className="mt-1 space-y-0.5">
                    {m.figures.map((f) => (
                      <li key={f.label} className="text-[10px] text-ink-600">
...
export function StageBenchmarkSection() {
  return (
    <section aria-labelledby="stage-benchmark-heading" data-testid="stage-benchmark-section">
      <h2 id="stage-benchmark-heading" className="text-xl font-semibold text-ink-900">
...
const VERDICT_STYLE: Record<string, string> = {
  ...
  needs_nuance: "bg-amber-100 text-amber-900",
...
export function FolkloreChecksSection() {
  return (
    <section aria-labelledby="folklore-heading" data-testid="folklore-section">
      <h2 id="folklore-heading" className="text-xl font-semibold text-ink-900">
...
export function HumanReviewFlagsSection() {
  return (
    ...
    >
      <h2 id="review-flags-heading" className="text-sm font-semibold text-ink-900">
      ...
      <ul className="mt-3 space-y-2">
        {ATLASSIAN_HUMAN_REVIEW_FLAGS.map((f) => (
          <li key={f.id} data-flag={f.id} className="text-xs text-ink-700">
            <span className="font-semibold text-ink-900">{f.what}</span> — {f.why}{" "}
            <code className="rounded bg-surface-100 px-1 text-[10px]">{f.surface}</code>
          </li>
        ))}
      </ul>
    </section>
