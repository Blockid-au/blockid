  ...
  return (
    <section id="idea-tools" className="bg-surface-50 py-20 text-brand-900 sm:py-24">
      <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-9">
        <div className="grid gap-7 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base font-medium leading-relaxed text-slate-600 sm:text-lg">
              ...
              everything as your{" "}
              <span className="font-semibold text-gold-500">Founder Pack</span>{" "}
              when you&apos;re ready.
            </p>
          </div>
          <Link
            href="/tools/idea-valuation"
            className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-brand-500 px-6 text-sm font-semibold text-white shadow-[0_18px_48px_rgba(59,125,216,0.28)] transition-colors hover:bg-brand-600 sm:text-base"
          >
            Start free now
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3 rounded-2xl border border-surface-300 bg-surface-100 px-5 py-4">
          <Calculator
            strokeWidth={1.75}
          ...
          <p className="text-sm font-medium leading-relaxed text-slate-600 sm:text-base">
            ...
            <Link
              href="/score"
              className="font-semibold text-gold-500 underline underline-offset-4 hover:text-gold-600"
            >
...
function ToolCard({ tool }: { tool: Tool }) {
  ...
  return (
    <Link
      href={tool.href}
      className="group flex flex-col rounded-2xl border border-surface-300 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-brand-500 hover:shadow-md"
    >
      ...
      <p className="mt-5 text-xs font-semibold uppercase tracking-[0.18em] text-gold-500">
        {tool.eyebrow}
      </p>
      ...
      <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-gold-500 transition-transform group-hover:translate-x-0.5">
        {tool.cta}
