...
export function LocaleSwitcher() {
  ...
  return (
    <div
      role="group"
      aria-label="Language"
      className="inline-flex min-h-8 items-center gap-1 rounded-full border border-line-subtle bg-surface-sunken px-1 py-0.5 text-xs font-semibold text-secondary"
      data-i18n-skip
    >
      {LOCALES.map((code, i) => {
        const active = code === current;
        // G26 light chrome: navy pill for the active locale, dark ink otherwise.
        const cls = active
          ...
          <span key={code} className="contents">
            {active ? (
              ...
              <button
                type="button"
                onClick={() => onPick(code)}
                className={`${cls} focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-navy`}
              >
                {LABELS[code]}
              </button>
            )}
