...
function levelClass(level: CoverageLevel): string {
  ...
  return "border-red-300 border-dashed bg-red-50";
}

function LevelIcon({ level }: { level: CoverageLevel }) {
  if (level === "strong")
    return <Check className="h-3.5 w-3.5 text-emerald-700" aria-label="Strong evidence" />;
  if (level === "partial")
    return <CircleDashed className="h-3.5 w-3.5 text-amber-700" aria-label="Partial evidence" />;
  return <CircleAlert className="h-3.5 w-3.5 text-red-700" aria-label="Missing evidence" />;
}
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between gap-2 flex-wrap">
        <p className="text-xs uppercase tracking-[0.14em] font-semibold text-ink-700">
          Deck coverage per dimension
        </p>
        <div className="flex items-center gap-2 text-[11px] text-ink-500 flex-wrap">
          <span className="inline-flex items-center gap-1"><Check className="h-3 w-3 text-emerald-700" /> Strong</span>
          <span className="inline-flex items-center gap-1"><CircleDashed className="h-3 w-3 text-amber-700" /> Partial</span>
          <span className="inline-flex items-center gap-1"><CircleAlert className="h-3 w-3 text-red-700" /> Missing (chargeable)</span>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1.5">
                <meta.Icon className="h-3.5 w-3.5 shrink-0 text-brand-600" aria-hidden="true" />
                <span className="text-[11px] font-semibold text-ink-800 uppercase tracking-wider">
                  {meta.short}
                </span>
                <span className="ml-auto"><LevelIcon level={cov.level} /></span>
              </div>
              <p className="text-[11px] text-ink-700 leading-tight mb-1.5 font-medium">
                {meta.label}
              </p>
              {cov.excerpt && (
                <p className="text-[10px] text-ink-500 leading-snug italic line-clamp-2 mb-1.5">
                  “{cov.excerpt}”
                </p>
              )}
              <div className="flex items-center justify-between gap-1 mt-1">
                ...
                {isSpeculative && speculativeCost > 0 && (
                  <span className="inline-flex items-center rounded-full bg-brand-100 text-brand-800 px-1.5 py-0.5 text-[10px] font-semibold tabular-nums">
                    +{speculativeCost} cr
                  </span>
                )}
              </div>
