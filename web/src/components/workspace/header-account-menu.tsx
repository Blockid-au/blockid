...
// Workspace topbar — the account cluster below the `sm` breakpoint.
//
...
// now shows only the notification bell and this avatar button; the avatar
// opens a menu carrying the name, the credit balance, the wallet button,
// the theme toggle and Sign out. From `sm` up the layout renders the
// original inline cluster and this component is `sm:hidden`.
//
...
/** The 28 px avatar used inline on desktop and as the mobile menu trigger. */
...
export function HeaderAccountMenu({ user, initialOpen = false }: HeaderAccountMenuProps) {
  ...
  return (
    <div ref={ref} className="relative sm:hidden" data-testid="header-account-menu">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="h-9 flex items-center gap-1 rounded-lg pl-1 pr-1.5 hover:bg-surface-hover transition-colors cursor-pointer"
