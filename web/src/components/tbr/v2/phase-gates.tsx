...
export function TbrPhaseGates({ report, title, locale = "en" }: { report: ReportV2; title: string; locale?: "en" | "vi" }) {
  ...
  return (
    <TbrSection id={TBR_V2_SECTION_IDS.phaseGates} kicker="11" title={title}>
      <div className="flex items-center gap-2 text-xs text-ink-600 dark:text-ink-300">
        <AgentBadge role="coo" />
        <span>Current phase: {GROWTH_PHASE_LABELS[g.current][locale]}</span>
      </div>
      {route && <VisualFigure spec={route} caption={null} />}
      <table className="w-full text-xs">
        <caption className="py-1 text-left text-[10px] font-semibold uppercase tracking-wide text-ink-500">Required criteria for {GROWTH_PHASE_LABELS[g.current][locale]}</caption>
        <tbody>
          {currentRows.map((m) => (
            <tr key={m.criterion} className="border-t border-ink-100 dark:border-ink-800/60">
              ...
              <td className="py-1 text-right">{m.met ? "✓ met" : "✗ not met"}</td>
            </tr>
          ))}
        </tbody>
      </table>
