// API key management for the auth microservice.
//
// Keys follow the format: bk_live_<48 hex chars> (56 chars total).
// We store only the SHA-256 hash — the raw key is shown once at creation
// time and never persisted. The key_prefix (first 16 chars + "...") is
// stored for display in the management UI.
//
...
export async function createApiKey(
  ...
  name?: string,
  permissions?: string[],
): Promise<{ ok: true; key: string; keyId: string } | { ok: false; error: string }> {
  ...
  if (!canCreateApiKeys(plan)) {
    return { ok: false, error: "API keys require a Growth plan or above." };
  }
  ...
  if (error || !data) {
    console.error("[auth:api-keys] insert failed", error);
    return { ok: false, error: "Failed to create API key." };
  }

  return { ok: true, key: raw, keyId: data.id };
}
...
export async function validateApiKey(rawKey: string): Promise<ValidatedKey> {
  ...
  const { data: keyRow, error } = await supabase
    .from("api_keys")
    .select(
      "id, user_id, permissions, rate_limit_per_min, is_active, expires_at",
    )
    .eq("key_hash", hash)
    .maybeSingle();
...
export async function listApiKeys(userId: string): Promise<ApiKeyInfo[]> {
  ...
  if (error || !data) {
    console.error("[auth:api-keys] list failed", error);
    return [];
  }
...
export async function revokeApiKey(
  ...
  if (error) {
    console.error("[auth:api-keys] revoke failed", error);
