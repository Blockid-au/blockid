...
| 3 | **The startup's own evidence, accumulating.** Evidence vault, weekly SVI snapshots, score history, data room, founder-side next steps — the company updates itself and you see the delta | `svi-snapshot`, `svi_readiness_snapshots`, `watchlist-digest`, TBR, `investor_link_views` | ChatGPT has no private data or memory; Equidam/valuers are one-off; portfolio tools depend on founders emailing updates (~50 % do) |
...
Supporting bullets: consistent verdicts (vs model-to-model variance) · no hallucinated founder histories — claims are tied to evidence or marked missing · confidential: founder-consented access tiers, no pasting decks into a public chat — **approved sentence (founder, 2026-09-10):** "Your inputs are not used to train our models; founder-consented access tiers control who sees what; reports are generated by our AI providers under their business terms." (T0275 verifies each provider policy in the free-model chain before go-live) · time: minutes, not 30–60 min per deck · audit trail for your IC / LP.
