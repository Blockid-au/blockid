...
> **T0250 shipped (2026-09-10):** the lines are code now — [`web/src/lib/marketing/hero-variants.ts`](../../web/src/lib/marketing/hero-variants.ts) is the typed catalogue (`FOUNDER_LINES` F1–F4, `INVESTOR_LINES` I1–I3, `GENERAL_LINES` G1–G3, EN + VI), with `speakabilityCheck()` as the automated gate and `pickHeroVariant()` / `?hero=F2|F3` as the A/B switch; `messages/en.json` + `vi.json` mirror them under `hero.line.*` (the old `hero.v3.*` keys are marked deprecated). Shipped defaults: homepage H1 = **F1**, sub-line = **F3**, `SITE_DESCRIPTION` / `og:description` = **G1**, homepage `<title>` = first breath of F1. GA4 reads the arm from `hero_variant_shown{arm}` and `svi_submitted{arm}`. The human tests (5-second, say-it-back, 2 weeks or 500 sessions/arm) are in [`hero-one-liner-test-protocol.md`](./hero-one-liner-test-protocol.md). **Truth softening (G9 SC2 / G10):** F3 said "the grants you qualify for — free"; the eligibility match is the A$3 Money Finder, so the shipped F3 promises "every open Australian grant — free" (the free `/funding/grants` directory). **Winner log:** _none yet — F1 is the incumbent until the A/B closes; add dated rows below this line._
