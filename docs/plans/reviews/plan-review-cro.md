...
## Recommendations
1. **BLOCK P8 GA on fixing the remove path.** Either (a) switch to `stripe.subscriptionSchedules` and schedule item removal at `current_period_end`, keeping the item live until then; or (b) if immediate removal is preferred for accounting cleanliness, change UX copy to "Remove immediately (no refund for unused days)" + require an explicit "I understand" confirmation, and drop the `cancel_at_period_end-style` framing from plan §F.5 and code comments.
