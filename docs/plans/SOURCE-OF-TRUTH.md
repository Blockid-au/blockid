...
- **Post-ship review** ([`reviews/post-ship-s28-2026-09-13.md`](./reviews/post-ship-s28-2026-09-13.md), GO) → fix deploy in flight: **P1** CRM data-room hook wrote a touchpoint on every engage beacon (timeline flood) → one per (contact, link, trigger) per 24 h; P2 DRIP statement re-frozen when the cap-table write fails; P2 bank-CSV MRR now revenue-category only (grants stayed on income); P3 strict ISO cursor. Open P3s in the report (regenerate-retry charging, no pro-rata AI refund, burn precedence mismatch between `burnRateWithBankFallback` and `sources.ts`, CRM import rate limit, `shares_held` non-atomic increment).
