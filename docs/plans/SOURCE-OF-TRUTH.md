...
- **S22-A merged (deploying):** investor-DD valuation certificate PDF — 3-page A4 (range + radar, method/`svi+arr_multiple` note, evidence summary, doctoral sentence, APES 225 / general-advice disclaimer, Auschain signature block, optional watermark), `VC-XXXXX-XXXXX` + content hash, public `/verify/valuation/[no]`, revoke, 5 credits / included Growth+, data-room save; migration 0341 applied.
- **S23 in flight (founder-approved 2026-09-12):** **S23-A** OAuth connector tokens sealed at rest (fail-closed, transitional flag, key rotation, reseal script + runbook, status signal) · **S23-B** GA4 custom-dimension registration script + admin route (blocked at GCP: enable `analyticsadmin.googleapis.com` in project 990415480608 + grant the SA Editor on the property — two console clicks), weekly `ga4-event-audit` cron, status signal.
