...
- **S18-B in flight:** `(founder)` server pages member-aware (no split `svi_accounts` for members, viewer read-only props, pages scope guard). **S18-B review fixes (merged, deploying):** P1 `report_sections` are per ANALYSIS (unique `analysis_id,section_id,depth`) — `dashboard` + `dashboard/svi` drop the caller `user_id` filter so a member sees the owner's unlocked sections and the owner never loses them; `api/svi/report-section` returns an existing row as `alreadyUnlocked` / `credits_spent: 0` (no AI call, no spend, no write — the purchaser's `user_id` is never re-stamped; affordability checked after that short-circuit; insert `ignoreDuplicates`); pages-scope-guard rule F. P2-1 `api/founder/tech-analysis` = `assertProjectScope(startup_id, "editor")`, project row + `tech_analyses` upsert keyed on `ownerUserId` (one row per project), `TechIntelligencePanel readOnly` for viewers. P2-2 data-room viewer sees no Drive-setup / Generate / AI-Fill CTAs. P2-3 `ViewOnlyNote` editor copy → "ask an admin or the project owner". P2-4 c-level report "export" form (dead `/api/investor-pack/append`) → Link to `/workspace/investor-pack/generate` (prices before charging). P2-5 `dashboard/accelerator` + `workspace/investor-pack` read `svi_accounts` by `id` via `resolveSVIAccountIdForPage` (no `account_id`/`user_id`/`score`/`stage` columns exist; `svi_milestones.badge_label`) — cards were always empty before. P2-6 onboarding redirect guarded `!isMember`. P2-7 Money Radar tile = **owner's project data, caller's credits**: `getMoneyRadarTileData(user, project, pre, { ownerUserId, dataEmail })` keys report / matches / data room / prefill on the owner, entitlement + calendar token on the caller, `creditNote` under the CTAs for a member. P2-8 `dashboard/page.test.tsx` (8 cases). Verified: `tsc` clean, eslint 0 errors, 73 files / 728 tests green over the touched dirs. Known leftovers (not in the review): `workspace/funding` page still keys its own tile/report on `user.id` (legacy owner path, `getActiveProject(user.id)`); `svi_reports` table referenced by `workspace/investor-pack` has no migration (read is try/catch-guarded).
