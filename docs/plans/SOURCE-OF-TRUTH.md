...
- **S31-A** (in flight): Anthropic tier via the official SDK (upgrade to 0.125.x), task-class routing Haiku 4.5 / Sonnet 5 / Opus 5 with adaptive thinking + effort, prompt caching, streaming for long outputs, rate-limit headers into the dispatcher, provider validity probes → `ai_providers` on `/api/status` (invalid keys never retried on the hot path), `AI_DAILY_SPEND_CAP_AUD` + `OPENROUTER_MIN_CREDIT_USD` guardrails with founder notification, bounded queue + per-user fairness + 503/Retry-After instead of 500s, dead-model auto-prune, `docs/ops/ai-providers.md`.
- **S31-B** (in flight): feature-completeness review + polish (stubs → honest "not available yet", stale prices → `plans-v2`, gated pages → upgrade card, dead controls, empty states, validation), report `reviews/feature-completeness-2026-09-13.md`.
...
- Then: merge → full suite → deploy → live-qa + `qa` lane → fix → deploy.
