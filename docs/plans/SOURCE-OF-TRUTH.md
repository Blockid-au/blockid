...
**Cập nhật thực thi24/09, 23:33 UTC — CFO/projection valuation:** scoped release đã **deployed** trên BlockID `871f2fee425f7faf5cbaa487fb4d14a78eb587ea` (build `euTqmQtYoKNROtDD9i1TI`, active4143) và SVI `edebcea1127c6432282cf541f5b1f54e7bc47649` (build `R5H7MK_-s_2Z6ee8OhAQZ`, active4212). [Deployment receipt](../reviews/2026-09-24-cfo-production-deployment.md), [execution receipt](../reviews/2026-09-24-cfo-implementation-receipt.md), [full-plan follow-up](../reviews/2026-09-24-full-plan-followup.md) phân biệt source/tests/deploy/acceptance. Có deterministic scenario core, shared source hai site, criteria summary, DeepInfra customer routing và aggregate report-down status. Operational release accepted theo accelerated policy; scenario vẫn `scenario_only`, panel shadow-only. Nguồn doanh thu đủ điều kiện, immutable official valuation, calibration độc lập, authenticated live calculation và G33 stability còn mở; không đánh dấu toàn G31/G32/G33 hoàn tất.

**Baseline review toàn app24/09/2026,13:59UTC — trước implementation bên trên:** founder yêu cầu rà soát toàn bộ hai app, bảo toàn đầy đủ G31/G32/G33 và đưa policy C-level DeepInfra tối ưu chất lượng/cost/capacity vào plan. **G31 chưa implemented; G32 approved nhưng chưa implemented; G33 partial implementation/live, chưa accepted.** Source baseline BlockID `c2e147066` / SVI `565250b`; manifest T16k có `deployed_at` rỗng nên không suy live từ source. Inventory **441 pages +722 handlers** và traceability mọi IL/SV/T/phase tại [full-app review](../reviews/2026-09-24-full-app-g31-g32-g33-reconciliation.md). **§12.12** điều chỉnh thứ tự/closure; **§11.3.5** và [AI policy§8](g30-ai-routing-model-policy-2026-09-23.md#8-cập-nhật-24092026--c-level-deepinfra-tối-ưu-chi-phí-và-capacity) là target C-level mới nhất, thay đề xuất provider cũ khi mâu thuẫn. Đây là review/plan update, không runtime activation, không benchmark spend mới, không tuyên bố full complete.
...
**Historical live checkpoint24/09,02:15UTC:** BlockID compiled `3839ef907fd687e61a11c25df8fa369347c037af` active4131/warm4130; SVI remains compiled `93478aa926f2db2283b4ebb74871594a7700ff27` active4211/warm4210. Immutable readers now verify SHA-256 `report_hash` for writer-created revisions and reject mismatched JSON; null-hash migration rows remain compatible. Read-only reconciliation confirms the applied 0410 table exists but has zero rows; no customer report was generated without a founder-designated safe project/scope. Origins4125/4126 were retired with exact identity and artifact retention. HTTP/static/identity gates and mark-good succeeded; detached-work/quiescence coverage remains incomplete. **Full G30 implementation and qualification remain open.** [Current hash-reader receipt](../reviews/2026-09-24-reader-hash-rollout.md); [stream receipt](../reviews/2026-09-24-stream-revision-rollout.md). DeepInfra-only US$0.50/report remains unchanged.
...
| Phase | Nội dung | Gate |
|---|---|---|
| SV0 | D22 đã duyệt 24/09. Viết **rubric@v1** cho 52 câu (anchors 0–4, 2–4 checklist nhị phân, ví dụ level 2 và 4, loại bằng chứng yêu cầu, applicability theo stage/sector) + publish methodology draft. **26/09: rubric@v1 implemented** (`web/src/lib/screening/rubric.ts`, 52 câu + 51 overlay = 103 entries, chưa nối scoring) + [methodology draft EN](../research/2026-09-26-g32-sv0-rubric-v1-methodology.md) | Rubric review bởi analyst; không đổi code |
...
| SV2 — method metadata | Additive migration: `svi_method`, `profile_sha256`, `rubric_version`, `knowledge_cutoff`, `contribution_ledger` (theo câu hỏi) trên snapshot/revision; writer ghi cho report mới (Lane A writer). **26/09 implemented không cần migration:** optional `methodMeta` trong ReportV2 JSON (`report-v2/method-meta.ts`, `screening/profile.ts`), stamp ở run-for-project/run-report-pipeline trước snapshot + revision + assembled insert; `svi_method` lấy từ version của SVIAnalysis; paywall order path chưa stamp | Snapshot cũ đọc được; không backfill |
...
- **Nguồn factor Scorecard:** lấy q_c đã chuẩn hóa 0–1 từ question-level panel/calibration và e_c từ evidence ledger, không lấy index không trần. Formula dưới dùng q chưa nhân e; chỉ attenuation một lần, eligibility/coverage kiểm riêng (§9.5.4). Ánh xạ trọng số Payne: team 30 (founder/team/structure) · opportunity 25 (idea/market) · product 15 (code/roadmap/website) · competition 10 (market research R02) · sales 10 (customers/gtm/revenue) · financing 5 · other 5 (documents/dataroom). `factor = 1 + (q − 0.5) × 2 × range × e`: bằng chứng yếu thì factor co về 1.0 (trung bình vùng), để claim chưa kiểm chứng không đẩy định giá.
- **Anchor AU:** **một bảng duy nhất** có version/ngày/nguồn. Cut Through Venture / Folklore *State of Australian Startup Funding 2025* (3/02/2026) chỉ công bố median round size (pre-seed A$1.0M, seed A$2.5M, Series A A$11M), **không có median pre-money**. Với simple primary priced-equity round, post-money scenario = round size ÷ assumed dilution; pre-money scenario = post-money − primary round size. Đây chỉ là **giả định có ngày**, không observed median pre-money; các terms khác đi qua scenario engine (§9.5.4). Hoặc dùng DB comparables khi đạt n rules. Carta là dữ liệu Mỹ, chỉ tham khảo.
...
#### 9.5.4 CFO-led projection methodologies và đồng bộ đánh giá agent — PLAN ONLY24/09

**Yêu cầu founder mới nhất:** đưa vào kế hoạch thực hiện chi tiết, **chưa code**. [Đặc tả đầy đủ](g32-cfo-projection-valuation-implementation-2026-09-24.md) là phần mở rộng có hiệu lực của §9.5.2–9.5.3 khi chi tiết hơn; không tuyên bố source CFO hiện đã đạt chuẩn quốc tế.

1. **CFO là owner methodology:** đối chiếu IVS scope/data/model/reporting và IVS200, IPEV2025 calibration/method selection, tài liệu projection/valuation học thuật với các nghiên cứu CFO hiện có. Registry từng method có version/basis/source/inputs/eligibility/formula/oracle/reviewer. Berkus/Scorecard là heuristic có điều kiện; không gọi mọi method là chuẩn IVS hoặc buộc mọi report có4/7methods.
2. **Tái dùng có review:** `cfo-valuation.ts` có `dcf_proxy` không phải DCF; `compute-c-level-dcf.ts` và `clevel-valuation.ts` có discounting nhưng fixed WACC/g/presets cần qualified inputs. Hợp nhất forecast-builder/financial-projections/CFO projections và các source repos; crosswalk từng method `keep/correct/replace/retire/legacy-read-only`, không bỏ lịch sử nghiên cứu hoặc tạo engine mới cạnh tranh.
3. **Projection trước valuation:** monthly36tháng và annual5năm/đến steady-state dùng một driver set; bottom-up theo SaaS/marketplace/services/hardware/milestones, top-down kiểm feasibility. Linked P&L/balance sheet/cash flow gồm capex, working capital, debt, tax và funding gap. Actual/management forecast/assumed tách rõ; forecast traceable được dùng làm scenario chứ không giả actual. Bear/base/bull thay drivers, không nhân valuation ±%.
4. **Methods đủ điều kiện:** FCFF/WACC hoặc FCFE/cost of equity; market/transaction comps; recent financing calibration; VC/First Chicago scenarios; asset approach khi phù hợp; Scorecard/Berkus cho pre-revenue có coverage. Method thiếu input là not_estimable riêng; không MRR kill-switch toàn bộ. WACC/g/FX/benchmark có nguồn và ngày, không mặc định theo stage. EV→equity→rights allocation, pre/post-money và funding feasibility phải nhất quán. ESIC benefit không tự cộng operating EV; RDTI theo qualified cash-flow schedule.
5. **Assessment↔CFO crosswalk:** cùng evidence revision, mapping từng question/panel result sang driver có source, bounds, transformation version và calibration. Không total SVI→money; không valuation cao→score cao; không nhân evidence confidence hai lần; thiếu evidence không tự đủ điều kiện average-company factor. G32 ba family/panel median/N/A và no-rerun-bonus giữ nguyên. Delta bridge giải thích evidence→score→driver→cash-flow→valuation; score và valuation có thể đổi khác hướng.
6. **CFO AI orchestration:** agents cung cấp evidence/driver proposals; deterministic engine tính; CFO giải thích output đã kiểm; independent checks và reviewer xử lý judgement. G31 Lens chỉ render; DeepInfra role policy/budget giữ nguyên; không tự thêm15nightly calls từ thiết kế cũ.
7. **Một accepted result:** shared registry/core trên hai app; same revision/hash/source/units trong web/PDF/DOCX/email/XLSX/API/C-level reports. Scenario sliders tạo fork, không overwrite official score; migrate tất cả share-price/vesting/dividend consumers, giữ nguyên quyền và số đã phát hành.

| Substeps trong V04, không goal mới | Deliverables / dependencies |
|---|---|
| P0 | CFO method/research/source crosswalk và standards mapping; docs-only có thể chuẩn bị độc lập |
| P1–P2 | Financial input/provenance + projection core/schedules/sector drivers; V01/E01/E02 prerequisites |
| P3 | Audited deterministic valuation methods/eligibility, discount/terminal/rights bridges và oracle |
| P4 | G32 question-level score/driver mapping, version/lineage/no-double-count; A05/SV0–SV3 |
| P5 | Calibration/comps/human validation/sensitivity; V03/Q02, không target valuation từ founder ask |
| P6–P7 | Immutable publish/export/workbook parity + consumer migration; F02/F04/T02/O08, G31/SV5 |
| P8 | Corpus/holdout/canary/budget/load/rollback acceptance; G33 stability và G32 calibration gates |

**Phép tính cần sửa trong anchor design:** chỉ với simple primary priced-equity round, `new cash / ownership = post-money`, **pre-money = post-money − new cash**. Không dùng median round size chia assumed dilution rồi gọi observed median pre-money. Terms/phí/secondary/convertibles cần scenario engine; benchmark disclosure riêng.

**Gate:** input/formula/method parity100% trên material outputs, reviewer tái tính được, forecast-vs-actual theo valuation-date information, no critical factual/numeric errors, same final revision cả2site và exports. Không lấy word count,4methods,correlation với raise hoặc forecast lạc quan làm acceptance. Tất cả P0–P8 trong đặc tả mới vẫn **PLANNED**, chưa code.
...
### 11.3.5 C-level DeepInfra theo task, cost và capacity — cập nhật24/09

Yêu cầu founder mới nhất được đưa vào cùng O01/O02/Q01/Q02/S02, không tạo registry/backlog thứ hai: **target mọi C-level customer inference hai app dùng DeepInfra**, model rẻ nhất đạt quality floor theo task/materiality/language/modality. G31 deterministic Lens không gọi AI thêm. G32 giữ3-family votes; code tính số, AI chấm rubric/trích driver có nguồn. [Exact IDs, pricing official24/09 và role matrix đầy đủ](g30-ai-routing-model-policy-2026-09-23.md#8-cập-nhật-24092026--c-level-deepinfra-tối-ưu-chi-phí-và-capacity).

- Flash baseline: `deepseek-ai/DeepSeek-V4-Flash` $0.09/$0.18 per 1M input/output; **ưu tiên benchmark** `DeepSeek-V4-Flash-0731` $0.06/$0.18, không auto-admit vì rẻ hơn. Model0731 có official page; đoạn historical23/09 nói không tồn tại không còn là kết luận hiện hành.
- Qwen3-235B Instruct $0.09/$0.55 cho reasoning/challenger; V3.2 $0.26/$0.38 cho material reasoning/summary chỉ khi gain được chứng minh. Không gán model đắt mặc định vì chức danh C-level.
- G32 target DeepSeek+Qwen+Llama3.3-70B Turbo ($0.10/$0.32), **ba family**; Llama phải qualify và scoped admission trước activation. 52 × 3=156 phiếu; thiết kế batch 4 câu/criterion giả định 13 owner calls +26 judge calls, không phải runtime count đã đo. Reuse owner call vẫn thêm output tokens cho question_scores. Panel target ≤ US$0.10 nằm trong hard report cap US$0.50, không cộng ngoài.
- Vision: baseline Qwen3-VL235B $0.20/$0.88; benchmark30B $0.15/$0.60 trước thay. GPT-OSS120 rất rẻ nhưng smoke report cũ không có citation: không đủ qualify final report/judge. Model-price list không phải allowlist.
- Shared semaphore/quota ledger cho cả2 sites và mọi origin/cron; fair tenant queue, durable lease/retry, reserve summary capacity, bounded model-specific429 backoff. Public default200 concurrent/model/account không chứng minh quota account hay200 reports cùng lúc. Tối ưu **cost/accepted-report** gồm retries/failed/unknown usage, P95 và throughput measured.
- Source T16j/k thêm Groq CEO fallback +40 s reserve, chưa phù hợp target trên. O01/T16 phải reconcile routing/budget/telemetry và release evidence, không claim DeepInfra-only trong khi direct fallback còn active. Không âm thầm đổi runtime từ tài liệu.

Gates và ngân sách benchmark giữ riêng: candidate→smoke-tested→admitted→release-qualified. Không dùng citation count hoặc model marketing benchmark thay claim correctness. Model parameters/capabilities kiểm theo exact ID; policy giá revalidate trước activation và không quá expiry23/10. Các đề xuất free-provider fallback§11.3.3/historical D-A4 không được tự đưa vào target C-level mới này; nếu giữ emergency exception, phải ghi riêng và không gọi DeepInfra-only.
...
| V04 | P0 · Valuation/Backend | Một engine `valuation-core`; CFO methodology registry + financial projections + audited DCF/market/scenario methods; gỡ mọi SVI→tiền consumer; nguồn anchor/multiple/FX, question-level driver mapping và calibration; triển khai V04-P0…P8 theo [CFO spec](g32-cfo-projection-valuation-implementation-2026-09-24.md), §9.5.2–9.5.4 | V01; V02/V03 dùng cùng engine; D22-e cho share price | Không surface nào nhân SVI ra tiền; cùng input → cùng số mọi surface; `not_estimable` theo method |
...
| F02/T02 completion persistence | PARTIAL LIVE24/09 / STREAM WRITER LIVE — ACCEPTANCE DEFERRED | BlockID assembled row + canonical JSON + complete status inserted together and returned document confirmed; done withheld until save. Full, rescore, and legacy stream callers now write and read back immutable `report_revisions` tokens after the daily snapshot document is confirmed, while the daily token remains a legacy projection. Stream save status fails closed when the revision cannot be confirmed. Order retries share the US$0.50 DeepInfra scope. SVI uses atomic per-file replacement, slug-before-completed-run publication and done-after-save in both pipelines. Controlled real-run reconciliation, unknown-commit recovery evidence, migration-source authority transition, and complete non-stream legacy-writer coverage remain open. [Stream receipt](../reviews/2026-09-24-stream-revision-rollout.md) |
...
| G34 BT0–BT6 · DC01–DC10 · EM01–EM21 · RQ01–RQ28 Biz Trust Report v4 | BT0 + BT1 LIVE 25/09 (`51bc51673`, origin 4151, mark-good): screening registry 78 items/8 dims (pure data), D24-b free page-1 signal status (leak-tested web/PDF/DOCX/email), DC01–DC09 (0462/0463/0464 applied; INTAKE_UPLOAD_DIR=/data/intake-uploads) · BT2 LIVE (`981dc4c04`, origin 4152): one engine (lifecycle-mailer/nurture stubs retired, crontab 3 lines commented), `email_sends` log + fail-closed C-class cap, Resend bounce/complaint webhook (needs RESEND_WEBHOOK_SECRET — founder), express consent unticked (0465 applied, commercial defaults false), RFC 8058 one-click, T-class lint · BT3 LIVE (`e45c3b292`, origin 4153): dashboard v4 page 1 on web/PDF/DOCX/email from ONE projection (5 tiles, meeting label, key metrics never derived, 8-row scorecard with lead + emphasis band, red flags deduped, signal chips, why/stop/ask), leak-tested; not yet: trend (no prior-method record), preliminary/old-revision states, peer percentile/stage ladder (BT6) · BT4 LIVE DRY (`413eb32c6`, 0466 applied; LIFECYCLE_EMAIL unset = no lifecycle sends until founder approval; weekly founder digest kept) · BT6-lite LIVE (`af652ac4e`, origin 4155): RQ19 peer position (published n≥10 only), RQ20 stage ladder (evidence-only; Scaling ≥A$1M / Established ≥A$10M connector ARR — thresholds pending founder confirmation), RQ21 calibration line, RQ27 round readiness (no data yet), RQ28 spike · also live: G33 T14 listener ceiling, T12 image-size override (0 prod vulns), erasure of email_sends (0467), share-view e-mail burst fix (link-checker HEADs had e-mailed ~12 owners on every deploy since 22/09) · GROUNDED SHARE LIVE (`b3832e0f9`, origin 4156): claim-gate hygiene (law-name years, advice lines, hyphenated terms), malformed citation markers, GST row, no figures in titles — live canary 19:12 UTC degraded 0/8, grounded 0.91 (KPI 0.85), US$0.031, 6.1 min; 24 h median 0.96 over 4 runs (budgetOverruns 2 to watch) · G30 capacity fix: a quarantined origin is released only when procfs proves its PID was reused (thread id / start ticks) · BT5/BT6 gated (G33 S1 7 days, SV4) | Plan + spec + research annex committed; chờ duyệt D24; BT3 chờ Codex land screening UI; BT5 chờ G33 S1 đạt 7 ngày (§10.14, §12.13) |
| AF01–AF15 · ER1–ER3 luồng /analyze (25/09) | INCIDENT FIX LIVE (`52ccc0880`, origin 4146) + WAVE A LIVE (`61030f2df`, origin 4147, mark-good); AF01–AF07, AF09, AF10 fixed · WAVE B LIVE (`d0b248888`, origin 4148): ER1 + ER3 + AF08 + AF14; ER2 valuation draft deliberately not shown (H10) · WAVE C LIVE (`1ff499264`, origin 4149, mark-good): AF11, AF12, AF13, AF15 · all AF01–AF15 + ER1/ER3 live; FINAL `973d5d808` (origin 4150, mark-good): Codex investor-screening UI landed (web/PDF/DOCX/email) + G34 DC01 `analyses.project_id` (0462 applied live) → AF13 full · only ER2 valuation draft remains, waits for G32 V04b (no SVI→money, H10) | Receipt 2026-09-25-analyze-upload-failure-review; §12.14 |
...
### 12.11 G33 — Kế hoạch implement hợp nhất (24/09/2026, PARTIAL IMPLEMENTATION — acceptance mở)

**Trạng thái hiện hành sau release23:33:** S0 aggregate report-down fix deployed, T01 threshold reconciliation còn mở; S1 partial và chưa có canary acceptance thay thế failure13:36; S2 fixes live nhưng cron/schema authority acceptance còn; S3 partial/open; S4–S6 chưa đạt exit gates. CFO scenario và shadow-only reducer đã được deploy độc lập, không phải activation scoring/official valuation. Bảng phase dưới là requirements, không phải bằng chứng đã done. **§12.12** và [follow-up](../reviews/2026-09-24-full-plan-followup.md) điều chỉnh dependency/closure.

**Vì sao có mục này:** test toàn bộ bản live ngày 24/09 ([receipt](../reviews/2026-09-24-live-version-test-review.md)) cho thấy site, trang và thanh toán chạy tốt, nhưng **sản phẩm lõi — sinh report — gần như ngừng từ 21/09**. Đồng thời monitoring và QA vẫn báo xanh. Mục này hợp nhất G30 còn mở (§12.9–12.10), G31 (§10.13), G32 (§9.4–9.5) và các lỗi test thành **một thứ tự thực hiện duy nhất**, thay thứ tự lượt E1–E5 ở §12.10 khi khác nhau. IDs cũ giữ nguyên; G33 chỉ đặt thứ tự và thêm các việc mới T-xx. G32 deterministic scenario/shadow helpers được thực hiện độc lập; **SV3 live shadow inference và SV5 activation vẫn giữ stability/calibration gates**, không bị mở bởi deployment của pure helper.
...
| T16 | P0 | Dành cửa sổ thời gian riêng cho SYNTH (CEO summary) như W4 reserve; 24/09 canary: summary chỉ còn 22–34 s trước deadline 420 s | S1 — **LIVE** (+T16b–e) |
| T16f | P1 | Lý do từ chối reservation hiển thị trong log; lock wait ledger 1.5 → 3.5 s cho ~10 call song song | S1 |

### 12.12 Full-app reconciliation G31/G32/G33 — 24/09/2026

**Scope theo yêu cầu mới:** review và kế hoạch bao phủ toàn bộ **BlockID.au và StartupValueIndex.com**, public/private/admin/API/integrations/tools, không chỉ report routes. [Inventory441 pages / 722 handlers](../reviews/2026-09-24-full-app-route-inventory.csv) gắn source SHA; [review/matrix](../reviews/2026-09-24-full-app-g31-g32-g33-reconciliation.md) là evidence annex cho queue hiện hữu, không backlog riêng. Static inventory/link/HTTP checks không phải full functional acceptance.

#### Trạng thái và yêu cầu không được bỏ sót

| Gói | Covered IDs/phases | Kết quả review | Việc quyết định closure |
|---|---|---|---|
| G31 | IL00–IL15; R0, R1a/b, R2, R3, R4/R4b, R5, R6a/b, R7a/b | **NOT IMPLEMENTED / NOT ACCEPTED** theo source baseline; primitives cũ không phải Investor Lens | Flag+golden, 4 / 6 / 3 / 3 / 3, neutral labels, 6 signals+confidence/freshness/trends, questions/risk/cap-table/liquidity, cohort CSV/API, 4-output parity, 20-user validation và slide source |
| G32 | A04/A05/V04; SV0–SV6, V04a/b | **PARTIAL DEPLOYED / NOT FULLY ACCEPTED** | Scenario methods/projections và pure shadow reducer live; rubric52, SCORE calls/cache/ledger, C+S+T−A no base/no cap, calibration, official report publish và all consumers migration còn. Pure reducer không đóng SV3 |
| G33 S0 | T01–T04 | PARTIAL LIVE; **AGGREGATE FIX DEPLOYED; THRESHOLD ACCEPTANCE OPEN** | `api/status/route.ts` nay buộc `ok:false` khi report status `down`;122tests và production release có receipt. `quality-log.ts` threshold khác plan cần reconcile riêng; watch vẫn advisory,0.56partial degraded không là56%no-report |
| G33 S1 | T05–T07, T16–T16k | PARTIAL; **ACCEPTANCE FAILED/OPEN** | QA13:36:5 pass / 1 fail, 2 degraded >1; chuỗi5 runs reset, không “còn1run”. Đủ free+paid, revision read-back, time/cost targets; task routing/no outside-policy fallback |
| G33 S2 | T08–T11 | FIXES LIVE / FULL ACCEPTANCE OPEN | Snapshot2 days; pending-authority0460/0461→canonical migration/manifest; signed reconciliation requirement vs5 pinned forks ghi explicit design delta |
| G33 S3 | F02/F04/T02/O05/O06/O08/E03 | PARTIAL/OPEN | Paid/legacy/all writers→immutable revision; lease/reclaim/reconcile; semantic verification; safe origin auto-retirement |
| G33 S4–S6 | G31/G32+U/R/V items | PLANNED/GATED | Hoàn tất toàn scope Lens/scoring/valuation, không chỉ snapshot UI; G30/S03 sale gates còn |
| G33 backlog | T12–T15 | OPEN/PARTIAL | Refresh dependency audit/PPTX; T13 owner fix không đóng route rate-limit; SSE MaxListeners và performance; O09 offsite backup DEFERRED |

#### Những thay đổi bắt buộc vào thứ tự§12.11

1. **S0/S1 trước:** status phản ánh core health, ổn định report/cost/DeepInfra. Probe13:59:[receipt](../reviews/2026-09-24-full-app-public-probes.json) trả `ok:true`, 27 runs, 3 noReport, 0.56 degraded, median grounded 0.67 vs KPI 0.85. `degradedShare` là run có bất kỳ chương degraded; không gọi.56 là56% fully-degraded. Chốt ngưỡng đúng semantics, không đổi tên metric để báo xanh.
2. **Prerequisite S3→S1:** kéo phần tối thiểu F02/T02/O08 cần cho canary writer/revision read-back và retry reconciliation lên trước exit S1; giữ full all-caller migration ở S3. Không để dependency vòng “S1 cần read-back nhưng writer sauS1”.
3. **Song song:** SV0 rubric/52 question catalog+16mapping, G31 fixtures/flag/read-only derivation, SV1 reader safety, inventory tất cả score→money consumers và O01 task-model qualification preparation. Không chờ7days để viết rubric; **SV3 shadow scoring chỉ bắt đầu sau khi S1 đạt 7 ngày; SV5 activation còn phải qua calibration và consumer-migration gates** (§12.11).
4. **Valuation prerequisite:** hoàn tất authenticated source producer/per-method eligibility thay vì để empty trusted-producer list rồi yêu cầu khách hàng rerun. Có pre-revenue branch; stated/verified/assumed tách rõ. Không bỏ evidence gates để hiện số. V01/V04 nằm trong critical path, không chỉ chỉnh copy.
5. **Criteria + persistence:** same final question ledger có đủ13 criteria/52 question states và16 investor mappings; giữ full narrative/citation IDs, không cắt600 chars khi save. Report public luôn có section/status định giá và catalog dù unavailable. Report cũ chỉ recover từ artifact có lineage hoặc tạo revision mới.
6. **All consumers:** sửa clamps **chỉ của index** trong mandates/saved views/cohort URL parser/adapter, index methodology và draft SVI `100+adjustment`; giữ thang0–100 của confidence/dimensions/min_fit. `cohort-snapshots.ts` clamp `evidence_confidence` là đúng, không phải index migration. Share-price/vesting/dividends dùng valuation contract riêng, giữ số đã phát hành. SVI dataroom không hiện giả0/16 khi chưa tích hợp; cap-table simulator phải gắn nhãn default assumptions.
7. **Whole-app exit:** auth/onboarding/project isolation; evidence; free/paid report+email/export; quote/credits/refund; equity/finance; investor/program/CRM; founder/tools/reseller; admin/API/cron/backup; mobile/EN-VI/a11y/navigation/copy/performance. Từng route inventory nối journey/owner/acceptance receipt trong U06; không bỏ persona hoặc utility để claim full.

#### Closure và provenance
...
| AI routing/cost/capacity | DeepInfra-only cho customer C-level/report theo chỉ đạo23/09; exact-ID admission, không fallback ngoài provider; quality gates giữ nguyên | O01/O02 role benchmark, account quota/concurrency audit, outage/load/cost evidence theo §11.3.5 và AI policy§8; catalog price không chứng minh quota/cost tối ưu |
...
- **26/09/2026 — Nghiên cứu thị trường cho định giá + rate-limit + sửa hash revision: LIVE `e59e6572e`** (qa:live 211/1 → spec dashboard cập nhật theo v4 `665549305`): Brave → Claude CLI WebSearch → R01 fetch → 1 lần DeepInfra trích xuất (≤ 5 nguồn, cache 7 ngày, trần 45 s, chạy song song GATHER); khối "Market references" + dòng cross-check (không phải phương pháp định giá); ops note `docs/ops/market-research.md`. Founder: nâng gói Brave (quota tháng = 0). Rate-limit các route POST công khai (proxy buckets public-ai/public-write/public-event/i18n-translate; 5 chỗ dùng X-Forwarded-For hop đầu → IP tin cậy); posture 197→638/691. `report_hash` revision dùng JSON sắp key (jsonb đổi thứ tự key). Tạm dừng email `svi-review` (bị sót ngày 25/09; 42 email ngày 21/09) + sửa đếm score_views.
- **26/09/2026 — /analyze: timeline tiến độ Biz Trust Report theo từng giai đoạn (khiếu nại của founder: sau khi upload deck trang trông như bị treo). LIVE `300e7f043` 26/09 (deploy-auto, qa:live 212/0):**
