...
- **Post-ship review** ([`reviews/post-ship-s25-2026-09-13.md`](./reviews/post-ship-s25-2026-09-13.md), GO): merged — P1 provider 401/429/5xx were snapshotted as "MRR 0" and rescored accounts *down* (typed `ConnectorHttpError`; 401/403 → `needs_reconnect`, else `failed`, nothing written) · P1 the `stripe`/**mpc** customer-count row lost its +15 (carve-out keyed on type AND dimension) · P2 v2 rows in `status="error"` were never retried. Second pass in flight: statements POST partial-failure refund/idempotent charge, Stripe `mrrAud: 0` must not beat Xero income, Xero evidence dimensions → SVI keys (0353), register-PDF test, void reason inline instead of `window.prompt()`.
