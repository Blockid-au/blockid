...
- **Still needs the founder (not blocking sales):** Drive off-site backup OAuth (`scripts/db-backup-offsite-auth.mjs`), off-box uptime/backup monitor account, revoke Telegram bot token + GitLab PAT, rotate `STRIPE_WEBHOOK_SECRET` + Supabase service key, GA4 Admin API enable + SA Editor, Cloudflare Scrape Shield → Email Obfuscation off, delete QA accounts (`qa-founder-20260912@…`, `qa-member-20260912@…`), G2 #10 real founder quotes, InfoVision ABN.
