...
*Last updated: 2026-07-24 by autonomous loop (tick shipping P1j — R&D Tax Incentive Chapter 6 section: adds a `sections[]` array to `06-revenue` in web/src/lib/guide/startup-journey.ts with a single `rnd-tax-incentive` section (heading + 9-bullet body EN + VI) covering refundable 43.5% offset, R&D-entity eligibility, s355-25 core-vs-supporting test, A$20k minimum + A$20M turnover thresholds, 10-month AusIndustry registration deadline, ATO Schedule 743 two-step process, contemporaneous-record obligations, excluded expenditure classes, and BlockID.au surfaces; renders `sections[]` in both /guide/[chapter] and /workspace/guide/[chapter] via a small map block mirroring the existing qualifyingTests styling; extends web/src/lib/guide/startup-journey.test.ts with a 3-case describe block pinning presence + statutory-anchor grep + single-chapter guard, all 15 tests green). This file survives `git reset --hard` because it is committed + pushed on every tick.*
