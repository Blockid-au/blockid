...
*Last updated: 2026-07-24 by autonomous loop (tick shipping P1k — compliance-calendar.ics generator: new pure module `web/src/lib/compliance/calendar.ts` (`buildComplianceCalendar` + `renderIcs`) plus authed route `GET /api/compliance/calendar` (`text/calendar; charset=utf-8`, `?download=1` → `compliance-calendar.ics` attachment) emitting three RFC 5545 VEVENT kinds inside a 12-month horizon: (a) BAS quarterly lodgment (Q1 28-Oct / Q2 28-Feb / Q3 28-Apr / Q4 28-Jul standard-FY self-lodger, gated on `compliance_gst_status.registered_for_gst`), (b) ASIC annual company review anchored on `equity_plans.incorporation_date` anniversary (`s 345A Corporations Act 2001 (Cth)`; skipped when incorporation_date absent / unparseable), (c) AusIndustry R&D Tax Incentive deadline (10-month post-FY, `IR&D Act 1986 s 27A`) reusing `buildRDCalendar()` rows so both surfaces share one FY-math source-of-truth; skips FYs `registered` / `not_applicable` / overdue. Every event carries a stable deterministic UID (`bas-fy{yyyy-yy}-q{n}@blockid.au`, `asic-annual-review-{yyyy}@blockid.au`, `rd-registration-fy{yyyy-yy}@blockid.au`), all-day `DTSTART;VALUE=DATE`, 14-day `VALARM` (`TRIGGER:-P14D`, matches guide Chapter 11 copy at `web/src/lib/guide/startup-journey.ts:870`), authoritative source `URL`, plus a calendar-level `X-WR-CALDESC` disclaimer. In-module RFC 5545 line-folding (75-octet CRLF chunks with continuation-space) and text-escape (`\,`, `\;`, `\\`, `\n`). Test coverage in `web/src/lib/compliance/calendar.test.ts` — 14 vitest cases across BAS gating, ASIC anniversary, R&D deadline skip-branches, and `renderIcs` RFC 5545 shape. All 64 compliance tests green (50 existing + 14 new). Closes §1 phase 11 P1 gap (compliance-calendar.ics generator missing) + the reminder half of §1 phase 6 P1 gap flagged by P1j. This file survives `git reset --hard` because it is committed + pushed on every tick.*
