...
*Last updated: 2026-07-24 by autonomous loop (tick shipping P6a — Div 83A ESOP scheme-rules gate: two-file split mirroring the ESIC gate. `web/src/lib/compliance/div83a-qualifying-tests.ts` exposes the 8-point ATO start-up concession checklist as a typed `readonly Div83AQualifyingTest[]` fixture (each row `{key, section, en, vi}` with a stable ITAA-1997 anchor per test) + `div83aQualifyingTestsFor(locale)` accessor + `DIV83A_QUALIFYING_TESTS_DISCLAIMER` — same source-of-truth copy previously duplicated inside Chapter 8 body at `web/src/lib/guide/startup-journey.ts:662-682`, so future ticks import it instead of re-declaring. `web/src/lib/compliance/div83a-funding-gate.ts` exports `assertDiv83AEligibleOrWarn(supabase, {userId, projectId, requireEligible, action})` mirroring `assertESICEligibleOrWarn()` shape exactly — reads `esop_option_grants` (migration 0089) scoped to `(user_id, project_id, status='active')` with worst-first precedence over 6 reasons (`grants_ineligible` → `grants_unsure` → `unchecked_grants` → `check_stale` → `no_grants_on_file` pass-through → `db_unavailable`); 180-day stale window (vs ESIC's 90 — grant terms rarely change between checks). Wired into `web/src/app/api/fundraise/route.ts` immediately after the ESIC gate; happy-path 200 now carries `esic_warn?` AND `div83a_warn?` when either fires, 412 blocks on wholesale-only rounds. Tests: 5 qualifying-tests cases (EN/VI parity + unique keys + s83A statutory anchor grep + verbatim match against Chapter 8 body via `getChapter("08-team")` + disclaimer presence) + 15 funding-gate cases (warn-only + wholesale-only branch matrices across empty/all-eligible/unchecked/stale/unsure/ineligible/db-unavailable). All 113 `src/lib/compliance/` tests green (93 existing + 20 new). Typecheck `npx tsc --noEmit -p tsconfig.json` green. Closes P6 exit-criterion #2 (Div 83A tests exposed as shared module + wired into fundraise gate). Prior tick (P1n-persist — WGEA + Modern Slavery persistence + dashboard wiring: schema migration `web/supabase/migrations/0112_compliance_wgea_modern_slavery.sql` (two tables `compliance_wgea_status` + `compliance_modern_slavery_status` mirroring the `compliance_gst_status` shape from 0108, RLS default-deny, `(user_id, project_id, computed_at DESC)` index), route `web/src/app/api/compliance/wgea-threshold/route.ts` (POST + GET mirroring `gst-threshold/route.ts` exactly — `getCurrentUser()` gate → JSON parse → required-field validation → `assessWGEA()` → persist scoped to `getActiveProject(user.id)` — WGEA_DISCLAIMER on every response), route `web/src/app/api/compliance/modern-slavery-threshold/route.ts` (same shape, required field `current_period_revenue_aud`, MODERN_SLAVERY_DISCLAIMER on every response), and dashboard `web/src/components/dashboard/compliance-panel.tsx` grows from 4 tiles to 6 with `WgeaTile` (5-branch action_required matrix) + `ModernSlaveryTile` (7-branch matrix) — grid switches from `lg:grid-cols-4` to `lg:grid-cols-3` for a 2×3 layout. Typecheck `npx tsc --noEmit` green. Completes the WGEA + Modern Slavery user-facing loop: founder POSTs headcount / revenue → detector classifies → snapshot persists → dashboard tile renders with the correct action band. Prior tick (P1n — WGEA + Modern Slavery threshold detectors: two pure helpers under `web/src/lib/compliance/` (`wgea-threshold.ts` + `modern-slavery-threshold.ts`) that mirror the P1i `gst-threshold.ts` shape so downstream cron / dashboard-panel wiring reuses the same result-envelope contract. WGEA anchors on WGE Act 2012 (Cth) s3 (100-employee "relevant employer" threshold) + s3B(2) (2-reporting-period grace once headcount drops below 100), 5-branch matrix (not_required / approaching / report_required — uses trailing-12mo peak headcount not just current so a mid-year hire spike still fires / grace_period / already_reporting), computes next_report_due_iso for the 1-Apr–31-Mar reporting period → 31-May-following deadline. Modern Slavery anchors on Modern Slavery Act 2018 (Cth) s5 (A$100M consolidated revenue + Australian-entity-or-carrying-on-business-in-AU scope) + s13 (statement due within 6 months of FY end, governing-body approval + responsible-member signature required) + s16 (7 mandatory reporting criteria), 6-branch matrix (not_required / approaching / statement_required / statement_due_soon / statement_overdue / already_lodged) with days_until_statement_due signed integer + statement_due_iso clamped to end-of-target-month so 31-Dec FY → 30-Jun-following-year not 1-Jul. Both surface a fixed disclaimer (WGE Act headcount excludes contractors + offshore + public-sector; MSA "consolidated revenue" follows AASB 10 controlled-entity aggregation) so no rendering surface can leak the classification without the hedge (avoids s923B / s911A Corps Act exposure). Tests: 13 wgea + 16 modern-slavery vitest cases across 4 describe blocks each, all 93 src/lib/compliance/ tests green (64 existing + 29 new). Closes §1 phase 11 P1 gap "WGEA + Modern Slavery threshold detectors (currently upload-only in data-room, no automated headcount/turnover trigger)" and §2 folder 12 items 12.5 + 12.6 (both were NUDGE with detector missing). Persist/route + dashboard-panel wiring is a follow-up (P1n-persist) — this tick ships the pure detector primitives only. Prior tick (P1l — primary vs secondary Chapter 10 split: web/src/lib/guide/startup-journey.ts:10-fundraise chapter now carries a `sections[]` entry `primary-vs-secondary` (heading + 8-bullet body EN + VI) using the previously-declared Chapter.sections slot — walks primary-issues-new-shares vs secondary-transfers-existing-shares, the Atlassian precedent (2010 Accel US$60M + 2014 T. Rowe US$150M @ US$3.3B post — both 100% secondary), when secondary is the right call, the s707 Corporations Act 2001 (Cth) on-sale rule + s708(1)/(8) + s761G(7) re-classification of every transferee, ROFR + pre-emptive-rights workflow anchored on Blackbird / AirTree templates, the Subscription Agreement (s254A + Form 484) vs Share Purchase Agreement (Form 484 + stamp-duty carve-outs) documentation split, tax fork (primary = capital contribution to company; secondary = CGT event A1 ITAA 1997 s104-10 to seller with Div 152 SBC / Div 115 50%-discount lookups + ESIC Div 360 offset DOES NOT survive secondary — s911A / s1041H exposure guard), and the BlockID wiring (Workspace → Fundraise deal_type tag → term-sheet AI clause-weight split → blockchain-hash worker identical → reseller Timeline surfaces deal_type). Tests: web/src/lib/guide/startup-journey.test.ts gains a "primary vs secondary section (chapter 10 P1l)" describe block — 2 vitest cases pinning presence + EN/VI alignment ≥ 6 bullets AND statutory + precedent anchor grep (Accel, T. Rowe, US$60M, US$150M, s707, s708, Right of First Refusal, CGT event A1, Subscription Agreement, Share Purchase Agreement, Div 360, Form 484). Pre-existing "no other chapter carries sections yet" guard now allow-lists chapters 06 AND 10 via a Set. All 17 startup-journey tests green (15 existing + 2 new). Closes §1 phase 10 P0 gap and open_question Q5. Prior tick (P1k — compliance-calendar.ics generator) full rationale preserved in the P1k entry above. This file survives `git reset --hard` because it is committed + pushed on every tick.*
