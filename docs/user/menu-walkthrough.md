...
**Known seam (documented, not fixed here):** the sidebar gates on a coarse 0–5 `currentPhase`. `/dashboard` derives it from the SVI band (`computePhase()`: <30 → 0, ≤50 → 1, ≤70 → 2, ≤85 → 3, ≤120 → 4, else 5), while the Next-unlock card reads `projects.growth_phase_current` (the 12-phase id). Table 1 maps the 12 phases onto the six steps using the bucketing declared in [`workflow-steps.ts`](../../web/src/lib/nav/workflow-steps.ts) `currentPhaseToStep()` (1–2 → validate, 3–5 → build, 6–8 → fundraise, 9–11 → grow, 12 → exit). Wiring `growth_phase_current` into the sidebar is the follow-up that would make the two agree by construction.
