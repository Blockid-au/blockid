...
## Phase 3.4 — Report Quality (G19) (S41–S45 LIVE 2026-09-20; S46 built, ships with the next deploy)
...
- [x] S43 Evidence & CTAs — pipeline reads `svi_dimension_evidence`; missing rows → linked CTA rows; one lift model; Money on the Table from gather — live 2026-09-20 (`Axp15hEhaiTKaArhuuH1c`)
- [x] S44 Synthesis + one phase rule + layout — cover "current value" hero, `/dashboard` executive synthesis from `report_v2`, `inferPhase` everywhere — live 2026-09-20 (`aff82c90a`)
- [x] S45 Paid view + i18n + clarity survey — purchase lands on ReportV2, `tbr-strings.ts` parity (VI diacritics), 0–10 survey + admin tile — live 2026-09-20 (`Axp15hEhaiTKaArhuuH1c`)
- [ ] S46 BlockID showcase + quality telemetry — `/showcase/blockid/report` (own report, weekly cron), `tbr-quality.jsonl` → `/api/status.tbr_quality` + admin tile, valuation-inputs / ledger prompt-eval fixtures; running our own report found and fixed 5 pipeline defects (markdown-vs-JSON contract, per-user cap, W4 token budget, nullable floor, `assembled_reports.analysis_id` 0415) — built 2026-09-20, ships with the next deploy (migration 0415 applied)
