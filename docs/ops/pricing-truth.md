...
| **Cohort Validation Pilot** (`cohort_pilot_25` / `cohort_pilot_50`, G21 P0-C) | A$1,500 inc. GST (≤ 25 applicants) / A$2,500 inc. GST (≤ 50) — `PILOT_SKUS` in `lib/pricing/pilot-skus.ts`, 90-day Cohort-tier entitlement | **not minted yet — founder mints** (`scripts/stripe/seed-pilot-prices.mjs --live`, one-off, `tax_behavior=inclusive`), catalogue rows carry `founder_mints: true` | `STRIPE_PRICE_COHORT_PILOT_25` / `STRIPE_PRICE_COHORT_PILOT_50` | `/solutions/accelerator#pilot`, `/pilot`, `/pricing?segment=programs` (`PilotBuyButton` → `/api/stripe/checkout {plan:"cohort_pilot_25"}`, `mode:"payment"`) | until the env var is set checkout answers `409 sku_unconfigured` and the button lands on `/contact?topic=pilot`; webhook → `pilot_orders` (migration 0416) + `startPaidPilot()` |
...
6. **G21 P0-C (F-2):** mint the two Cohort Validation Pilot prices — `node scripts/stripe/seed-pilot-prices.mjs --live` (A$1,500 / A$2,500 one-off, `tax_behavior=inclusive`) — then set `STRIPE_PRICE_COHORT_PILOT_25` / `STRIPE_PRICE_COHORT_PILOT_50` in `web/.env.production` and restart; checkout goes live without a deploy. Apply migration `0416_pilot_orders.sql` first.
...
## 12. Review before pay — no click ever opens Stripe (G25-D, 2026-09-21)

Founder rule 2026-09-21 (verbatim, Vietnamese): *"khi chuyển sang trang price, cần để người dùng đọc kỹ và tự nhấn nút pay mới chuyển qua stripe, chứ không tự động chuyển qua stripe page khi người dùng mới click vào trial 7 days hoặc price, phải click nút pay thì mới chuyển sang stripe page"* — a click on a plan, a price or "Start 7-day trial" **never** redirects to Stripe. The visitor lands on a review step, reads it, and only an explicit **Pay A$X now** / **Add card & start N-day free trial** click posts to a checkout route and follows the Stripe URL.

**The one review step:** `/checkout/review?plan=<id>[&interval=annual][&trial=1]&entry=<surface>` · `?pack=<credits>` · `?sku=founder_package|svi_api_team` (VI mirror `/vi/checkout/review`). Outside the `(app)` gate on purpose: a signed-out visitor sees the same review and its button is a **link** to sign-up (`/signup?plan=…&trial=1&next=<review>`, evaluator + programs rungs with `segment=evaluator`) or sign-in (`/auth/login?next=<review>` for packs / SKUs) — never Stripe. Unknown plan / pack / sku, the Free rung and no params → 404. Code: `lib/billing/checkout-review.ts` (order model, `checkoutReviewHref()`, `parseCheckoutReviewParams()`, `resolveCheckoutOrder()`), `lib/billing/checkout-review-strings.ts` (EN/VI `checkout.review.*` catalogue keys), `components/billing/checkout-review-card.tsx` (the ONE checkout-route caller), `app/checkout/review/checkout-review-page.tsx`.

What the review shows, all from the catalogue: plan name + included bullets (plans-v2 / `CREDIT_PACKS` / `STARTUP_PACKAGE_INCLUDED` / the `index_api` rung), **price inc. GST with the GST share** (`calculateGst` 1/11), billing interval (annual only when `annualAvailablePlanIds()` lists the rung — otherwise monthly + the fallback note), the trial line *"N-day free trial · card required · you can cancel before day N and pay nothing · then A$X per month"* (N = `plans.trial_days`: 7 founder / evaluator, 14 programs), renewal + self-serve cancellation (Billing / Stripe Billing Portal, `TRIAL_WARNING_HOURS_BEFORE` = 72 h reminder), the one-off / non-refundable line for packs and SKUs (ACL carve-out kept), the seller of record from `LEGAL_ENTITY` (`{entityOperator}` / `{entityAbnLabel}` tokens), the approved data-principle sentence (`solutions.principle.data`) and the general-advice line.

**Entry points → review (the table lane 34 walks):**

| Entry | Links to | Pay label on the review |
|---|---|---|
| `/pricing` card (founder / evaluator / programs), `/vi/pricing` | `/checkout/review?plan=<id>&trial=1[&interval=annual]&entry=pricing_card` (`/vi/…` on VI) | Add card & start N-day free trial |
| `/pricing` hero "Start 7-day free trial" | `…?plan=founder_growth&trial=1&entry=pricing_hero` | Add card & start 7-day free trial |
| `/signup?plan=…` (signed in) · `/onboarding?plan=…` (onboarded) · wizard v4 exit · legacy step-payment | `signedInSignupRedirect()` → `…&entry=signup` / `onboarding` (+ `origin=onboarding` from the legacy step) | Add card & start N-day free trial |
| `/workspace/billing` **Upgrade** · legacy `?plan=` deep link | `…&entry=billing` / `billing_deeplink` (the page never posts on mount any more) | Add card & start N-day free trial |
| `/workspace/billing#credits` **Buy** · SVI paywall packs · credit gate | `…?pack=<credits>&entry=credits` / `gate` | Pay A$X now |
| `/startup-package` CTA | `…?sku=founder_package&entry=startup_package` | Pay A$149 now |
| Upgrade modal (feature gates) | `…?plan=<suggested>&trial=1&entry=upgrade_modal` | Add card & start N-day free trial |
| `/workspace/settings/enterprise` "Upgrade to Team" | `…?sku=svi_api_team&entry=svi_api` | Pay A$299 now |
| Custom-priced rungs (Enterprise, VC Enterprise, Cohort Enterprise, Index API contact row) | `/contact?plan=<id>` — no card online | — |

The card-required **sign-up** (`/signup?plan=…&trial=1`) is itself a review: the Stripe Elements card field sits under a `signup-review` block (name, price inc. GST + GST share, trial line, renewal, seller, data principle) and the submit reads **"Add card & start N-day trial"** (`TRIAL_COPY.cta_card`). Nothing is submitted before that click; `?next=` is honoured on "Sign in" so an existing account returns to the review. The A$3 one-off reports (Trusted Business Report, Money Finder, One-Click) keep their G16 quote-then-pay panels ("Confirm & Pay A$3", price on the button) — they are not plan / trial clicks.

**Guard:** `src/lib/billing/checkout-entry.guard.test.ts` walks every browser module under `src/` for a caller of `/api/stripe/checkout`, `POST /api/credits` or `/api/svi-api/checkout` and fails unless the file is in its `ALLOW` map with a reason. Allowed today: `components/billing/checkout-review-card.tsx` (the review step) and, until G25 lane A's deletion merges, `components/marketing/PilotBuyButton.tsx` + `workspace/accelerator/pilot/pilot-convert-card.tsx` (both quote-then-confirm panels). **Do not extend the list to ship a new buy button — link it to `checkoutReviewHref()`.** The same test pins that Billing, the upgrade modal, the Startup Package CTA, the Index API section and the legacy onboarding step no longer fetch on mount or on the first click.

**Analytics:** `checkout_review_viewed` (once per review render; anonymous allowed with a per-tab session id) and `checkout_started` (only on the Pay click) carry `{ plan, kind, interval, trial, entry, amount_cents }` in the server union (`lib/analytics/events.ts`), the client GA4 map (`lib/analytics.ts`, `plan` dimension) and the funnel reducer (`scripts/lib/funnel-core.mjs`: steps `review_views` / `pay_clicks`, conversion `review_to_pay`); `/admin/funnel` shows the two rows and the review → pay rate. Live-qa lane 34 intercepts every checkout route and asserts nothing is posted before the Pay click; the Pay click's Stripe URL is captured and never opened (spend 0).
