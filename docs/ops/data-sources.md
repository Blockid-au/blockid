...
#     Headline, Announced By, Source URL — see fixtures/funding-sample.csv). G25-B:
#     commit it IN THE REPO as web/content/external-signals/funding-announcements-YYYY-MM.csv
#     (read first, every file; the first sheet — 2026-09, 33 rows — is already there) or,
#     for a one-off, save it as ~/blockid-data/external-signals/funding-announcements/funding-YYYY-MM-DD.csv
#     (read after the repo sheets). Rows without a checksum-valid ABN, an AUD amount or an
#     https source are skipped; the weekly cron needs no founder step.
