...
| View | Who | Content |
|---|---|---|
| **Grant report** (G25-C: first two full reports per e-mail) | guest / founder with an unused grant | **As shipped in v3.26.0:** the S32 first-analysis PDF + e-mail (`renderFirstAnalysisReportPdf`, no investment view) — routing the grant path through the ReportV2 pipeline so it ships this v3 document is a G28 item. **Target:** the **full standard document** (16 sections, no trim). E-mail = 1-page summary in the body (tiles ①–④, verdict + conditions, 5 key points, top 3 improvements) + the full PDF attached + secure link. |
