...
Root integration follow-up: the /analyze preliminary intake panel no longer renders the heuristic SVI monetary valuation while waiting for canonical results. It explains that a preliminary score is not business value. This addresses the known preview money gap on this surface; independent calculator/valuation routes remain separate. Full repeated UI checks deferred by founder, production build still required.
