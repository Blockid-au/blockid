...
The server passes a durable `context` containing `accountId`, `jobId`, `callId`, `originalMonth` and `grantId`. The stable call identity must be globally unique for the authorized durable job/purpose/review batch and survive retries; it must never be reused by another job or rotated to reset a call allowance. The context is cloned; the factory returns the existing `ResearchAttemptBudget` shape. It never creates an admission identity from a clock, request ID or random value. Random UUIDs are used only for temporary filesystem writes.
...
Every reserve obtains the same cross-process directory lock; it waits at most 1.5 seconds and never removes another worker's existing lock. A crashed lock requires explicit recovery. Directory creation is fsynced; ledger replacement uses an exclusive private temporary file, file fsync, atomic rename and directory fsync. A write/acknowledgment failure can leave a counted reservation, which is safe to reconcile, not permission to redispatch. Lock cleanup removes only the lock obtained by the current invocation. If an attempted durable write fails, the recovery lock is retained so a later caller cannot admit work against potentially stale or undercounted state. Capacity is bounded to 5,000 entries/4MB per month; exceeding storage capacity fails closed and is not a substitute for an admitted budget limit.

Inside the lock, the coordinator re-reads authorization and rechecks the clock after that asynchronous read. A new dispatch must still be in the **original** month. Policy/grant/model/payload scope must match; changed content under an existing policy/grant ID, changed job or call limits (including through another grant), call-ID reuse under another job, attempt-ID mismatch, or scope substitution is rejected. Existing reservation replay returns `dispatchAllowed:false`. The per-attempt ID exactly matches the existing hook's deterministic hash of durable call ID, provider, model and payload digest.
...
Missing, ambiguous, uncertified, zero or out-of-bound usage becomes sticky `unknown`, preserving the entire maximum. An identical settlement replay is idempotent until a contradiction has been recorded. A different later settlement restores the **entire original maximum**, clears accepted token totals, marks the entry unknown and stores both the original and first conflicting receipt digests. This restoration is committed and fsynced before the caller receives a reconciliation error. Every subsequent receipt, including the original one, is then rejected without reducing the full hold. Resolving unknowns needs a separate reviewed authoritative reconciliation procedure; this hook has no automatic refund or administrative override. Attempt counts never reset, and a failed settlement-authority check cannot modify the ledger.
...
Root-review follow-up: 24 coordinator/hook tests pass, including certified low usage → contradictory usage → full durable restoration → later reduction denied → another reservation denied by the restored account hold. A real failed filesystem commit during conflict handling retains the recovery lock. Call ceilings now have their own immutable binding across grants, and a call cannot be reassigned to a different job. Strict targeted TypeScript checks include production and test modules. The draft entry format adds `callLimitsBinding` and nullable `conflictSettlement`; no live ledger exists for this new coordinator. Any pre-follow-up experimental ledger must undergo explicit reviewed conversion or remain rejected, never acquire silently defaulted fields. Source only; no activation or provisioning.
