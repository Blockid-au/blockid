...
|---|---|---|---|---|
| `TELEGRAM_BOT_TOKEN` | never (public in git history for months; stripped from history 2026-09-10) | **REVOKED — replacement NOT installed** (2026-09-18: `getMe` → 401 Unauthorized from both env files; every Telegram alert was silently dropped until the e-mail fallback `web/scripts/ops-alert-email.mjs` shipped in G15) | `web/src/lib/telegram.ts`, 14 cron routes/scripts, `web/scripts/cron-runner.sh`, `scripts/lib/ops-alert.sh` | BotFather → `/revoke` → new token into `web/.env` + `.env.runtime` → `bash web/scripts/cron-runner.sh agent-guardian` proves alerts still post |
